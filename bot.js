...
try{
await pgPool.query(`CREATE TABLE IF NOT EXISTS bot_state ( key   TEXT PRIMARY KEY, value TEXT NOT NULL, updated_at TIMESTAMPTZ DEFAULT NOW() )`);
...
try{
await pgPool.query(
`INSERT INTO bot_state(key,value,updated_at) VALUES($1,$2,NOW()) ON CONFLICT(key) DO UPDATE SET value=$2, updated_at=NOW()`,
...
[OpenSea](${osUrl}) · [TraitView](${tvUrl})`) .setFooter({ text: `on-chain-all-stars · rank alert #${rankAlertCfg.min}–#${rankAlertCfg.max}` }) .setTimestamp(); // Resolve image try{ const imgResult = await resolveImage({ identifier: String(id) }, contract, 'ethereum'); if(imgResult?.type === 'buffer'){ const att = new AttachmentBuilder(imgResult.buffer, {name: imgResult.filename}); alertEmbed.setThumbnail(`attachment://${imgResult.filename}`); await alertChannel.send({ embeds:[alertEmbed], files:[att] }); } else { if(imgResult?.type === 'url') alertEmbed.setThumbnail(imgResult.url); await alertChannel.send({ embeds:[alertEmbed] }); } }catch(e){ await alertChannel.send({ embeds:[alertEmbed] }); } console.log(`[Rank Alert] #${id} OS Rank #${osRank} listed at ${priceStr}`);
}
...
await interaction.reply({content:`Removed **${value}** from ${filterType} filter for **${trait}**. Remaining ${filterType} filters: ${remaining}`, flags: MessageFlags.Ephemeral});
return;
}
...
*Tip: Add `RAILWAY_API_URL` env var to search full history.*`); return; } await interaction.editReply(`Found **${matched.length}** sale${matched.length===1?’’:‘s’} with **${trait}: ${value}** (OpenSea, last ~${pages*100}):`);
const cfg={…config,slug};
