...
// ── GET /db/sweep-cost ───────────────────────────────────────────────────────
// Calculates exact ETH needed to sweep the cheapest listed tokens matching filters.
// Query params:
//   count       — requested sweep count, default 10, max 100
//   groups      — JSON array of trait match groups; AND across groups, OR inside group
//   trait_count — optional exact trait count
//   rank_min/rank_max/rank_type — optional rank filter
// Returns: summary math + included listings sorted cheapest first.
app.get('/db/sweep-cost', auth, async (req, res) => {
  try {
    const requested = Math.min(Math.max(parseInt(req.query.count || '10'), 1), 100);
    const groups = parseTraitGroupsParam(req);
    const traitCount = req.query.trait_count ? parseInt(req.query.trait_count) : null;
    const rankMin = req.query.rank_min ? parseInt(req.query.rank_min) : null;
    const rankMax = req.query.rank_max ? parseInt(req.query.rank_max) : null;
    ...
    if (traitCount !== null && !isNaN(traitCount)) {
      conditions.push(`t.trait_count = $${p++}`);
      params.push(traitCount);
    }
    if (rankMin !== null && !isNaN(rankMin)) {
      conditions.push(`${rankCol} >= $${p++}`);
      params.push(rankMin);
    }
    if (rankMax !== null && !isNaN(rankMax)) {
      conditions.push(`${rankCol} <= $${p++}`);
      params.push(rankMax);
    }

    const whereClause = conditions.length ? `WHERE ${conditions.join(' AND ')}` : '';
    const fromClause = `FROM listings l JOIN tokens t ON t.id = l.token_id`;

    const countResult = await pool.query(
      `SELECT COUNT(*)::int AS available, MIN(l.price_eth) AS floor_before ${fromClause} ${whereClause}`,
      params
    );
    const available = parseInt(countResult.rows[0]?.available || 0);
    const floor_before = countResult.rows[0]?.floor_before != null
      ? parseFloat(countResult.rows[0].floor_before)
      : null;

    const listParams = params.slice();
    const limitParamIndex = listParams.length + 1;
    listParams.push(requested);
    const listingResult = await pool.query(
      `SELECT l.token_id, l.price_eth, l.url,
              t.os_rank, t.obs_rank, t.trait_count
       ${fromClause}
       ${whereClause}
       ORDER BY l.price_eth ASC, COALESCE(t.os_rank, t.obs_rank) ASC, l.token_id ASC
       LIMIT $${limitParamIndex}`,
      listParams
    );

    const listings = listingResult.rows.map(r => ({
      token_id: parseInt(r.token_id),
      price_eth: parseFloat(r.price_eth),
      url: r.url || null,
      os_rank: r.os_rank ? parseInt(r.os_rank) : null,
    ...
    const sweptIds = listings.map(l => l.token_id);
    let floor_after = null;
    if (sweptIds.length) {
      const afterParams = params.slice();
      const sweptParamIndex = afterParams.length + 1;
      afterParams.push(sweptIds);
      const afterWhere = conditions.length
        ? `${whereClause} AND l.token_id != ALL($${sweptParamIndex}::int[])`
        : `WHERE l.token_id != ALL($${sweptParamIndex}::int[])`;
      const afterResult = await pool.query(
        `SELECT MIN(l.price_eth) AS floor_after ${fromClause} ${afterWhere}`,
        afterParams
      );
      floor_after = afterResult.rows[0]?.floor_after != null
        ? parseFloat(afterResult.rows[0].floor_after)
        : null;
    }

    const sweep_count = listings.length;
    const total_eth = listings.reduce((sum, l) => sum + (l.price_eth || 0), 0);
    const avg_eth = sweep_count ? total_eth / sweep_count : null;
    const cheapest_eth = listings.length ? listings[0].price_eth : null;
    const highest_included_eth = listings.length ? listings[listings.length - 1].price_eth : null;
    ...
    res.json({
      ...
      avg_eth,
      cheapest_eth,
      highest_included_eth,
      floor_before,
      floor_after,
      listings,
      groups,
      trait_count: traitCount !== null && !isNaN(traitCount) ? traitCount : null,
      rank_min: rankMin !== null && !isNaN(rankMin) ? rankMin : null,
      rank_max: rankMax !== null && !isNaN(rankMax) ? rankMax : null,
      rank_type: rankType,
    });
  } catch(e) {
    console.error('/db/sweep-cost error:', e.message);
    res.status(500).json({ ok: false, error: e.message });
  }
});
