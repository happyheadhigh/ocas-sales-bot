...
async function getConstraintShape() {
  const r = await pool.query(`
    ...
    WHERE c.conrelid = 'token_traits'::regclass AND contype IN ('p','u')
  `);
  return r.rows;
}

async function main() {
  const sqlPath = path.join(__dirname, 'migrations', '005_fix_token_traits_unique_constraint.sql');
  if (!fs.existsSync(sqlPath)) { console.error('Could not find', sqlPath); process.exit(1); }
  const sql = fs.readFileSync(sqlPath, 'utf8');

  console.log('🚀 Running migrations/005_fix_token_traits_unique_constraint.sql ...\n');
