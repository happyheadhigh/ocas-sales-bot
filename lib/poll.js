...
async function pollListings(){
  for(const [guildId,config] of getAllConfigs()){
    for(const ctx of await buildPollContexts(guildId, config)){
    if((!ctx.listingsChannelId && !ctx.arbitrageChannelId)||!ctx.slug||ctx.paused) continue;
    const config = ctx; // alias so rest of function works unchanged
    try{
      ...
      if(!listingsChannel && !arbitrageChannel) continue; // nothing configured to actually post to
      ...
      if(!listingsChannel){
        // No regular Listings channel configured for this server — arbitrage
        // checks above still ran (if that channel was set), but there's
