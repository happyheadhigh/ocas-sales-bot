...
async function checkPriceAlerts(newListings, slug, pool){
  ...
  try {
    ...
    const res = await pool.query(
      `SELECT * FROM user_price_alerts WHERE slug=$1 AND token_id = ANY($2) AND triggered_at IS NULL OR repeat_alert=true`,
      [slug, tokenIds]
    );
  ...
  try {
    const res = await pool.query(
      `SELECT * FROM user_floor_alerts
       WHERE slug=$1 AND threshold_eth >= $2
       AND (last_alerted_at IS NULL OR last_alerted_at < NOW() - (cooldown_hours || ' hours')::interval)`,
      [slug, floorEth]
    );
    if(!res.rows.length) return;
    ...
    for(const alert of res.rows){
      try {
        ...
        if(!user) continue;
        const embed = new EmbedBuilder()
          .setTitle(`📉 Floor Alert — ${slug}`)
          .setColor(0xED4245)
          .setDescription([
            `The floor for **${slug}** has dropped to **Ξ ${parseFloat(floorEth).toFixed(4)}**`,
            `Your threshold: **Ξ ${parseFloat(alert.threshold_eth).toFixed(4)}**`,
          ].join('\n'))
          .setTimestamp();
        await user.send({ embeds: [embed] }).catch(()=>{});
        ...
        console.log(`[Floor Alert] Sent to ${alert.discord_id} for ${slug} at Ξ${parseFloat(floorEth).toFixed(4)}`);
