...
async function sendPersonalAlerts(event, type, config){
  ...
  for(const [userId, alert] of Object.entries(_userAlerts)){
    try{
      ...
      // jv confirmed live: still not getting DMs for a trait listing alert
      // even after the case-sensitivity fix to matchesFilters. Everything
      // upstream of this point looked structurally correct on inspection
      // (toPost passed into this function is the full, unfiltered new-
      // listings set -- not narrowed by the server's own listingFilters
      // first -- and persistence via dbLoad/dbSave looks sound), so rather
      // than guess further, logging exactly what this function sees for
      // any user whose alert actually reaches this check (passed slug/
      // paused/type gates) -- whether the real listing payload even has
      // trait data at the path being checked, and whether that's why the
      // match is failing.
