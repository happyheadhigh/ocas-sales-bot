...
async function pollListings(){
  for(const [guildId,config] of getAllConfigs()){
    ...
    try{
      ...
      // on startup — just advance the cursor so future listings post normally.
      if(newListings.length > 50){
        console.log('['+config.slug+'] Skipping '+newListings.length+' stale listings on startup (cursor advanced)');
        continue;
      }
