...
async function writeSalesToDb(rawSales, slug){
  ...
  try{
    ...
    const ins = await client.query(`
      ...
      RETURNING token_id, price_eth, currency, buyer, seller, sale_ts, tx_hash
    `, params);
    await client.query('COMMIT');
    // Sale alerts (lib/push.js): only rows actually inserted -> once per sale.
    try{ const push = require('./push'); const fresh = push.freshSales(ins.rows); if(fresh.length) push.notifySales(slug, fresh); }catch(e){ console.warn('[Poll sales] push hook:', e.message); }
...
async function pollSales(){
  for(const [guildId,config] of getAllConfigs()){
    ...
    //
    // jv: "I want that too" -- Token Alert is now a fully independent
    // alert record (_userTokenAlerts, its own slug/alertSales/paused),
    // not a field on this same trait-alert record -- checking both
    // stores, matching sendPersonalAlerts' own two-loop structure below.
    const hasPersonalSaleAlert = Object.values(_userAlerts).some(a =>
      !a.paused && a.alertSales && (!a.slug || a.slug === ctx.slug)
    ) || Object.values(_userTokenAlerts).some(a =>
      !a.paused && a.alertSales && (!a.slug || a.slug === ctx.slug) && Array.isArray(a.tokenIds) && a.tokenIds.length > 0
    );
...
async function pollListings(){
  ...
  for(const [guildId,config] of getAllConfigs()){
    ...
    // to do here.
    //
    // jv: "I want that too" -- same independent-record fix as pollSales'
    // own guard above.
    const hasPersonalListingAlert = Object.values(_userAlerts).some(a =>
      !a.paused && a.alertListings && (!a.slug || a.slug === ctx.slug)
    ) || Object.values(_userTokenAlerts).some(a =>
      !a.paused && a.alertListings && (!a.slug || a.slug === ctx.slug) && Array.isArray(a.tokenIds) && a.tokenIds.length > 0
    );
...
// Shared by both loops in sendPersonalAlerts() below -- building/sending
// the actual DM is identical either way, only which alert store the
// inline Pause/Stop buttons need to act on differs (encoded into their
// customId as :trait or :token so bot.js's handler knows which one).
async function _sendAlertDM(userId, event, type, config, kind){
  const dedupKey = `${userId}:${type}:${event.id||event.event_timestamp}`;
  if(alertedEventIds.has(dedupKey)) return;
...
async function sendPersonalAlerts(event, type, config){
  // jv: "I want that too" -- Trait Alert and Token Alert are now fully
  // independent alert records (see getTokenAlert/setTokenAlert above),
  // each with their own collection scope, Sales/Listing DM toggle, and
  // pause flag. Two separate loops below rather than one shared one,
  // since a user's trait alert and token alert can now be scoped to two
  // entirely different collections. The dedup check inside _sendAlertDM
  // still only ever sends one physical DM per user per event even if
  // both alerts somehow match the same event.
  for(const [userId, alert] of Object.entries(_userAlerts)){
    try{
      ...
      // independent-condition way tokenIds used to here: watching "any
      // token with exactly N traits" has no reason to also require a
      // trait name/value filter to be configured.
      //
      // jv: "the trait counts for each collection match traitview trait
      // counts correct?" -- they didn't for argonauts specifically
      // until this: traitview's own displayed count excludes a
      // per-collection "non-worn" category list (getTraitCount() /
      // countSaleTraits() in js/traitUtils.js) that this raw OpenSea
      // trait array knows nothing about on its own -- filtering it the
      // same way here before counting.
      const nonWorn = await getNonWornTraitCategories(config.slug);
      const countedTraits = nonWorn.length ? traits.filter(t => !nonWorn.includes(String(t.trait_type||'').toLowerCase())) : traits;
      const traitCountMatch = Array.isArray(alert.traitCountFilters) && alert.traitCountFilters.length > 0
        && alert.traitCountFilters.includes(countedTraits.length);
      let matched = traitCountMatch;
      if(!matched){
      ...
      await _sendAlertDM(userId, event, type, config, 'trait');
    }catch(e){ console.warn('[DM alert]',userId,e.message); }
  }

  // jv: "Does the bot support personal alerts for specific token #'s? Or
  // just traits?" -- fully independent record from the trait-alert loop
  // above: its own slug scope, its own alertSales/alertListings, its own
  // paused flag.
  for(const [userId, tokenAlert] of Object.entries(_userTokenAlerts)){
    try{
      if(tokenAlert.paused) continue;
      if(tokenAlert.slug && tokenAlert.slug !== config.slug) continue;
      if(type==='sale'&&!tokenAlert.alertSales) continue;
      if(type==='listing'&&!tokenAlert.alertListings) continue;
      if(!Array.isArray(tokenAlert.tokenIds) || !tokenAlert.tokenIds.length) continue;
      ...
      if(!Number.isFinite(eventTokenId) || !tokenAlert.tokenIds.includes(eventTokenId)) continue;
      await _sendAlertDM(userId, event, type, config, 'token');
    }catch(e){ console.warn('[DM alert:token]',userId,e.message); }
  }
}
...
// jv: trait-count alerts (below) need to count a token's traits the exact
// same way traitview's own site does -- js/config.js's
// nonWornTraitCategories, moved to this shared collections.
// non_worn_trait_categories column (see lib/db.js's migration comment)
// so both read the same value instead of two hardcoded copies that could
// drift apart. Cached with a short TTL so a change made there is picked
// up without a bot restart, without a DB round-trip on every single event.
const _nonWornCache = new Map(); // slug -> { list: string[] (lowercased), ts }
const NON_WORN_TTL = 10 * 60 * 1000;
async function getNonWornTraitCategories(slug){
  const cached = _nonWornCache.get(slug);
  if(cached && (Date.now() - cached.ts) < NON_WORN_TTL) return cached.list;
  try{
    const res = await pgPool.query(`SELECT non_worn_trait_categories FROM collections WHERE slug = $1`, [slug]);
    const list = (res.rows[0]?.non_worn_trait_categories || []).map(c => String(c).toLowerCase());
    _nonWornCache.set(slug, { list, ts: Date.now() });
    return list;
  }catch(e){
    return cached?.list || [];
  }
...
// jv: "I want that too" (following the earlier independent-pause change)
// -- Token Alert now needs its own collection scope and Sales/Listing DM
// toggle too, not just its own pause flag. A field on the same shared
// alert record can't do that (one record = one slug, one pair of DM
// toggles) -- this is a genuinely separate alert record per user now,
// with its own getTokenAlert/setTokenAlert/deleteTokenAlert mirroring
// the trait alert helpers exactly, rather than a nested field.
const _userTokenAlerts = {};

function getTokenAlert(userId){ return _userTokenAlerts[userId] || null; }

function setTokenAlert(userId, updates){
  _userTokenAlerts[userId] = { ...(_userTokenAlerts[userId] || {}), ...updates };
  dbSave('user_token_alerts', _userTokenAlerts).catch(()=>{});
}

function deleteTokenAlert(userId){
  delete _userTokenAlerts[userId];
  dbSave('user_token_alerts', _userTokenAlerts).catch(()=>{});
}
...
async function loadAllAlerts(){
  ...
  const tokenDb = await dbLoad('user_token_alerts');
  if(tokenDb) Object.assign(_userTokenAlerts, tokenDb);

  // One-time migration: token-# watching used to live as tokenIds/
  // tokenAlertPaused fields on this same _userAlerts record, before it
  // became its own independent alert type. Anyone who already set one up
  // (confirmed working by jv before this split) needs it carried over
  // into the new store rather than silently lost, and the old fields
  // removed from the trait alert record they never really belonged to.
  let traitAlertsChanged = false;
  for(const [userId, alert] of Object.entries(_userAlerts)){
    if(Array.isArray(alert.tokenIds) && alert.tokenIds.length && !_userTokenAlerts[userId]){
      _userTokenAlerts[userId] = {
        slug: alert.slug,
        tokenIds: alert.tokenIds,
        alertSales: alert.alertSales ?? true,
        alertListings: alert.alertListings ?? true,
        paused: alert.tokenAlertPaused || false,
      };
      delete alert.tokenIds;
      delete alert.tokenAlertPaused;
      traitAlertsChanged = true;
    }
  }
  if(traitAlertsChanged) dbSave('user_alerts', _userAlerts).catch(()=>{});
  if(Object.keys(_userTokenAlerts).length) dbSave('user_token_alerts', _userTokenAlerts).catch(()=>{});
}
