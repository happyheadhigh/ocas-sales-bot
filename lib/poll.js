...
function buildPollContexts(guildId, config){
  ...
  console.log('[PollCtx] guild='+guildId+' slug='+config.slug+' collectionSlug='+config.collectionSlug+' listingsCh='+config.listingsChannelId);
  if(config.slug){
    contexts.push({
      guildId,
      slug:             config.slug,
...
async function pollListings(){
  for(const [guildId,config] of getAllConfigs()){
    for(const ctx of buildPollContexts(guildId, config)){
    if(!ctx.listingsChannelId||!ctx.slug||ctx.paused){
      console.log('[PollListings] SKIP guild='+guildId+' slug='+ctx.slug+' listingsCh='+ctx.listingsChannelId+' paused='+ctx.paused);
      continue;
    }
    const config = ctx; // alias so rest of function works unchanged
    try{
      ...
      const toPostListings = toPost.filter(l=>{
        const traits = (l.asset&&l.asset.traits)||[];
        const result = matchesFilters(traits, config.listingFilters);
        const traitStr = traits.map(t=>t.trait_type+':'+t.value).join(', ');
        console.log('[ListingFilter] token='+( l.asset?.token_id||'?')+' traits=['+traitStr+'] filters='+JSON.stringify(config.listingFilters)+' match='+result);
        return result;
      });
      ...
      for(let i=0;i<embeds.length;i++){
        const embed=embeds[i]; if(!embed){ console.error('[Listing embed] null embed for index '+i); continue; }
        ...
        console.log('[Listing post] attempting to post #'+tokenId+' to channel '+channel.id);
        try{ await sendEmbed(channel,embed); console.log('[Listing post] SUCCESS #'+tokenId); }catch(e){ console.error('[Listing post] FAILED #'+tokenId+':', e.message); }
        await new Promise(r=>setTimeout(r,300));
      }
