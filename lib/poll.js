...
const { matchesFilters, formatEth, formatListingEth, shortAddr, isDiscordOk } = require('../utils/format');
...
async function buildTokenSearchEmbed(token, config, footerLabel){
  ...
  // Same per-server toggle as buildSaleEmbed/buildListingEmbed — some
  // servers want to lead with a large, full-width image instead of a
  // small thumbnail + full trait list. Keeps the compact "Stats: Traits: N"
  // count either way, only toggles the detailed trait breakdown + image size.
