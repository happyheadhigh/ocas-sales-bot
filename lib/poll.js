...
const { checkNewListingsForVaultValue } = require('./stackers-vault-listing-alerts');
  ...
  for(const [guildId,config] of getAllConfigs()){
    ...
    try{
      ...
      for(let i=0;i<embeds.length;i++){
        ...
        // Arbitrage check — posted to the same listings channel as its own,
        // clearly-distinguished follow-up embed rather than a separate
        // channel, since building a full new channel-selection flow in
        // /config wasn't warranted for this. Doesn't block or slow down the
        // regular listing post above (already sent), and a failure here
        // never affects the listing alert itself.
        if(tokenId){
          ...
          if(listingPriceEth){
            checkArbitrageOpportunity(config.slug, tokenId, parseFloat(listingPriceEth))
              .then(async (result) => {
                if(!result) return;
                const arbEmbed = buildArbitrageEmbed({ ...result, chain: config.chain || 'ethereum', name: lid?.asset?.name });
                await sendEmbed(channel, arbEmbed).catch(e => console.error('[Arbitrage post]', e.message));
              })
      ...
      // ── Stackers vault-listing alerts — checks ALL new listings, same
      // reasoning as the rank alert above, not just the trait-filtered
      // subset. Fire-and-forget: the module itself never throws by design,
      // and this must never affect the normal listing flow above, which
      // serves every other collection too.
      if(config.slug === STACKERS_SLUG){
        const priceByTokenId = new Map();
        const stackersTokenIds = [];
        for(const listing of toPost){
          const id = parseInt(
            listing.asset?.token_id ||
            listing.asset?.identifier ||
          ...
          if(!id) continue;
          stackersTokenIds.push(id);
          const eth = formatListingEth(listing);
          if(eth != null) priceByTokenId.set(id, parseFloat(eth));
        }
        if(stackersTokenIds.length){
          checkNewListingsForVaultValue(stackersTokenIds, priceByTokenId).catch(e =>
            console.warn('[StackersVaultAlerts]', e.message)
          );
        }
      }
