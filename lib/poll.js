  ...
  for(const [guildId,config] of getAllConfigs()){
    ...
    try{
      ...
      for(let i = 0; i < builtEmbeds.length; i++){
        ...
        if(!embed) continue;
        const tokenId = sale.nft?.identifier || sale.nft?.token_id || '';
        if(!tokenId || dedupeChannelPost(channel.id, tokenId)){
          try{ await sendEmbed(channel, embed); }catch(e){ console.error('[Sale post]', e.message); }
          await new Promise(r => setTimeout(r, 300));
        }else{
          console.log(`[Sale dedup] skipping #${tokenId} already posted to channel`);
        }
...
module.exports = {
  ...
  traitGroupsLabel, buildTokenSearchEmbed, buildPollContexts,
