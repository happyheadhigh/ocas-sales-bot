...
const { fetchOpenSeaMetadata, extractTraits } = require('./collection-backfill');
...
// slug -> setTimeout handle. A metadata reveal can fire this event for
// hundreds of tokens within seconds of each other -- recomputing ranks for
// the whole collection after every single one would mean redoing the same
// full-collection pass hundreds of times in a row for no benefit, since only
// the LAST one's result matters once the burst settles. Reset on every new
// event for that slug; only the final, settled state triggers a recompute.
const _rankRecomputeDebounce = new Map();
const RANK_RECOMPUTE_DEBOUNCE_MS = 30_000;
function _scheduleRankRecompute(slug, isOcas){
  ...
  const handle = setTimeout(() => {
    ...
    computeObsRanks(pgPool, slug, { isOcas }).catch(e => {
      console.warn(`[MetadataStream] [${slug}] Rank recompute after metadata update failed:`, e.message);
    });
...
function subscribeToCollection(slug){
  ...
  const unsubscribeMetadata = client.onItemMetadataUpdated(slug, async (event) => {
    ...
    try{
      ...
      // times in a row.
      _scheduleRankRecompute(slug, false);
    }catch(e){
