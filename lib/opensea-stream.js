...
const { dedupeChannelPostPersistent } = require('./cache');
...
async function handleItemListed(slug, event){
  try{
    ...
    for(const ctx of contextsForSlug(slug)){
      ...
      if(ctx.listingFilters && ctx.listingFilters.length){
      ...
      if(!posted) continue; // already posted (by Stream or the REST poller) — skip silently
