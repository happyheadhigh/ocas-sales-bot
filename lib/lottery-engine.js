...
function buildBurnLotteryEmbed({title='OCAS Burn Lottery', prize, mode, start, discordPing=null, end, seed, entries, wallets, burns, pick, lotteryId, timezone=DEFAULT_LOTTERY_TIMEZONE, seedMeta=null}){
...
if(pick?.winner) embed.addFields({ name:'Winner', value:`${etherscanAddressLink(pick.winner)}${discordPing ? '\n' + discordPing : ''}`, inline:false });
  if(pick?.proof){
...
// NOTE FOR JV: this is preserved from main's "lottery winner announcement
// cards" work (not present anywhere in bot-modular before this merge). It is
// NOT currently called by drawAndPostBurnLottery below -- that function still
// only sends buildBurnLotteryEmbed for both the general result and the
// winner announcement, same as it did before this merge. If you want the
// dedicated winner-card behavior main had, drawAndPostBurnLottery needs an
// additional call to this function where it currently sends resultEmbed.
    ...
    try{
      ...
      if(ch){
        ...
        if(originalMsg){
          ...
          const bFields = fetchingEmbed.data.fields || [];
          const walletsIdx = bFields.findIndex(f => f.name === 'Qualified Wallets');
          const burnsIdx   = bFields.findIndex(f => f.name === 'Total Burns');
          if(walletsIdx >= 0) bFields[walletsIdx] = { ...bFields[walletsIdx], value: String(wallets.length) };
          if(burnsIdx >= 0)   bFields[burnsIdx]   = { ...bFields[burnsIdx],   value: String(burns.length) };
          fetchingEmbed.setFields(bFields);
...
const discordPing = await lookupDiscordPing(pick?.winner || null);
  ...
  const resultEmbed = buildBurnLotteryEmbed({discordPing, title:row.title||'OCAS Burn Lottery', prize:row.prize, mode:row.mode, start, end, seed:drawSeed, entries, wallets, burns, pick, lotteryId:row.id, timezone:timeZone, seedMeta});
  ...
  if(discordPing){
    const ch = await resolveDiscordChannel(row.channel_id);
    if(ch) await ch.send({ content:`${discordPing} Congratulations — you won the OCAS Burn Lottery! 🎉` }).catch(()=>{});
  }
}
...
function buildGenericLotteryComponents(lotteryId, type='giveaway', active=true){
  ...
  } else if(active && type === 'guess'){
    // Active guess lottery — opens a modal asking for the number
    rows.push(new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId(`lottery_guess:${lotteryId}`).setLabel('Submit Guess').setStyle(ButtonStyle.Success)
    ));
  } else {
...
function buildGenericLotteryStartEmbed(row, count=0){
  ...
  if(type === 'guess'){
    embed.addFields(
      ...
      { name:'How to Play', value:'Click **Submit Guess** below.',                              inline:false },
    );
  } else {
    embed.addFields(
      { name:'Entries',    value:String(count),                                                inline:true },
      { name:'How to Enter', value:'Click **Enter Giveaway** below.', inline:false },
    );
  }
...
// NOTE FOR JV: same situation as buildBurnLotteryWinnerEmbed above -- preserved
// from main's "lottery winner announcement cards" work, not present anywhere
// in bot-modular before this merge, and NOT currently called anywhere. Wire
// this in wherever the generic-lottery equivalent of drawAndPostBurnLottery
// currently sends buildGenericLotteryResultEmbed, if you want the dedicated
// winner-card behavior main had for giveaways/guess-the-number too.
...
async function processDueGenericLotteries(){
  ...
  for(const row of r.rows){
    try{
      ...
      if(row.message_id && row.channel_id){
        try{
          ...
          if(ch){
            originalMsg = await ch.messages.fetch(row.message_id).catch(() => null);
            if(originalMsg){
              const liveCount = await getGenericLotteryEntryCount(row.id);
              const fetchingEmbed = EmbedBuilder.from(originalMsg.embeds[0])
                .setDescription('⏳ Entry window closed — fetching Ethereum block hash for tamper-proof seed...');
              const fFields = fetchingEmbed.data.fields || [];
              const fEntriesIdx = fFields.findIndex(f => f.name === 'Entries');
              if(fEntriesIdx >= 0) fFields[fEntriesIdx] = { ...fFields[fEntriesIdx], value: String(liveCount) };
              fetchingEmbed.setFields(fFields);
      ...
      const drawResult = await drawGenericLottery(row, true, ethSeed, ethBlockNumber, true);

      // Update original message to show draw complete
      if(originalMsg){
        try{
          const finalCount = drawResult?.entries?.length ?? await getGenericLotteryEntryCount(row.id);
          const doneEmbed = EmbedBuilder.from(originalMsg.embeds[0]).setDescription('✅ Draw complete.');
          // The original embed's Entries field is stale — it reflects however
          // many people had entered at the moment the giveaway was posted
          // (usually 0), not the actual final count. Update it in place
          // rather than leaving that stale number displayed forever.
          const fields = doneEmbed.data.fields || [];
          const entriesIdx = fields.findIndex(f => f.name === 'Entries');
          if(entriesIdx >= 0) fields[entriesIdx] = { ...fields[entriesIdx], value: String(finalCount) };
          doneEmbed.setFields(fields);
...
module.exports = {
  buildBurnLotteryEmbed, buildBurnLotteryWinnerEmbed, buildActiveBurnLotteryComponents, buildBurnLotteryComponents,
  buildGenericLotteryStartEmbed, buildGenericLotteryResultEmbed,
  buildGenericLotteryWinnerEmbed, formatGenericLotteryWinner,
