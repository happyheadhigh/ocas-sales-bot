...
if(!rpcUrl){
  console.warn('[Burn] No RPC configured; set ETH_RPC_URL, ALCHEMY_RPC_URL, ALCHEMY_WEBSOCKET_URL, or ALCHEMY_API_KEY');
  return;
}
...
if(!Number.isFinite(latest)){
  throw new Error(`eth_blockNumber returned non-finite raw=${JSON.stringify(latestRaw)} rpc=${rpcHostForLog(rpcUrl)}`);
    ...
    if(lastBlockRaw === 'NaN' || (lastBlockRaw && !Number.isFinite(parseInt(lastBlockRaw, 10)))){
      console.warn('[Burn] burn_last_block is invalid ('+lastBlockRaw+'), resetting to latest-2000');
      await dbSave('burn_last_block', String(Math.max(0, latest - 2000)));
    }
    const lastBlockClean = (lastBlockRaw && lastBlockRaw !== 'NaN' && Number.isFinite(parseInt(lastBlockRaw, 10))) ? lastBlockRaw : null;
    ...
    console.log(`[Burn] Cursor state last=${lastBlockRaw || 'none'} from=${fromBlock} latest=${latest} chunk=${BURN_BLOCK_CHUNK} rpc=${rpcHostForLog(rpcUrl)}`);
