...
const { burnRpc, burnRpcUrl, rpcHostForLog, getBurnBlockTimestamp, fetchTokenUriFromContract, isTransientRpcError } = require('./rpc');
...
const { shortAddr, isDiscordOk, verifyImageIsRaster } = require('../utils/format');
  ...
  try{
    ...
    if(contractImgSrc){
      ...
      if(!imgResult && contractImgSrc.startsWith('http') && isDiscordOk(contractImgSrc)){
        if(await verifyImageIsRaster(contractImgSrc)){
          imgResult = { type:'url', url:contractImgSrc };
        } else {
          const buf = await extractPngFromSvg(contractImgSrc).catch(() => null);
          if(buf) imgResult = { type:'buffer', buffer:buf, filename:`token-${survivorId}.png` };
        }
      }
    }
...
// pendingBurns: survivorTokenId -> ARRAY of not-yet-finalized start events, oldest
// first. This used to be a single-slot Map (one startEvent per survivor), which
// silently broke when the same survivor was burned more than once in quick
// succession: a second BurnStarted would overwrite the first's slot before its
// matching BurnFinalized arrived, so that finalize (and every one after it)
// would get matched to the WRONG start event's fuel-token list. Confirmed via
// check-repeated-burn-inputs.js: entire fuel-token groups were being duplicated
// across multiple distinct, correctly-blocked finalize transactions for the
// same survivor. A FIFO queue fixes this: each finalize always consumes the
// OLDEST still-pending start for that survivor, matching on-chain order.
const pendingBurns = new Map();
function queuePendingBurn(survivorTokenId, startEvent){
  const key = String(survivorTokenId);
  const queue = pendingBurns.get(key) || [];
  queue.push(startEvent);
  pendingBurns.set(key, queue);
}
function takeNextPendingBurn(survivorTokenId){
  const key = String(survivorTokenId);
  const queue = pendingBurns.get(key);
  if(!queue || !queue.length) return null;
  const next = queue.shift();
  if(queue.length) pendingBurns.set(key, queue);
  else pendingBurns.delete(key);
  return next;
}
// After a finalize successfully consumes a start event (from either the
// in-memory queue above or the DB fallback below), mark that specific
// burn_started_events row as matched so the DB fallback never re-selects it
// for a later, unrelated finalize of the same survivor.
async function markBurnStartMatched(burnStartedId, burnEventId){
  if(!burnStartedId || !burnEventId) return;
  try{
    await pgPool.query(
      `UPDATE burn_started_events SET matched_burn_event_id=$1 WHERE id=$2`,
      [burnEventId, burnStartedId]
    );
  }catch(e){ console.warn('[Burn] markBurnStartMatched failed:', e.message); }
}
...
if(snapshotInputs){
  ...
  console.log(`[BurnMeta] Skipping input snapshots for historical burn start #${event.survivorTokenId}`);
    ...
    // Queue in memory for cross-referencing with BurnFinalized (FIFO — see
    // queuePendingBurn/takeNextPendingBurn above). burnStartedId is carried
    // along so the finalize handler can mark this exact row consumed.
    queuePendingBurn(event.survivorTokenId, {
      burnStartedId,
      owner:         event.owner,
...
async function storeBurnFinalized(finalEvent, startEvent){
  try{
    ...
    // Push alerts (lib/push.js): only for a genuinely NEW burn row -- this is
    // an upsert that re-runs over the same burns during catch-up, and an
    // update must not re-notify.
    if(r.rows[0]?.inserted && burnedIds.length){
      try{ require('./push').notifyBurns(OCAS_SLUG, burnedIds.map(Number).filter(Number.isFinite)); }catch(e){ console.warn('[Burn] push hook:', e.message); }
    }
...
async function pollBurnEventsLegacy(){
  ...
  try{
    ...
    // Small safety margin below the reported head — Alchemy is load-balanced
    // across multiple nodes, and this eth_blockNumber call and the
    // eth_getLogs call just below it aren't guaranteed to land on the same
    // one. If the getLogs call hits a node a block or two behind this one,
    // it correctly rejects a range it doesn't consider to exist yet ("block
    // range extends beyond current head block") — matches the intermittent
    // nature of the errors seen, not a deterministic logic bug. Costs at
    // most a few seconds of extra delay on Ethereum, negligible for burn
    // detection specifically.
    const toBlock = Math.max(0, parseInt(j2.result,16) - 2);
    ...
    for(const log of logs){
      ...
      try{
        ...
        // Cross-reference with BurnStarted data — FIFO: always the oldest
        // still-pending start for this survivor (see takeNextPendingBurn).
        const startEvent = takeNextPendingBurn(survivorTokenId) ||
        ...
        await markBurnStartMatched(startEvent?.burnStartedId, finalEvent.burnEventId);
...
// Consecutive polls that failed on a temporary RPC error (see catch below).
let _burnRpcFailStreak = 0, _burnRpcLastAlertTs = 0, _burnRpcAlerted = false;
const BURN_RPC_ALERT_STREAK = 10;
    ...
    try{
      ...
      if(process.env.BURN_RPC_OVERRIDE){
        if(blockGapEst > 100){
          ...
          // This used to just log a message claiming the override was
          // ignored without actually reassigning rpcUrl -- since rpcUrl
          // was already set to the override a few lines above (burnRpcUrl()
          // always checks BURN_RPC_OVERRIDE first, regardless of catch-up
          // status), every single poll kept hitting Infura indefinitely
          // any time BURN_RPC_OVERRIDE was left set in Railway past the
          // actual catch-up window -- not just during catch-up as intended.
          rpcUrl = burnRpcUrl(false);
      ...
      // Same load-balanced-node race as the other eth_getLogs call in this
      // file (see comment there) — small safety margin below latest,
      // specifically for the getLogs upper bound only. latest itself stays
      // untouched everywhere else (blockGapEst, lag-alert threshold,
      // logging) since those should reflect the true chain head.
      const safeLatest = Math.max(0, latest - 2);
      if(fromBlock > safeLatest) return; // nothing safely queryable yet this cycle — next tick will have enough of a gap
      const chunkTo = Math.min(safeLatest, fromBlock + adaptiveChunk - 1);
      ...
      // A clean poll after an alerted run of RPC failures: say so once.
      if(_burnRpcFailStreak >= BURN_RPC_ALERT_STREAK && _burnRpcAlerted){
        sendErrorWebhook('Burn Poller Recovered', new Error(`RPC calls succeeding again after ${_burnRpcFailStreak} failed polls`));
      }
      _burnRpcFailStreak = 0; _burnRpcAlerted = false;
    }catch(e){
      console.error('[Burn poller]', e.message);
      // jv: "Keep getting these errors" (Alchemy 503s). The cursor only
      // advances after a successful fetch, so a failed poll just retries
      // the same blocks 30s later -- no burns are lost, and a sustained
      // outage is already caught by the lag alert above. Temporary RPC
      // failures (already retried inside burnRpc) now only alert when they
      // persist: 10 consecutive failed polls (~5 min), then at most every
      // 30 min, plus one "recovered" message. Every other error still
      // alerts immediately, exactly as before.
      if(isTransientRpcError(e)){
        _burnRpcFailStreak++;
        if(_burnRpcFailStreak >= BURN_RPC_ALERT_STREAK && Date.now() - _burnRpcLastAlertTs > 30 * 60 * 1000){
          _burnRpcLastAlertTs = Date.now(); _burnRpcAlerted = true;
          sendErrorWebhook('Burn Poller Error', e, `RPC failing for ${_burnRpcFailStreak} consecutive polls (~${Math.round(_burnRpcFailStreak * 0.5)} min); each retried 3x. Will retry the same blocks every 30s.`);
        }
      }else{
        sendErrorWebhook('Burn Poller Error', e);
      }
    }
...
async function loadBurnStartFromDB(survivorTokenId){
  ...
  //
  // FIFO, not "most recent": if this survivor was started more than once
  // before either finalized (or the bot restarted between them), we must
  // pick the OLDEST start that hasn't already been matched to a finalize —
  // the previous "ORDER BY block_number DESC" always grabbed the newest
  // start regardless of which finalize was asking, which is the same class
  // of bug as the in-memory pendingBurns overwrite this file just fixed.
  // matched_burn_event_id is set by markBurnStartMatched() once a start is
  // consumed, so it's excluded from being picked again.
  try{
    const r = await pgPool.query(
      `SELECT bse.id, bse.owner_wallet, bse.points_used, bse.result_body_type, bse.result_is_angel,
       ...
       WHERE bse.survivor_token_id = $1 AND bse.matched_burn_event_id IS NULL
       GROUP BY bse.id
