...
async function buildBurnEmbed(finalEvent, startEvent, overrideTraits = null, showSurvivorField = false){
  ...
  // Get survivor's pre-burn type for display
  let survivorPreBurnType = null;
  try{
    const survivorSnap = await pgPool.query(
      `SELECT traits_json FROM token_image_snapshots
       WHERE token_id=$1 AND source='burn-start-input'
       ORDER BY id DESC LIMIT 1`,
      [parseInt(survivorId)]
    );
    if(survivorSnap.rows[0]?.traits_json){
      const tj = typeof survivorSnap.rows[0].traits_json === 'string'
        ? JSON.parse(survivorSnap.rows[0].traits_json)
        : survivorSnap.rows[0].traits_json;
