...
const {
  ...
  ALCHEMY_KEY, RANK_SYNC_DELAY_MS,
} = require('./constants');
const {
  ...
  burnTypeLabel, burnTypeColor, burnTypeEmoji, normalizeOcasType,
...
const { snapshotTokenFromContract, resolveImage, extractPngFromSvg } = require('./images');

let _lastLagAlertTs = 0; // debounce lag alerts
const { imageCache, setCachedImage } = require('./cache');
...
async function buildBurnEmbed(finalEvent, startEvent, overrideTraits = null, showSurvivorField = false){
  ...
  const burnedCountStr = trulyBurnedIds.length
  ...
  try{
    ...
    // 1. burn-start-input snapshot (exists if survivor was previously burned)
    ...
    if(snap1.rows[0]?.traits_json){
      ...
      survivorPreBurnType = normalizeOcasType(tj?.Type || tj?.type || null);
    }

    // 2. burn_state_snapshots — post-state from a previous burn event (pre-state for this burn)
    if(!survivorPreBurnType && burnEventId){
    ...
    // 3. token_original_snapshots — original mint traits
    if(!survivorPreBurnType){
      ...
      if(snap3.rows[0]?.traits_json){
        const tj = typeof snap3.rows[0].traits_json === 'string'
          ? JSON.parse(snap3.rows[0].traits_json) : snap3.rows[0].traits_json;
        survivorPreBurnType = normalizeOcasType(tj?.Type || tj?.type || null);
      }
    }

    // 4. backfill-chunks snapshot — original mint traits from backfill
    if(!survivorPreBurnType){
      const snap4 = await pgPool.query(
        `SELECT traits_json FROM token_image_snapshots
         WHERE token_id=$1 AND source='backfill-chunks'
         ORDER BY id DESC LIMIT 1`, [sid]
      );
      if(snap4.rows[0]?.traits_json){
        const tj = typeof snap4.rows[0].traits_json === 'string'
          ? JSON.parse(snap4.rows[0].traits_json) : snap4.rows[0].traits_json;
        survivorPreBurnType = normalizeOcasType(tj?.Type || tj?.type || null);
      }
    }

    // 5. token_traits — current traits (last resort, may be post-burn)
    if(!survivorPreBurnType){
      const snap5 = await pgPool.query(
        `SELECT trait_value FROM token_traits WHERE token_id=$1 AND LOWER(trait_name)='type' LIMIT 1`, [sid]
      );
      if(snap5.rows[0]?.trait_value)
        survivorPreBurnType = normalizeOcasType(snap5.rows[0].trait_value);
    }
  }catch(_){}
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      // Show Survivor field: always when showSurvivorField=true (from /burn command),
      // or when type changed (e.g. was Zombie → now Angel) for auto-posts/burnlatest
      ...(survivorPreBurnType && (showSurvivorField || survivorPreBurnType !== (isAngel ? 'Angel' : normalizeOcasType(E1_TYPE_NAMES[bodyType])))
        ? [{ name:'Survivor', value:`#${survivorId} · was ${survivorPreBurnType} → now ${isAngel ? 'Angel' : normalizeOcasType(E1_TYPE_NAMES[bodyType]) || 'Unknown'}`, inline:false }]
        : []),
    );
