...
const { burnRpc, burnRpcUrl, getBurnBlockTimestamp, fetchTokenUriFromContract } = require('./rpc');
...
async function pollBurnEvents(){
  ...
  try{
    ...
    if(!Number.isFinite(latest)) throw new Error(`eth_blockNumber returned non-finite: ${latest}`);
