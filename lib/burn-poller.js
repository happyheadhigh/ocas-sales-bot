...
async function pollBurnEvents(){
  ...
  try{
  let rpcUrl = burnRpcUrl(); // default; may be overridden below once block gap is known
  // Auto-switch: BURN_RPC_OVERRIDE used only while >100 blocks behind.
  // Within 100 blocks (~20 min of chain), falls back to Alchemy automatically.
  // Set BURN_RPC_OVERRIDE=https://mainnet.infura.io/v3/KEY + BURN_BLOCK_CHUNK=5000
  // in Railway to catch up fast. No need to remove it — auto-ignored when live.
if(!rpcUrl){
...
if(!effectiveLastBlockRaw && _lastKnownBurnBlock !== null){
  effectiveLastBlockRaw = String(_lastKnownBurnBlock);
  console.warn(`[Burn] burn_last_block DB read returned empty; using in-memory cursor ${effectiveLastBlockRaw} to avoid rewind`);
}
...
if(!Number.isFinite(latest)){
  ...
  await dbSave('burn_last_block', String(resetBlock));
...
if(!fromBlock){
  ...
  rpcUrl = (process.env.BURN_RPC_OVERRIDE && blockGapEst > 100)
    ? (process.env.BURN_RPC_OVERRIDE.startsWith('wss://') ? process.env.BURN_RPC_OVERRIDE.replace('wss://','https://') : process.env.BURN_RPC_OVERRIDE)
    : burnRpcUrl();
  if(process.env.BURN_RPC_OVERRIDE && blockGapEst <= 100)
    ...
    // Auto-switch chunk size: use 5000 with override RPC, normal cap with Alchemy
