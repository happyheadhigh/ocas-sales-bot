...
async function checkAndBackfillNewTokens(col){
  ...
  try{
    ...
    let written = 0;
    let cursor = startId;
    // Mint alerts (lib/push.js): ids that already existed before this run,
    // so only genuinely new tokens are reported (ranges overlap a little).
    const _known = new Set((await pgPool.query(`SELECT id FROM tokens WHERE collection_slug = $1 AND id >= $2`, [slug, startId]).catch(() => ({ rows: [] }))).rows.map(r => Number(r.id)));
    const _mintedIds = [];
    ...
    while(cursor < ceiling){
      ...
      for(const n of batchNfts){
        const raw = n?.tokenId ?? n?.id?.tokenId;
        const id = typeof raw === 'string' && raw.startsWith('0x') ? parseInt(raw, 16) : parseInt(raw, 10);
        if(Number.isFinite(id) && !_known.has(id)){ _known.add(id); _mintedIds.push(id); }
      }
    ...
    if(written > 0){
      ...
      if(_mintedIds.length){
        try{ require('./push').notifyMints(slug, _mintedIds.sort((a, b) => a - b)); }catch(e){ console.warn('[new-token-poller] push hook:', e.message); }
      }
    }
  }catch(e){
