...
const {
  SUPPORTED_CHAINS, fetchOnChainTotalSupply, fetchTokenRangesBulk, writePage,
...
async function checkAndBackfillNewTokens(col){
  ...
  try{
    ...
    const onChainSupply = await fetchOnChainTotalSupply(contract, chain, ALCHEMY_KEY);
    if(onChainSupply == null) return; // contract doesn't implement totalSupply(), or the read failed -- nothing safe to compare against
