...
const { fetchTokenUriFromContract, realTraitCount, realWornTraitCount, traitsArrayFromInput, traitsObjectFromArray, getTraitImageSource } = require('./rpc');

const fetch  = require('node-fetch');
// Sharp/libvips rendering moved to the API service (/render/svg-token) —
// running Sharp in this long-lived bot process caused native memory that
// was never released, leading to OOM crashes after several hours.
const { EmbedBuilder, AttachmentBuilder } = require('discord.js');
const { getCachedImage, setCachedImage, imageCache } = require('./cache');
const { isSvg, isDiscordOk, verifyImageIsRaster } = require('../utils/format');
const { osHeaders, OCAS_SLUG, getRailwayApiUrl, TOKEN_META_CACHE_MAX, TOKEN_META_CACHE_EVICT } = require('./constants');
const { pgPool } = require('./db');

// ── SVG → PNG via remote render endpoint ──────────────────────────────────────
// Calls the API service's /render/svg-token endpoint instead of running Sharp
// locally. Returns a PNG buffer, same interface as before. Optional `size`
// (defaults to 500, matching every existing caller's expectation) renders
// natively at that resolution in one pass, instead of the old approach of
// always rendering at 500 then separately upscaling via a second
// nearest-neighbor pass — avoids compounding rounding artifacts from two
// sequential scales when the source (e.g. a small embedded on-chain bitmap)
// doesn't divide evenly into 500, and lets any genuine vector content in the
// SVG render with real anti-aliasing at the higher resolution instead of
// being rasterized small and then blockily scaled up.
// Detects whether this code is running within the API service's own
// process (started via `node api.js`) rather than the bot's (`node bot.js`
// / `node bot-market-download.js`). Checked via the actual entry script
// path rather than an env var that would need to be manually set and could
// be forgotten — this is true regardless of configuration, based on how the
// process was actually started. Matters because sharp is a shared
// dependency in the same package.json (installed in both processes'
// node_modules regardless of which one's running), so "is sharp available"
// can't distinguish them — but the point of moving rendering to the API
// service in the first place was to keep sharp's native memory OUT of the
// long-lived bot process specifically, not to avoid installing it at all.
function isRunningAsApiService(){
  return /(^|[\\/])api\.js$/.test(process.argv[1] || '');
}

async function extractPngFromSvg(svgSource, size = 500){
  // Confirmed live: this function running from WITHIN the API service's own
  // process (e.g. the metadata-update poller's catch-up scan, triggered via
  // an API-service endpoint) tried to call back out to itself over HTTP
  // using RAILWAY_API_URL — a URL that was never configured for that
  // direction, since the API service never previously needed to know its
  // own address. Every image render failed as a result. Render directly,
  // in-process, via the same sharp-based function the API service's own
  // /render/svg-token routes use — lazily required so the bot process,
  // which should never load sharp at all, never does.
  if(isRunningAsApiService()){
    const { renderSvgTextToPng } = require('./svg-render');
    let svgText;
    if(svgSource.startsWith('data:image/svg')){
      const b64 = svgSource.split(',')[1];
      if(!b64) throw new Error('empty svg data');
    ...
    return await renderSvgTextToPng(svgText, size);
  }

  const RAILWAY_URL = getRailwayApiUrl();
  const API_SECRET = process.env.API_SECRET;
  if(!RAILWAY_URL) throw new Error('RAILWAY_API_URL not configured — cannot render SVG remotely');

  const isDataUri = svgSource.startsWith('data:image/svg');

  // Confirmed live: a large on-chain-generated SVG (base64 data URI, can
  // embed an entire PNG inline) sent as a GET query-string parameter blew
  // past whatever request-line length limit sits in front of this service,
  // failing with HTTP 431 (Request Header Fields Too Large). A URL is
  // always short regardless of what it points to, so svgUrl stays on GET;
  // only svgData (which embeds the actual, potentially large content) needs
  // to move to a POST body, which has a much more generous size limit.
  if(isDataUri){
    const qs = new URLSearchParams();
    if(API_SECRET) qs.set('key', API_SECRET);
    const r = await fetch(`${RAILWAY_URL}/render/svg-token?${qs}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ svgData: svgSource, size }),
    });
    if(!r.ok){
      const body = await r.text().catch(()=>'');
      throw new Error(`Remote SVG render failed: ${r.status} ${body}`);
    }
...
async function resolveImage(nft, contract, chain){
  ...
  // Confirmed live: isDiscordOk alone isn't reliable — Alchemy's own CDN can
  // serve genuine SVG content through a URL with zero textual indication of
  // that (no .svg extension, no "image/svg" substring). Verify the real
  // content-type before accepting a candidate; if it turns out to be SVG,
  // move on to the next candidate rather than returning a URL that will
  // never actually render in Discord.
  for(const url of candidates){
    if(isDiscordOk(url) && await verifyImageIsRaster(url)){
      const r={type:'url',url}; if(id) imageCache.set(key,r); return r;
    }
  }
  if(id){
    try{
...
try{ r=await fetch(`https://api.opensea.io/api/v2/chain/${chainForImg}/contract/${contract}/nfts/${id}`,{headers:osHeaders(),signal:_ctrl.signal}); }finally{ clearTimeout(_timer); }
      if(r.ok){
        ...
        for(const url of deep){
          if(isDiscordOk(url) && await verifyImageIsRaster(url)){
            const res={type:'url',url}; imageCache.set(key,res); return res;
          }
        }
        const svgSrc=deep.find(u=>u&&!u.startsWith('<svg')&&!u.startsWith('data:')&&isSvg(u))||candidates.find(u=>u&&isSvg(u))
          ||deep.find(u=>u&&isDiscordOk(u))||candidates.find(u=>u&&isDiscordOk(u)); // isDiscordOk-passing candidates that failed the raster check above are genuine SVG too, just without a URL-pattern hint — same fallback applies
...
function buildEmbedPayload(embed){
  ...
  const large=!!embed._imageLarge; delete embed._imageLarge;
  const components = embed._components || [];
  delete embed._components;
  // large=true is the per-server "artwork-focused" toggle — a full-width
  // image (setImage) instead of a small side thumbnail (setThumbnail),
  // paired with skipping the Traits field in the embed builders themselves.
  if(ir?.type==='buffer'){
    const att=new AttachmentBuilder(ir.buffer,{name:ir.filename});
    if(large) embed.setImage(`attachment://${ir.filename}`); else embed.setThumbnail(`attachment://${ir.filename}`);
    return {embeds:[embed],files:[att],components};
  }
  if(ir?.type==='url'){ if(large) embed.setImage(ir.url); else embed.setThumbnail(ir.url); }
...
async function fetchTokenMetaFromDb(tokenId, slug=OCAS_SLUG){
  ...
  try{
    ...
    const meta = {
      ...
      // Same fix as embeds.js's identical function -- realTraitCount() is
      // the raw, uncorrected count; realWornTraitCount(slug, ...) applies
      // the per-collection exclusion (Bones/Palette/Print/Fate for
      // Argonauts) so embed text matches the site.
      trait_count: realWornTraitCount(slug, bestTraits) || (j.token.trait_count ? parseInt(j.token.trait_count) : null),
      // Was silently dropped here even though /db/token/:id already returns
      // it straight from tokens.image_url -- meaning the backfill's own
      // fetched images were never actually used for the /token embed. The
      // embed instead only ever tried a LIVE OpenSea API call, which for a
      // collection on a chain OpenSea doesn't properly index (e.g. Robinhood
      // Chain) just silently returns nothing, showing "Image unavailable"
      // regardless of what real image data already sits in the DB.
      image_url: j.token.image_url || null,
    };
...
async function fetchTokenMetaFromLocalDb(tokenId){
  ...
  try{
    ...
    // /db/token/:id (api.js) and fetchTokenMetaFromDb (this file / embeds.js) —
...
async function fetchSnapshotImageForToken(tokenId){
  ...
  if(!id) return null;
  try{
    // token_image_snapshots is OCAS-only (burn history + pre-burn SVGs).
    // Scope to OCAS contract to prevent cross-collection ID collisions
    // (e.g. Fluxeto token #81 pulling OCAS token #81's snapshot).
    const snap = await pgPool.query(
...
module.exports = {
  ...
  snapshotTokenFromContract, tokenMetaCache,
};
