...
async function enrichCostBasis(wallet, slug, pgPool, openSeaKey, alchemyKey) {
      ...
      if((!priceEth || priceEth <= 0) && alchemyKey) {
        try {
          ...
          if(mintTx.rows.length && mintTx.rows[0].tx_hash) {
            ...
            const rpcRes = await fetch(`${ALCHEMY_BASE}/${alchemyKey}`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
...
async function backfillWallet(wallet, pgPool, alchemyKey, contract = OCAS_CONTRACT, slug = OCAS_SLUG) {
  ...
  await enrichCostBasis(wallet, slug, pgPool, process.env.OPENSEA_KEY, process.env.ALCHEMY_API_KEY).catch(e => {
