...
const ALCHEMY_BASE   = 'https://eth-mainnet.g.alchemy.com/v2'; // kept as the ethereum default; use alchemyBaseFor(chain) for anything chain-aware
const { SUPPORTED_CHAINS } = require('./collection-backfill');
const { getLinkedWalletAddresses } = require('./linked-wallets');
function alchemyBaseFor(chain){
  const subdomain = SUPPORTED_CHAINS[chain || 'ethereum'] || SUPPORTED_CHAINS.ethereum;
...
async function fetchTransfers(wallet, contract, alchemyKey, chain = 'ethereum') {
  const url = `${alchemyBaseFor(chain)}/${alchemyKey}`;
  const transfers = [];
  ...
  for (const direction of ['to', 'from']) {
    ...
    while (page < 20) {
      ...
      // jv confirmed live: syncing 2 wallets found tokens in the newly-
      // linked one but missed everything in the original, more-established
      // wallet. Root cause traced here -- an HTTP error (e.g. a rate
      // limit, far more likely now that a sync covers N wallets x M
      // collections x 2 directions of paginated calls, instead of just
      // one wallet's worth) used to silently `break` and return whatever
      // had been collected so far, indistinguishable from "this wallet
      // genuinely has no transfers." Combined with syncWalletForUser's
      // delete-then-rebuild pattern, a wallet with substantial existing
      // history (more pages to fetch, so more exposed to hitting this)
      // could have its correct data deleted and never actually replaced.
      // Retrying transient failures a few times with backoff before
      // giving up, and throwing (not silently returning partial results)
      // if a page still fails after retries, so the caller can tell "no
      // transfers" apart from "the fetch didn't actually complete."
      let res, lastErr;
      for (let attempt = 0; attempt < 3; attempt++) {
        try {
          ...
          break; // non-retryable HTTP error (e.g. 400) — no point retrying
        } catch (e) {
          lastErr = e;
          await new Promise(r => setTimeout(r, 500 * (attempt + 1)));
        }
      ...
      if (lastErr) {
        console.warn(`[WalletBackfill] Alchemy request failed for ${wallet} (${direction}) after retries:`, lastErr.message);
        throw new Error(`fetchTransfers failed for ${wallet}: ${lastErr.message}`);
      }
      ...
      if (!result) {
        ...
        throw new Error(`fetchTransfers got no result for ${wallet}: ${data?.error ? JSON.stringify(data.error) : 'empty response'}`);
      }
...
async function enrichCostBasis(wallet, slug, pgPool, openSeaKey, alchemyKey, chain = 'ethereum') {
  ...
  // Account-level lookup, done once upfront rather than per-token. The
  // existing per-token lookup below queries by COLLECTION+token, which
  // returns every trader's activity for that token, not just this
  // wallet's -- for an actively-traded collection, this wallet's own
  // specific purchase can be buried many pages deep in a token's full
  // history (confirmed live for Stackers: 50 sale events on a single
  // token within one day, this wallet's actual 4-day-old purchase never
  // appeared on page one). A wallet's OWN trading history is comparatively
  // small and directly targeted, so this avoids that problem entirely.
  // Writes matches straight into the sales cache table using the same
  // pattern the per-token lookup already uses below, so the existing
  // "check sales table first" step (unchanged) just finds them naturally.
  if(openSeaKey){
    try{
      let cursor = null;
      let pageCount = 0;
      const MAX_PAGES = 10; // sane bound -- even a very active trader's own history shouldn't need more than this
      let written = 0;
      do {
        const qs = new URLSearchParams({ event_type: 'sale', ...(cursor ? { next: cursor } : {}) }).toString();
        const res = await fetch(`https://api.opensea.io/api/v2/events/accounts/${wallet}?${qs}`, {
          headers: { 'X-API-KEY': openSeaKey, 'Accept': 'application/json' }
        });
        if(!res.ok){
          console.warn(`[CostBasis] account-events HTTP ${res.status} for ${wallet.slice(0,8)}`);
          break;
        }
        const data = await res.json();
        const events = data?.asset_events || [];
        for(const ev of events){
          if((ev?.buyer || '').toLowerCase() !== wallet) continue;
          if(ev?.nft?.collection !== slug) continue;
          const tid = parseInt(ev?.nft?.identifier);
          if(!tid || !tokens.includes(tid)) continue; // only care about tokens actually needing enrichment
          ...
          await pgPool.query(
            ...
            [tid, price, ev?.payment?.symbol || 'ETH', wallet, seller, saleTs, txHash, slug]
          ).catch(()=>{});
          written++;
        }
      ...
      if(written) console.log(`[CostBasis] account-events: found ${written} matching purchase(s) for ${wallet.slice(0,8)} in ${slug}`);
    }catch(e){
      console.warn(`[CostBasis] account-events lookup failed for ${wallet.slice(0,8)}: ${e.message}`);
    }
  }
  ...
  for(const tokenId of tokens) {
    try {
      ...
      if(cached.rows.length) {
        ...
        await new Promise(r => setTimeout(r, 300)); // rate limit buffer -- 200 empirically hit 429s against this same OpenSea key
        ...
        // Retry on 429 with exponential backoff (same shape as
        // backfill-os-rank.js's fetchOsRank) instead of giving up on this
        // token immediately -- previously a single rate-limit response
        // permanently skipped BOTH the OpenSea lookup AND the mint-tx
        // fallback below (via `continue`), even though the fallback might
        // have succeeded. Non-429 failures now fall through to the mint-tx
        // fallback instead of skipping the token outright.
        let res = null;
        for(let attempt = 0; attempt <= 4; attempt++) {
          ...
          console.warn(`[CostBasis] 429 on token ${tokenId}, waiting ${wait / 1000}s (attempt ${attempt + 1}/5)...`);
          await new Promise(r => setTimeout(r, wait));
        }

        if(!res.ok) {
          console.warn(`[CostBasis] OpenSea ${res.status} for token ${tokenId} — falling back to mint-tx lookup`);
          res = null;
        }
        ...
        // jv: "still not correct" -- #1477 (a token with a real transfer
        // log of one mint + one transfer, zero genuine sales) kept
        // showing dozens of sales, each one's real on-chain receipt
        // pointing to a completely different, unrelated token every
        // time. Root cause: OpenSea's token_ids filter above apparently
        // isn't honored on this collection-wide events endpoint --
        // events here can be for ANY token in the collection, not just
        // tokenId -- but every insert below was written under tokenId
        // regardless, the outer loop's own variable, never the specific
        // event's own token. That silently mis-tagged whatever random
        // sale happened to come back in this response as if it belonged
        // to the token currently being cost-basis-enriched. Reading each
        // event's own id (same field/fallback pattern used correctly
        // everywhere else sale events get parsed in this codebase) and
        // skipping anything that doesn't actually match tokenId fixes
        // both the corruption and the comment's own stated intent
        // ("cache all sale events for this token") -- it now actually
        // only caches rows that are for this token.
        for(const ev of events) {
          const rawId = ev?.nft?.identifier ?? ev?.asset?.token_id;
          const evTokenId = parseInt(rawId, 10);
          if(!Number.isFinite(evTokenId) || evTokenId !== tokenId) continue;
          ...
          await pgPool.query(
            `INSERT INTO sales (token_id, price_eth, currency, buyer, seller, sale_ts, tx_hash, collection_slug)
             VALUES ($1,$2,$3,$4,$5,$6,$7,$8)
             ON CONFLICT DO NOTHING`,
            [evTokenId, price, ev?.payment?.symbol || 'ETH', buyer, seller, saleTs, txHash, slug]
          ).catch(()=>{});
      ...
      // 3. Mint price fallback — look up tx value via eth_getTransactionByHash
      // Used when OpenSea has no sale record (e.g. SeaDrop mints where payment is internal)
      if((!priceEth || priceEth <= 0) && alchemyKey) {
        try {
          // Find the mint transfer (from zero address to this wallet) for this token
          const mintTx = await pgPool.query(
            `SELECT tx_hash FROM nft_transfers
             WHERE token_id=$1
          ...
          if(mintTx.rows.length && mintTx.rows[0].tx_hash) {
            const txHash = mintTx.rows[0].tx_hash;

            // Fetch tx to get total ETH value sent
            const rpcRes = await fetch(`${alchemyBaseFor(chain)}/${alchemyKey}`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
            ...
            const txValue = rpcData?.result?.value;

            if(txValue && txValue !== '0x0') {
              const totalEth = parseInt(txValue, 16) / 1e18;

              // Count how many tokens were minted in the same tx (batch mint)
              // so we can split the cost evenly
              const batchCount = await pgPool.query(
              ...
              const mintCount = parseInt(batchCount.rows[0]?.cnt || 1);
              priceEth = totalEth / mintCount;
              console.log(`[CostBasis] token ${tokenId} (${slug}): Ξ${priceEth.toFixed(6)} from mint tx (${mintCount} in batch)`);
            }
          }
        } catch(e) {
          console.warn(`[CostBasis] mint tx lookup failed for token ${tokenId}: ${e.message}`);
        }
      }

      // 4. Update cost_eth in wallet_token_intervals
...
async function deriveIntervals(wallet, slug, pgPool) {
  ...
  // Only applies to OCAS — burn_events has no data for other collections
  let trulyBurnedIds = new Set();
  let burnedAtByToken = new Map(); // tokenId -> burned_at, ground truth from burn_event_inputs/burn_events
  if(slug === 'on-chain-all-stars'){
    try {
      const burnRes = await pgPool.query(
        `SELECT bei.burned_token_id, be.burned_at
      ...
      burnedAtByToken = new Map(burnRes.rows.map(r => [parseInt(r.burned_token_id), r.burned_at]));
  ...
  // Reconciliation pass: burn_event_inputs/burn_events is ground truth for
  // burns. A token can still be sitting in `open` here even though it's a
  // confirmed burn, for two reasons seen in production: (1) its burn-leg
  // nft_transfers row is missing entirely, so the state machine above never
  // saw a burn event for it, or (2) the burn row exists but sorted BEFORE
  // its own receive row (NULLS LAST puts a NULL-timestamp receive after a
  // real-timestamped burn), so the burn silently no-op'd against an empty
  // `open` and the later receive left it open with nothing to ever close it.
  // Rather than depend on nft_transfers row completeness/ordering for
  // something we already know authoritatively, force-close any trulyBurnedIds
  // token still open here using the burn event's own timestamp.
  for (const tokenId of trulyBurnedIds) {
    if (open.has(tokenId)) {
      const entry = open.get(tokenId);
      open.delete(tokenId);
      intervals.push({ ...entry, token_id: tokenId, disposed_at: burnedAtByToken.get(tokenId) || entry.acquired_at, sale_eth: null, is_burn: true });
    }
  }
...
async function backfillWallet(wallet, pgPool, alchemyKey, contract = OCAS_CONTRACT, slug = OCAS_SLUG, chain = 'ethereum', force = false) {
  if (!alchemyKey) { console.warn('[WalletBackfill] No ALCHEMY_API_KEY'); return; }
  wallet = wallet.toLowerCase();
  contract = contract.toLowerCase();
  console.log(`[WalletBackfill] backfillWallet called: wallet=${wallet.slice(0,8)} slug=${slug} force=${force}`);

  try { await ensureTables(pgPool); } catch(e) {
    console.warn(`[WalletBackfill] ensureTables failed (non-fatal): ${e.message}`);
  }

  // Skip if already backfilled for this collection -- unless force is set
  // (the manual/scheduled re-sync path), in which case this check is
  // bypassed entirely and a fresh fetch always runs.
  if (!force) {
  ...
  // jv confirmed live: syncing 2 wallets found tokens in the newly-linked
  // one but missed everything in the original, more-established wallet.
  // Root cause: the caller used to DELETE this wallet's existing
  // nft_transfers rows unconditionally before calling this function, so
  // that the (now-bypassed) already-backfilled check above wouldn't skip
  // it -- but if fetchTransfers then failed (far more exposed now that a
  // multi-wallet sync means many more paginated Alchemy calls than a
  // single-wallet one ever did), the correct, pre-existing data was
  // already gone with nothing to replace it. fetchTransfers now throws
  // on a real failure instead of silently returning partial/empty
  // results, and the delete (below, force-only) only ever happens AFTER
  // a successful fetch -- old data is never touched unless the new fetch
  // actually worked.
  let transfers;
  try {
    transfers = await fetchTransfers(wallet, contract, alchemyKey, chain);
  } catch(e) {
    console.error(`[WalletBackfill] fetchTransfers failed for ${wallet.slice(0,8)} (${slug}), leaving existing data untouched:`, e.message);
    throw e;
  }
  ...
  if (force) {
    await pgPool.query(
      ...
      [wallet, slug]
    ).catch(e => console.warn(`[WalletBackfill] force-delete failed for ${slug} (non-fatal, may leave stale rows): ${e.message}`));
  }

  if (!transfers.length) return { held: 0, sold: 0 };
  ...
  await enrichCostBasis(wallet, slug, pgPool, process.env.OPENSEA_KEY, process.env.ALCHEMY_API_KEY, chain).catch(e => {
...
async function backfillServerWallets(guildId, contract, slug, pgPool, alchemyKey, chain = 'ethereum') {
  if (!alchemyKey) { console.warn('[WalletBackfill] No ALCHEMY_API_KEY'); return; }
  ...
  try {
    ...
    for (const row of res.rows) {
      await backfillWallet(row.wallet, pgPool, alchemyKey, contract, slug, chain).catch(e => {
...
async function syncWalletForUser(discordId, guildId, pgPool, alchemyKey, getConfig) {
  ...
  try {
    // jv confirmed live: the manual Sync button only ever synced "the
    // latest one that I had synced" -- one wallet, not all of them. Root
    // cause: this read from user_registrations, which stays single-
    // wallet-per-(discord_id,guild_id) by design (its own upsert always
    // replaces the wallet column) -- linked_wallets is the actual multi-
    // wallet source of truth and this never queried it at all. Falls back
    // to the old user_registrations lookup only if linked_wallets somehow
    // has nothing for this user (shouldn't happen after the migration
    // that backfilled every existing registration into it, but kept as a
    // safety net rather than a hard failure).
    let wallets = await getLinkedWalletAddresses(pgPool, discordId, guildId).catch(() => []);
    if (!wallets.length) {
      ...
      if (reg.rows.length) {
        wallets = [reg.rows[0].wallet.toLowerCase()];
      } else {
        ...
        console.log(`[WalletSync] Using wallet from global lookup (guild=${globalReg.rows[0].guild_id})`);
        return syncWalletForUser(discordId, globalReg.rows[0].guild_id, pgPool, alchemyKey, getConfig);
      }
    }
    console.log(`[WalletSync] Syncing ${wallets.length} wallet(s): ${wallets.map(w=>w.slice(0,8)).join(', ')}`);
    ...
    // Resolve chain per collection from the collections registry — server
    // config (where uniqueCollections above comes from) never carries chain
    // info at all, same gap fixed in /download earlier. One query for all
    // slugs rather than a lookup per collection.
    const chainMap = {};
    try {
      const chainRes = await pgPool.query(
        `SELECT slug, chain FROM collections WHERE slug = ANY($1)`,
        [uniqueCollections.map(c => c.slug)]
      );
      for (const row of chainRes.rows) chainMap[row.slug] = row.chain;
    } catch (e) {
      console.warn('[WalletSync] chain lookup failed (defaulting to ethereum):', e.message);
    }

    // Mark every (wallet, collection) pair as pending in sync status --
    // wrapped in try/catch so it never blocks backfill. wallet is already
    // part of this table's own unique key, so tracking multiple wallets
    // per discord_id needed no schema change here.
    try {
      await pgPool.query(`
      ...
      for (const w of wallets) {
        for (const col of uniqueCollections) {
          await pgPool.query(
            ...
            [discordId, w, col.slug]
          );
        }
      }
      console.log(`[WalletSync] Status table updated for ${wallets.length} wallet(s) x ${uniqueCollections.length} collection(s)`);
    } catch(e) {
    ...
    for (const w of wallets) {
      for (const col of uniqueCollections) {
        try {
          // jv confirmed live: the old upfront DELETE here (before
          // backfillWallet even ran) is exactly what turned a failed
          // fetch into permanent data loss -- moved that delete inside
          // backfillWallet itself now (force=true), which only runs it
          // after a successful fetch. If backfillWallet throws here (a
          // genuine fetch failure), this wallet+collection's existing
          // data is left completely untouched, and the status row below
          // correctly reflects the failure instead of silently reporting
          // "0 held" as if that were a real result.
          const result = await backfillWallet(w, pgPool, alchemyKey, col.contract, col.slug, chainMap[col.slug] || 'ethereum', true);
          const heldCount = result?.held || 0;
          console.log(`[SyncWallet] ${w.slice(0,8)}... ${col.slug}: ${heldCount} held after backfill`);

          await pgPool.query(
            `UPDATE wallet_sync_status SET status='done', completed_at=NOW(), token_count=$3
             WHERE discord_id=$1 AND wallet=$2 AND slug=$4`,
            [discordId, w, heldCount, col.slug]
          ).catch(()=>{});
          synced++;
        } catch(e) {
          await pgPool.query(
          ...
          console.warn(`[SyncWallet] Failed ${w.slice(0,8)}... ${col.slug}:`, e.message);
        }
      }
    }

    return { ok: true, wallets, wallet: wallets[0], synced, collections: uniqueCollections.map(c => c.slug) };
  } catch (e) {
...
async function getSyncStatus(discordId, pgPool) {
  try {
    ...
    // jv: multiple linked wallets means multiple rows per slug now (one
    // per wallet), where callers here still expect one row per slug --
    // aggregating rather than pushing that reshaping onto every caller.
    // token_count sums across wallets (the actual combined holding count
    // for that collection); status is 'error' if any wallet errored,
    // 'syncing' if any is still in progress, 'done' only once every
    // wallet's sync for that slug has finished.
    const bySlug = new Map();
    for (const row of res.rows) {
      const existing = bySlug.get(row.slug);
      if (!existing) {
        bySlug.set(row.slug, { ...row, token_count: row.token_count || 0 });
        continue;
      }
      existing.token_count += (row.token_count || 0);
      if (row.status === 'error' || existing.status === 'error') existing.status = 'error';
      else if (row.status === 'syncing' || existing.status === 'syncing') existing.status = 'syncing';
      else existing.status = 'done';
      if (row.started_at > existing.started_at) existing.started_at = row.started_at;
      if (!existing.completed_at || (row.completed_at && row.completed_at > existing.completed_at)) existing.completed_at = row.completed_at;
    }
    return [...bySlug.values()];
  } catch(e) { return []; }
}

module.exports = { backfillWallet, backfillServerWallets, syncWalletForUser, updateIntervalForTransfer, deriveIntervals, enrichCostBasis, getSyncStatus };
