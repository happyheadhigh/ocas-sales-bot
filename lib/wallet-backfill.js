...
async function enrichCostBasis(wallet, slug, pgPool, openSeaKey) {
  ...
  // Get inbound transfers that need cost basis (cost_eth = 0 in wallet_token_intervals)
  let tokens;
  try {
    const res = await pgPool.query(
      `SELECT DISTINCT wti.token_id
       FROM wallet_token_intervals wti
       WHERE wti.wallet_address=$1 AND wti.collection_slug=$2
       AND (wti.cost_eth IS NULL OR wti.cost_eth = 0)`,
      [wallet, slug]
    );
      ...
      if(priceEth && priceEth > 0) {
        await pgPool.query(
          `UPDATE wallet_token_intervals SET cost_eth=$1
           WHERE wallet_address=$2 AND token_id=$3 AND collection_slug=$4 AND disposed_at IS NULL`,
...
async function backfillWallet(wallet, pgPool, alchemyKey, contract = OCAS_CONTRACT, slug = OCAS_SLUG) {
  ...
  // Enrich cost basis: DB cache first, OpenSea fallback for historical purchases
