...
async function deriveIntervals(wallet, slug, pgPool) {
  ...
  // Known burn destinations — transfers to these are burns, not sales
  ...
  for (const row of res.rows) {
    ...
    const isBurn    = fromAddr === wallet && BURN_ADDRS.has(toAddr);
    ...
    if (isReceive) {
      ...
      intervals.push({ ...interval, token_id: tokenId, disposed_at: row.transferred_at, sale_eth: 0, is_burn: true });
      open.delete(tokenId);
    } else if (isSend && open.has(tokenId)) {
      const interval = open.get(tokenId);
      intervals.push({ ...interval, token_id: tokenId, disposed_at: row.transferred_at, sale_eth: parseFloat(row.value_eth || 0), is_burn: false });
      open.delete(tokenId);
    }
  }
