...
async function ensureTables(pgPool) {
  await pgPool.query(`
    CREATE TABLE IF NOT EXISTS nft_transfers (
      ...
      UNIQUE(tx_hash, token_id, collection_slug)
    )
  `).catch(()=>{});
  ...
  await pgPool.query(`
    CREATE TABLE IF NOT EXISTS wallet_token_intervals (
      ...
      collection_slug TEXT DEFAULT 'on-chain-all-stars',
      UNIQUE(wallet_address, token_id, acquired_at, collection_slug)
    )
  `).catch(()=>{});
...
async function backfillWallet(wallet, pgPool, alchemyKey, contract = OCAS_CONTRACT, slug = OCAS_SLUG) {
  ...
  if (parseInt(existing.rows[0]?.cnt || 0) > 0) {
    console.log(`[WalletBackfill] ${wallet} already backfilled for ${slug} — skipping`);
    return;
  }

  console.log(`[WalletBackfill] Backfilling ${wallet} for ${slug}...`);
  const transfers = await fetchTransfers(wallet, contract, alchemyKey);
  console.log(`[WalletBackfill] ${wallet}: ${transfers.length} transfers found`);
  ...
  for (const t of transfers) {
    ...
    await pgPool.query(
      `INSERT INTO nft_transfers (token_id, from_address, to_address, value_eth, block_number, transferred_at, tx_hash, collection_slug)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8)
       ON CONFLICT (tx_hash, token_id, collection_slug) DO NOTHING`,
      [t.tokenId, t.from, t.to, t.valueEth, t.blockNum, t.timestamp, t.hash, slug]
    ).catch(()=>{});
  }

  await deriveIntervals(wallet, slug, pgPool);
}
...
async function syncWalletForUser(discordId, guildId, pgPool, alchemyKey, getConfig) {
  if (!alchemyKey) return { error: 'No Alchemy key configured.' };
  ...
  try {
    ...
    if (!reg.rows.length) return { error: 'No verified wallet found.' };
    const wallet = reg.rows[0].wallet.toLowerCase();

    // Get all collections configured in this server
    const config = getConfig(guildId) || {};
    const collections = [];
    if (config.contract) collections.push({ contract: config.contract, slug: config.collectionSlug || config.slug || OCAS_SLUG });
    for (const c of config.collections || []) {
      if (c.contract && c.slug) collections.push({ contract: c.contract, slug: c.slug });
    }
    // Always include OCAS
    ...
    for (const col of collections) {
      // Force re-backfill by deleting existing transfers for this wallet+collection first
      await pgPool.query(
    ...
    return { ok: true, wallet, synced, collections: collections.map(c => c.slug) };
  } catch (e) {
...
module.exports = { backfillWallet, backfillServerWallets, syncWalletForUser, updateIntervalForTransfer, deriveIntervals };
