...
// ── Fetch real ETH cost basis for inbound transfers via eth_getTransactionByHash ─
// Called after writes to nft_transfers, updates value_eth where it was 0
async function enrichCostBasis(wallet, slug, pgPool, alchemyKey) {
  const url = `${ALCHEMY_BASE}/${alchemyKey}`;
  wallet = wallet.toLowerCase();

  // Get inbound transfers with zero value where we have a tx_hash
  let rows;
  try {
    const res = await pgPool.query(
      `SELECT id, tx_hash, token_id FROM nft_transfers
       WHERE to_address=$1 AND collection_slug=$2
       AND (value_eth IS NULL OR value_eth = 0)
  ...
  if(!rows.length) {
    console.log(`[CostBasis] No zero-value transfers to enrich for ${slug}`);
    return;
  }

  console.log(`[CostBasis] Fetching tx receipts for ${rows.length} transfers (${slug})`);
  let enriched = 0;

  for(const row of rows) {
    try {
      // Small delay to avoid hammering Alchemy
      ...
      if(eth > 0) {
        await pgPool.query(
          `UPDATE nft_transfers SET value_eth=$1 WHERE id=$2`,
          [eth, row.id]
        );
        enriched++;
      }
    } catch(e) {
      console.warn(`[CostBasis] tx fetch failed ${row.tx_hash}: ${e.message}`);
    }
  }

  console.log(`[CostBasis] Enriched ${enriched}/${rows.length} transfers for ${slug}`);
}
...
async function deriveIntervals(wallet, slug, pgPool) {
  ...
  for (const row of res.rows) {
    ...
    if (isReceive) {
      // value_eth is the purchase price (enriched via tx receipt, or 0 if not available)
...
async function backfillWallet(wallet, pgPool, alchemyKey, contract = OCAS_CONTRACT, slug = OCAS_SLUG) {
  ...
  // Enrich cost basis via tx receipts (~17 CU per tx, fire sequentially with delay)
  await enrichCostBasis(wallet, slug, pgPool, alchemyKey).catch(e => {
