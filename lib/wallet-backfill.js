...
async function syncWalletForUser(discordId, guildId, pgPool, alchemyKey, getConfig) {
  ...
  try {
    ...
    console.log(`[WalletSync] Collections to sync: ${collections.map(c => c.slug).join(', ')}`);
    ...
    try {
      ...
      console.log(`[WalletSync] Status table updated for ${collections.length} collection(s)`);
    } catch(e) {
