...
async function backfillWallet(wallet, pgPool, alchemyKey, contract = OCAS_CONTRACT, slug = OCAS_SLUG, chain = 'ethereum') {
  ...
  const transfers = await fetchTransfers(wallet, contract, alchemyKey, chain);
    ...
    for (const w of wallets) {
      for (const col of uniqueCollections) {
        try {
          ...
          const result = await backfillWallet(w, pgPool, alchemyKey, col.contract, col.slug, chainMap[col.slug] || 'ethereum');
