...
const BLOCK_CHUNK = Math.max(1, parseInt(process.env.METADATA_UPDATE_BLOCK_CHUNK || '10', 10)); // same confirmed 10-block Alchemy account cap as burn-poller
...
async function refreshSingleTokenMetadata({ contract, tokenId, chain, slug }){
  ...
  try{
    ...
    console.log(`[metadata-update] refreshed ${slug}#${id}: ${attrs.length} trait(s)${imgSrc ? ', image updated' : ''}`);
    return { ok: true, traitsUpdated: attrs.length, imageUpdated: !!imgSrc };
  }catch(e){
...
async function pollOneCollection(col){
  ...
  while(cursor <= latestBlock){
    const chunkTo = Math.min(cursor + BLOCK_CHUNK - 1, latestBlock);
    let logs;
    try{
...
function startMetadataUpdatePoller(){
  ...
  console.log(`[metadata-update] poller started (interval=${POLL_INTERVAL_MS}ms, block chunk=${BLOCK_CHUNK})`);
}
