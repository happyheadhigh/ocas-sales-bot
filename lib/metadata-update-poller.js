...
// Confirmed live: running the full-collection verification at concurrency=10
// against this specific Alchemy account produced widespread "Premature
// close"/"Invalid response body" errors -- classic symptoms of a provider
// dropping connections under load, not a code bug. This account has already
// shown several unusual restrictions tonight (the confirmed 10-block
// eth_getLogs cap on Robinhood Chain, the isSpam classification not
// refreshing its own cache) -- consistent with a lower tolerance for
// concurrent load than a typical account. Scoped narrowly to this file
// rather than touching the shared burnRpc helper itself, so other callers
// (the burn poller, the regular metadata-update cycle's own light usage)
// aren't affected by retry behavior they never needed.
async function fetchTokenUriWithRetry(contract, id, chain, retries = 3){
  for(let attempt = 0; attempt <= retries; attempt++){
    try{
      return await fetchTokenUri(contract, id, chain);
    }catch(e){
      const isTransient = /premature close|invalid response body|ECONNRESET|socket hang up/i.test(e.message || '');
...
async function refreshSingleTokenMetadata({ contract, tokenId, chain, slug }){
  ...
  try{
    const uri = await fetchTokenUriWithRetry(contract, id, chain);
    ...
    if(attrs.length){
      ...
      // jv: automatic metadata-change detection -> automatic rank update,
      // nothing manual. This poller existed and has been refreshing traits
      // since before tonight's rank work -- confirmed directly it never
      // once triggered a rank recompute of its own, so ranks were silently
      // stale relative to every trait change this already correctly
      // detected, the whole time it's been running. Debounced (shared
      // across every caller of this same function, including the
      // thousands-of-tokens full-collection verification below) so a large
      // batch of changes settles into one recompute, not one per token.
      scheduleRankRecompute(pgPool, slug, slug === OCAS_SLUG);
    }
    ...
    // their own animated art (set by the collection's creator), separate
    // from the static image this whole function otherwise handles --
    // this pipeline never captured animation_url at all before (it was
    // only ever pulled ad-hoc for a Discord /download command, never
    // synced into the main tokens table this site's own API reads from).
    // Stored as-is, whatever URL/data URI the metadata gives -- the
    // frontend decides how to render it (video vs. GIF vs. static
    // fallback) based on what it actually gets.
    const animUrl = meta.animation_url || meta.animated_url || null;
    if(animUrl){
      await pgPool.query(`UPDATE tokens SET animation_url=$1 WHERE id=$2 AND collection_slug=$3`, [String(animUrl), id, slug]).catch(()=>{});
    }
...
async function fullCollectionVerification({ slug, contract, chain, concurrency = 4 }){
