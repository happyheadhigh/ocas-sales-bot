...
async function runInBackground(pgPool, { contract, slug, guildId, guildName }){
  ...
  try{
    ...
    try{
      ...
      // jv asked "what happens now?" after seeing this exact webhook --
      // the old wording ("...or wait for the next auto-backfill attempt")
      // was simply wrong: maybeStartBackfill (this whole module's only
      // entry point) is only ever called from a /config or /setup command
      // handler, never from any scheduled job, so there is no "next
      // attempt" to wait for. Only a manual re-run actually retries this.
      sendErrorWebhook(`seedMarketHistory failed: "${slug}"`, e, 'Collection will not appear on TraitView until this succeeds. There is no automatic retry -- re-run /config for this collection in any server to try again.').catch(()=>{});
    }
