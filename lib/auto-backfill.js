...
async function maybeStartBackfill(pgPool, { contract, slug, guildId = null, guildName = null }){
  ...
  try{
    ...
    if(existing.rows.length){
      ...
      let seedAttempted = false, seedOk = false;
      try{
      ...
      // jv: re-submitted the same slug via /config's Edit Collection form
      // specifically to retry seedMarketHistory, then said "it didn't
      // trigger it" -- this path always returned needed:false regardless
      // of whether seedMarketHistory actually ran above, so the caller
      // (commands/config.js) had no way to know anything happened and
      // showed a bare "Updated." with no wait message either way, whether
      // the retry silently succeeded, silently failed, or never ran at
      // all because status was already 'ready'. Distinguishing all three
      // now so the caller can show what actually happened.
      if(seedAttempted){
        return { needed: false, reason: seedOk ? 'seed-retried-ok' : 'seed-retried-failed', seedAttempted: true, seedOk };
      }
...
async function runInBackground(pgPool, { contract, slug, guildId, guildName }){
  ...
  try{
    ...
    try{
      ...
      // jv: "there should be an automatic retry, server admins shouldn't
      // have to manually re run it if it fails." Previously fixed this
      // same message to stop claiming a retry that didn't exist -- now
      // one genuinely does (retryFailedMarketHistory, bot.js, backed by
      // market_retry_count/market_next_retry_at on the collections row
      // seedMarketHistory's own catch block already sets), so the
      // wording needs to describe what actually happens now, not repeat
      // the same "no retry" caveat that's no longer true.
      sendErrorWebhook(`seedMarketHistory failed: "${slug}"`, e, 'Collection will not appear on TraitView until this succeeds. Will retry automatically with backoff -- or re-run /config for this collection in any server to try again sooner.').catch(()=>{});
    }
