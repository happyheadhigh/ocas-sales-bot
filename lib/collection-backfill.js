...
async function fetchTokensByRange(alchemyKey, contract, startId, count){
  const nfts = [];
  for(let i = startId; i < startId + count; i++){
    try{
      const url = new URL(`https://eth-mainnet.g.alchemy.com/nft/v3/${alchemyKey}/getNFTMetadata`);
      url.searchParams.set('contractAddress', contract);
...
async function backfillCollectionTraits(pgPool, { contract, slug }){
  ...
  while(hasMore){
    ...
    if(paginationBroken){
      ...
      const batchNfts = await fetchTokensByRange(ALCHEMY_KEY, contract, rangeStart, PAGE_SIZE);
      if(!batchNfts.length){ hasMore = false; break; }
      page = { nfts: batchNfts, pageKey: null };
      rangeStart += PAGE_SIZE;
      if(batchNfts.length < PAGE_SIZE) hasMore = false;
    } else {
      page = await fetchPage(ALCHEMY_KEY, contract, pageKey);
      if(page.error){
        stats.failed++;
        consecutiveFails++;
    ...
    console.log(`[backfill] ${slug} fetched page ${stats.pages + 1}: ${page.nfts?.length} nfts, nextPageKey=${!!page.pageKey}${paginationBroken ? ' (range mode)' : ''}`);

    // Detect broken Alchemy pagination: full page returned but no continuation key
    if(!paginationBroken && !page.pageKey && page.nfts?.length === PAGE_SIZE){
    ...
    let result;
    try{
      result = await writePage(pgPool, slug, page.nfts, contract, ALCHEMY_KEY);
    }catch(pageErr){
      console.error(`[backfill] ${slug} page ${stats.pages + 1} error: ${pageErr.message}`);
      stats.failed++;
      if(!paginationBroken){ pageKey = page.pageKey; hasMore = !!pageKey; }
