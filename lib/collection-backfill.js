...
// jv confirmed live on Argonauts: two categories, Fate ("Burned") and Relic
// ("Gold"), get layered onto a token's metadata only after it's been
// burned -- both cap at exactly ONE possible value across the whole
// 9,999, unlike a real worn trait (Cloak/Crown/Sight/Artifact each have
// several), because ACK added them after the fact for the animated-burn
// art rather than as part of the original trait design. Bones/Palette/
// Print are the three always-present base attributes every Argonaut has
// regardless of what's worn (a "bare" Argonaut has only these three).
// None of these five are wearable traits, so none should count toward
// "traits worn" -- counting them was inflating trait_count for both a
// bare Argonaut (+3, the original 55-vs-62 investigation) and a burned
// one (+2 more on top of that). This mirrors traitview's own
// COLLECTIONS[slug].nonWornTraitCategories (js/config.js) -- same
// exclusion, same reasoning, kept in sync by hand across the two repos
// since they're separate codebases with no shared config source. Empty/
// absent for every other collection, so nothing changes for them.
const NON_WORN_TRAIT_CATEGORIES = {
  'argonauts': ['bones', 'palette', 'print', 'fate', 'relic'],
};
