...
async function processOneRepairJob(job, uriTemplate){
  ...
  try{
    ...
    const imageUrl = toDisplayableImageUrl(rawImage);
    const isSvgImage = imageUrl && (imageUrl.startsWith('<svg') || imageUrl.startsWith('data:image/svg') || imageUrl.includes('image/svg'));
    const discordImageUrl = isSvgImage ? null : imageUrl;
...
async function runMetadataRepairQueue(pgPool, slug, { limit = 100, uriTemplate = null } = {}){
  ...
  try{
    ...
    for(const r of results){
      if(r.success){
        await client.query(`DELETE FROM token_traits WHERE token_id=$1 AND collection_slug=$2`, [r.tokenId, slug]);
        for(let i = 0; i < r.attrs.length; i++){
          const t = r.attrs[i];
          await client.query(
            `INSERT INTO token_traits (token_id, trait_name, trait_value, trait_index, collection_slug) VALUES ($1,$2,$3,$4,$5)`,
            [r.tokenId, t.trait_type, t.value, i, slug]
          );
        }
        await client.query(
          `INSERT INTO tokens (id, collection_slug, trait_count, image_url) VALUES ($1,$2,$3,$4)
           ON CONFLICT (id, collection_slug) DO UPDATE SET trait_count=$3, image_url=COALESCE($4, tokens.image_url)`,
          [r.tokenId, slug, r.attrs.length, r.discordImageUrl]
        );
        if(r.isSvgImage && slug !== 'on-chain-all-stars'){
          await client.query(
            `INSERT INTO token_svg_cache (token_id, collection_slug, image_data) VALUES ($1,$2,$3)
             ON CONFLICT (token_id, collection_slug) DO UPDATE SET image_data=$3`,
            [r.tokenId, slug, r.imageUrl]
          ).catch(()=>{});
        }
        await client.query(`UPDATE metadata_repair_jobs SET status='done', attempts=attempts+1, last_attempt_at=NOW() WHERE id=$1`, [r.id]);
        resolved++;
      }else{
        const upd = await client.query(
          `UPDATE metadata_repair_jobs SET attempts=attempts+1, last_attempt_at=NOW(), last_error=$2 WHERE id=$1 RETURNING attempts`,
          [r.id, r.error]
        );
        const attempts = upd.rows[0]?.attempts || 1;
        if(attempts >= MAX_ATTEMPTS){
          await client.query(`UPDATE metadata_repair_jobs SET status='permanently_failed' WHERE id=$1`, [r.id]);
        }
        failed++;
      }
    }
