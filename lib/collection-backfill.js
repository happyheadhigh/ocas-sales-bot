...
async function resolveUri(uri, alchemyKey, alchemySubdomain, tokenId){
  ...
  try{
    ...
    // cloudflare-ipfs.com removed from the list entirely -- confirmed
    // decommissioned since August 2024, a guaranteed-fail entry on every
    // single request. w3s.link added in its place.
    if(uri.startsWith('ipfs://')){
      const cid = uri.slice(7);
      const candidates = [];
      if(alchemyKey) candidates.push(`https://${alchemySubdomain}.g.alchemy.com/nft/v3/${alchemyKey}/ipfsGateway/${cid}`);
      candidates.push(
      ...
      try{
        ...
        if(verbose){
          ...
          console.warn(`[backfill] token ${tokenId} all IPFS gateways failed for ${cid}: ${reasons}`);
        }
        return null;
      }
    }
