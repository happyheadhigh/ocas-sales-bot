...
// ── On-chain totalSupply() — the actual source of truth for a collection's
// real token count ─────────────────────────────────────────────────────────
// jv confirmed live on Argonauts: the supply-gap check's own "1-N" range was
// only ever as good as whatever number it was given, and that number
// (collections.total_supply) is sourced from OpenSea's cached collection
// metadata at onboarding time and never refreshed after -- confirmed stale
// for Argonauts specifically (8626 stored vs the real, current on-chain
// count). Rather than trust a third-party cache for a contract that's
// already shown unreliable metadata twice before (the isSpam:true stale
// Alchemy cache, and now this), read totalSupply() directly from the
// contract itself -- a single, fast eth_call, same RPC path already used
// for tokenURI() reads elsewhere in this file. This does NOT touch or
// replace the actual Tier 1 bulk backfill (still Alchemy's paginated
// getNFTsForContract, unchanged) -- it only corrects the expected-count
// number that drives the supply-gap check and the pagination safety
// ceiling. Returns null (never throws) if the contract doesn't implement
// totalSupply() (not every ERC-721 does) or the call otherwise fails, so a
// caller can safely fall back to the stored/cached value.
...
// -- traitsOf(tokenId), MAX_ID(), and traitChunks -- where trait and
// rarity data for every token is predetermined on-chain from the
// start... The fix: swap that one check to use MAX_ID() for Argonauts
// instead, since that's the equivalent value in its own architecture."
// Computed via ethers.id('MAX_ID()').slice(0,10) rather than guessed,
// same as TOTAL_SUPPLY_SELECTOR above was originally.
const MAX_ID_SELECTOR = '0x17bac052'; // keccak256("MAX_ID()")[:4]

async function fetchOnChainTotalSupply(contract, chain, alchemyKey){
  const alchemySubdomain = SUPPORTED_CHAINS[chain];
  if(!alchemySubdomain || !alchemyKey) return null;
  try{
    ...
    if(!result || result === '0x') throw new Error('eth_call returned empty result (contract may not implement totalSupply())');
    const supply = parseInt(result, 16);
    if(!Number.isFinite(supply) || supply <= 0) throw new Error(`eth_call result could not be decoded as a positive integer: ${result}`);
    return supply;
  }catch(e){
    console.warn(`[backfill] on-chain totalSupply() read failed for ${contract} on ${chain} (falling back to cached value): ${e.message}`);
    return null;
  }
}

// jv: "Swap that one check to use MAX_ID() for Argonauts instead,
// since that's the equivalent value in its own architecture...
// needs to happen collection-by-collection rather than assuming every
// contract looks the same." Same eth_call pattern as
// fetchOnChainTotalSupply() above, just a different selector and
// error message -- kept as its own function (not a shared helper with
// a selector parameter) so each one's warning text stays accurate
// about which specific function actually failed.
async function fetchOnChainMaxId(contract, chain, alchemyKey){
  ...
  try{
    ...
    const r = await fetch(rpcUrl, {
      ...
      body: JSON.stringify({
        jsonrpc: '2.0', id: 1, method: 'eth_call',
        params: [{ to: contract, data: MAX_ID_SELECTOR }, 'latest']
      })
    });
    if(!r.ok) throw new Error(`eth_call HTTP ${r.status}`);
    const j = await r.json();
    if(j.error) throw new Error(`eth_call RPC error: ${JSON.stringify(j.error)}`);
    const result = j.result;
    if(!result || result === '0x') throw new Error('eth_call returned empty result (contract may not implement MAX_ID())');
    const maxId = parseInt(result, 16);
    if(!Number.isFinite(maxId) || maxId <= 0) throw new Error(`eth_call result could not be decoded as a positive integer: ${result}`);
    return maxId;
  }catch(e){
    console.warn(`[backfill] on-chain MAX_ID() read failed for ${contract} on ${chain}: ${e.message}`);
    return null;
  }
}
...
async function fetchTokenUriMetadata(contract, tokenId, alchemyKey, alchemySubdomain, chain, opts = {}){
  if(!alchemyKey) return opts.includeFailureReasons ? { meta: null, reasons: ['no Alchemy key configured'] } : null;
  ...
  try{
    ...
    return opts.includeFailureReasons ? { meta: result, reasons: [] } : result;
  }catch(aggregateErr){
    const reasons = (aggregateErr.errors || []).map(e => e.message);
    if(tokenId <= 5){
      console.warn(`[backfill] token ${tokenId} both OpenSea and on-chain/IPFS paths failed: ${reasons.join(' | ')}`);
    }
    return opts.includeFailureReasons ? { meta: null, reasons } : null;
  }
}
...
// jv confirmed live on Argonauts: Bones/Palette/Print are the three
// always-present base attributes every Argonaut has regardless of what's
// worn (a "bare" Argonaut has only these three). Fate ("Burned") is a
// fourth, separate case -- unconditionally tied to burn status in the
// renderer's own verified source (only appended when isDead(tokenId) is
// true, nothing else ever sets it). None of these four are wearable
// traits, so none should count toward "traits worn".
//
// CORRECTION: Relic was wrongly included in this list for one round --
// jv found token #5207, which has Relic:Gold with NO Fate at all
// (confirmed not burned), disproving the original theory that Relic was
// a second burn-only marker. That theory came from only ever having
// checked two tokens, both of which happened to be burned and have both
// fields -- coincidence, not causation. Relic is a genuine (just rare)
// worn trait slot like Cloak/Crown/Sight/Artifact -- its "1 values" in
// the frontend's filter panel means every occurrence happens to be
// "Gold" so far, not that only one token has it. It counts normally now.
//
// This mirrors traitview's own COLLECTIONS[slug].nonWornTraitCategories
// (js/config.js) -- same exclusion, same reasoning, kept in sync by hand
// across the two repos since they're separate codebases with no shared
// config source. Empty/absent for every other collection, so nothing
// changes for them.
const NON_WORN_TRAIT_CATEGORIES = {
  'argonauts': ['bones', 'palette', 'print', 'fate'],
};

function countWornTraits(slug, attrs){
  const excluded = NON_WORN_TRAIT_CATEGORIES[slug];
  if(!excluded || !excluded.length) return attrs.length;
  const excludedSet = new Set(excluded);
  return attrs.filter(a => !excludedSet.has(String(a.trait_type || '').toLowerCase())).length;
}

async function fetchPage(alchemyKey, contract, pageKey, alchemySubdomain, retries = 0, forceRefresh = false){
  ...
  // Confirmed live: a manual re-backfill (meant specifically to pick up
  // metadata changes a project pushed after initial reveal/onboarding) still
  // showed the old, pre-update traits even 15+ minutes later -- well past
  // any of our own in-memory caching. Root cause: refreshCache=false tells
  // ALCHEMY not to refresh its own cache either, so a re-backfill was just
  // re-writing whatever stale metadata Alchemy had cached from the
  // collection's very first backfill, regardless of what actually changed
  // on-chain/off-chain since. false remains the right default for a
  // brand-new collection's first backfill (nothing stale to refresh yet,
  // and this keeps the speed/cost benefit tokenUriTimeoutInMs=0 was built
  // for) -- forceRefresh lets a manual re-backfill specifically ask Alchemy
  // to actually re-check the source instead of trusting its cache.
  url.searchParams.set('refreshCache', forceRefresh ? 'true' : 'false');
  ...
  try{
    ...
    if(r.status === 429){
      ...
      await sleep(wait);
      return fetchPage(alchemyKey, contract, pageKey, alchemySubdomain, Math.min(retries + 1, 5), forceRefresh);
    }
    if(!r.ok){
      if(retries < 3){ await sleep(3000 * (retries + 1)); return fetchPage(alchemyKey, contract, pageKey, alchemySubdomain, retries + 1, forceRefresh); }
...
async function fetchTokenRangeBulk(alchemyKey, contract, startId, count, alchemySubdomain, forceRefresh = false){
  try{
    const url = new URL(`https://${alchemySubdomain}.g.alchemy.com/nft/v3/${alchemyKey}/getNFTsForContract`);
    url.searchParams.set('contractAddress', contract);
    url.searchParams.set('withMetadata', 'true');
    ...
    url.searchParams.set('tokenUriTimeoutInMs', '0');
    // See fetchPage's comment for why this needs to be forceRefresh-aware —
    // same fix applied here since this is the parallel-range-fetch path
    // used when normal pagination is broken for a contract, not just the
...
async function fetchTokenRangesBulk(alchemyKey, contract, startIds, count, alchemySubdomain, forceRefresh = false){
  const results = await mapWithConcurrency(startIds, 4, (startId) =>
    fetchTokenRangeBulk(alchemyKey, contract, startId, count, alchemySubdomain, forceRefresh)
  );
  return results.flat();
}
...
async function writePage(pgPool, slug, nfts, contract, alchemyKey, alchemySubdomain, chain){
  ...
  const extracted = nfts.map(nft => {
    ...
    // Nekoadz? It is animated but the download is a png." This whole
    // pipeline only ever detected "is this collection animated" as a
    // collection-level stats flag (detectAnimated(), further down in
    // this file) -- never actually stored the per-token animation
    // asset anywhere retrievable. The only place animation_url got
    // captured at all was refreshSingleTokenMetadata() (metadata-
    // update-poller.js), which only runs for collections with EIP-4906
    // support -- Nekoadz's tokens went through this main backfill path
    // instead and never got it captured on the way in. animation_url
    // is the standard, explicit ERC-721 metadata field for this (raw
    // metadata, not display_image_url -- OpenSea's own derived "what to
    // show" field, which is a decent fallback but not always the same
    // asset).
    const animationUrl = nft.raw?.metadata?.animation_url || nft.raw?.metadata?.animated_url || null;

    return { tokenId, attrs, imageUrl, isSvgImage, discordImageUrl, animationUrl };
  ...
  // jv: agreed a token with zero real traits shouldn't be written as if
  // that were its actual state -- every Argonaut (and every collection
  // like it) always has SOME base attributes, so a written trait_count of
  // 0 is never actually true, just a placeholder for "metadata didn't come
  // back yet." Only that specific case gets held back from the write below
  // (queued for repair, not represented in `tokens`/`token_traits` at all
  // until real data exists): a needsRepair item can also land here purely
  // because its IMAGE was missing/suspicious while its trait data was
  // completely fine (attrs.length > 0) -- that trait data is real and
  // should still be written normally, only the image needs the repair
  // pass. Held-back tokens simply don't appear in the site yet, which is
  // the honest state, rather than appearing with a real-looking (but
  // impossible) 0.
  const writable = allToWrite.filter(i => i.attrs.length > 0);
  const client = await pgPool.connect();
  try{
    await client.query('BEGIN');

    if(writable.length){
      const ids = writable.map(i => i.tokenId);
      await client.query(`DELETE FROM token_traits WHERE collection_slug=$1 AND token_id = ANY($2::int[])`, [slug, ids]);
    }

    const tokenRows = writable.map(i => [i.tokenId, slug, countWornTraits(slug, i.attrs), i.discordImageUrl, i.animationUrl]);
    await bulkInsert(
      client, 'tokens', ['id', 'collection_slug', 'trait_count', 'image_url', 'animation_url'], tokenRows,
      `ON CONFLICT (id, collection_slug) DO UPDATE SET trait_count=EXCLUDED.trait_count, image_url=COALESCE(EXCLUDED.image_url, tokens.image_url), animation_url=COALESCE(EXCLUDED.animation_url, tokens.animation_url)`
    );
    ...
    const svgRows = writable.filter(i => i.isSvgImage && slug !== 'on-chain-all-stars').map(i => [i.tokenId, slug, i.imageUrl]);
    if(svgRows.length){
      await bulkInsert(
  ...
  if(needsRepair.length){
    try{
      ...
      try{
        ...
        // jv: "I thought the backfill should have a fix for any tokens that
        // fail. It should try to backfill those tokens separately again."
        // Confirmed the gap: this was `DO NOTHING` on conflict, so once a
        // token's job hit MAX_ATTEMPTS in runMetadataRepairQueue and got
        // marked 'permanently_failed', the unique (collection_slug,
        // token_id) row already existing meant EVERY future backfill --
        // including a manual re-backfill run specifically to pick up
        // stragglers -- silently skipped re-queueing it forever. A token
        // showing up here again means it failed at least once before, so a
        // fresh backfill pass is exactly the signal to give it another
        // real shot: reset it back to pending with a clean attempts count
        // (rather than resuming from wherever the old attempt counter left
        // off, which could trip MAX_ATTEMPTS again almost immediately).
        // Genuinely still-pending or already-done rows are left untouched.
        //
        // One real exception, confirmed live on Argonauts token #0: some
        // "tokens" are genuinely not collectibles at all -- #0 resolves to
        // real, valid on-chain JSON ({"name":"Argonauts: The Site",...})
        // with no traits and no image by design (site/contract metadata
        // riding on the same tokenURI mechanism, not a real Argonaut --
        // the collection's own numbering is 1-9999). That's a *resolved*
        // permanent answer, not a fetch failure -- processOneRepairJob's
        // exact wording for that specific case ('...neither usable traits
        // nor an image') is what distinguishes it from every other genuine
        // failure (timeout, gateway down, bad tokenURI, etc.), which SHOULD
        // keep getting retried since conditions can change. Re-queuing a
        // token like #0 every backfill would just burn 5 attempts and fire
        // a bogus "permanently failed" alert forever for something that
        // will never resolve differently, since the on-chain data itself
        // never changes.
        //
        // Second exclusion added after jv confirmed live on Argonauts #21
        // (direct Etherscan tokenURI() call: "execution reverted"): a token
        // that's provably not minted yet gets its own distinguishable
        // last_error now ('token does not appear to be minted yet...', see
        // processOneRepairJob) -- same reasoning as #0, retrying it every
        // backfill can't produce a different result until it's actually
        // claimed, so it stays permanently_failed rather than getting reset
        // and re-alerting every single run.
        await bulkInsert(
          repairClient, 'metadata_repair_jobs', ['collection_slug', 'token_id', 'contract', 'chain'], rows,
          `ON CONFLICT (collection_slug, token_id) DO UPDATE SET
             status = 'pending', attempts = 0, last_error = NULL
           WHERE metadata_repair_jobs.status = 'permanently_failed'
             AND metadata_repair_jobs.last_error NOT LIKE '%neither usable traits nor an image%'
             AND metadata_repair_jobs.last_error NOT LIKE '%does not appear to be minted yet%'`
        );
      }finally{
...
async function processOneRepairJob(job, uriTemplate){
  ...
  try{
    ...
    if(!onChainMeta){
      const r = await fetchTokenUriMetadata(contract, tokenId, ALCHEMY_KEY, alchemySubdomain, chain, { includeFailureReasons: true });
      onChainMeta = r.meta;
      failureReasons = r.reasons;
    }
    if(!onChainMeta){
      // jv confirmed live on Argonauts #21 (direct Etherscan tokenURI()
      // call): "execution reverted" -- the standard ERC-721 signature for
      // "this token ID hasn't been minted yet," not a fetch problem. Before
      // this, every failure here got the exact same flat "all metadata
      // sources exhausted" message regardless of WHY, even though
      // fetchTokenUriMetadata's own AggregateError already carries the real
      // per-source reasons -- they were just being thrown away. Detecting
      // this specific signature (OpenSea 404 AND the on-chain read either
      // reverting or coming back completely empty) lets last_error say
      // something the requeue-on-conflict logic can actually recognize and
      // stop retrying, the same way it already does for #0's "resolved but
      // empty" case -- retrying a token that provably doesn't exist yet on
      // every future backfill is pointless noise, not a bug to keep fixing.
      const blob = failureReasons.join(' | ');
      const looksNonexistent = /opensea http 404/i.test(blob) && /(returned empty result|eth_call rpc error.*revert)/i.test(blob);
      if(looksNonexistent){
        throw new Error(`token does not appear to be minted yet (OpenSea 404, on-chain read reverted/empty): ${blob}`);
      }
      throw new Error(`all metadata sources exhausted (OpenSea + on-chain tokenURI + every IPFS gateway)${blob ? ': ' + blob : ''}`);
    }
...
async function runMetadataRepairQueue(pgPool, slug, { limit = 100, uriTemplate = null } = {}){
  ...
  try{
    ...
    if(success.length){
      ...
      const tokenRows = success.map(r => [r.tokenId, slug, countWornTraits(slug, r.attrs), r.discordImageUrl]);
      await bulkInsert(
...
async function runOpenSeaBulkRepair(pgPool, slug, chain){
  ...
  try{
    ...
    const tokenRows = resolvedRows.map(r => [r.tokenId, slug, countWornTraits(slug, r.attrs), r.discordImageUrl]);
    await bulkInsert(
      client, 'tokens', ['id', 'collection_slug', 'trait_count', 'image_url'], tokenRows,
      `ON CONFLICT (id, collection_slug) DO UPDATE SET trait_count=EXCLUDED.trait_count, image_url=COALESCE(EXCLUDED.image_url, tokens.image_url)`
    );
...
async function backfillCollectionTraits(pgPool, { contract, slug, chain = 'ethereum', totalSupply = null, guildId = null, guildName = null, forceRefresh = false }){
  ...
  while(hasMore){
    ...
    if(rpcOnlyMode){
      ...
      const batchNfts = await fetchTokenRangesBulk(ALCHEMY_KEY, contract, starts, PAGE_SIZE, alchemySubdomain, forceRefresh);
      ...
      page = await fetchPage(ALCHEMY_KEY, contract, pageKey, alchemySubdomain, 0, forceRefresh);
      if(page.error){
        stats.failed++;
        consecutiveFails++;
  ...
  // ── Supply-gap check: catch tokens Alchemy's own index never returned
  // at all ────────────────────────────────────────────────────────────────
  // jv confirmed live on Argonauts: the official site (reading straight
  // from the chain) shows all 9999 tokens live right now, but this
  // backfill's normal Alchemy-pagination path (fetchPage/getNFTsForContract)
  // ran to completion at 9241 -- 93 full pages, then a naturally short
  // final page, no error, no broken-pagination signature. That's a
  // DIFFERENT failure mode than anything above already handles: it's not
  // Alchemy erroring (consecutiveFails -> rpcOnlyMode) or a detectably
  // broken cursor (paginationBroken), it's Alchemy's own index for this
  // contract genuinely not having every token -- plausibly tied to the
  // same isSpam:true classification already confirmed for this contract
  // elsewhere in this codebase (metadata-update-poller.js), which could
  // easily extend to incomplete indexing, not just stale metadata refresh.
  // rpcOnlyMode/paginationBroken already enumerate the full 1..totalSupply
  // ID range directly regardless of what Alchemy has indexed, so they can't
  // have this gap -- only worth checking after the normal path completes.
  if(totalSupply && !rpcOnlyMode && !paginationBroken){
    try{
      const missingRes = await pgPool.query(
        `SELECT gs AS id FROM generate_series(1, $2) gs
         WHERE NOT EXISTS (SELECT 1 FROM tokens WHERE collection_slug = $1 AND id = gs)`,
        [slug, totalSupply]
      );
      const missingIds = missingRes.rows.map(r => r.id);
      // A near-total miss (almost the whole declared supply) means
      // totalSupply itself is probably wrong, not that every single token
      // failed to index -- queueing 9000+ jobs on a bad assumption would
      // just hammer every metadata source for nothing. Only trust this as
      // a real, fixable gap when it's a real minority of the collection.
      if(missingIds.length && missingIds.length < totalSupply * 0.5){
        console.log(`[backfill] ${slug} supply-gap check: ${missingIds.length} token(s) 1-${totalSupply} that Alchemy's own index never returned at all -- queueing for the same repair pipeline as any other straggler`);
        const rows = missingIds.map(id => [slug, id, contract, chain || 'ethereum']);
        const gapClient = await pgPool.connect();
        try{
          await bulkInsert(
            gapClient, 'metadata_repair_jobs', ['collection_slug', 'token_id', 'contract', 'chain'], rows,
            `ON CONFLICT (collection_slug, token_id) DO UPDATE SET
               status = 'pending', attempts = 0, last_error = NULL
             WHERE metadata_repair_jobs.status = 'permanently_failed'
               AND metadata_repair_jobs.last_error NOT LIKE '%neither usable traits nor an image%'
        ...
        stats.queuedForRepair += missingIds.length;
      } else if(missingIds.length){
        console.warn(`[backfill] ${slug} supply-gap check found ${missingIds.length}/${totalSupply} missing -- that's too large a fraction to trust as a real per-token gap (likely totalSupply itself is stale/wrong), not queueing`);
      }
    }catch(e){
      console.warn(`[backfill] ${slug} supply-gap check failed (non-fatal):`, e.message);
    }
  }
...
module.exports = {
  backfillCollectionTraits, SUPPORTED_CHAINS, fetchTokenUriMetadata, fixCollectionImages, mapWithConcurrency,
  ...
  fetchOpenSeaMetadata, extractTraits, fetchOnChainTotalSupply, fetchOnChainMaxId,
  toDisplayableImageUrl, needsSvgRender, bulkInsert, countWornTraits,
  // jv: "new tokens are still being claimed... is there something
  // capturing those events". Exported for lib/new-token-poller.js --
  // the same bulk, concurrent ID-range fetcher and DB-write primitive
  // backfillCollectionTraits() itself uses internally, reused there for
  // a lean incremental backfill scoped to just newly-minted token IDs
  // instead of touching this whole file's much larger, more delicate
  // state machine.
  fetchTokenRangesBulk, writePage,
};
