...
async function resolveUri(uri, alchemyKey, alchemySubdomain, tokenId){
  ...
  try{
    ...
    // IPFS — try Alchemy's gateway first (fast, no public-gateway rate limits),
    // falling back through a list of public gateways. Previously this never
    // fell back at all, and any failure was swallowed identically into a
    // silent `null` with no way to tell which of those actually happened.
    ...
    // simply doesn't support that URI shape. The single ipfs.io fallback
    // added earlier wasn't enough either: confirmed live it can time out on
    // cold/unpinned content even at 10s, and it's one of the more heavily
    // loaded public gateways. Now tries multiple public gateways in order
    // with a longer timeout, stopping at the first success.
    if(uri.startsWith('ipfs://')){
      const cid = uri.slice(7);
      if(alchemyKey){
        const alchemyGateway = `https://${alchemySubdomain}.g.alchemy.com/nft/v3/${alchemyKey}/ipfsGateway/${cid}`;
        try{
          const r1 = await fetch(alchemyGateway, { timeout: 10000 });
          if(r1.ok) return await r1.json();
          if(verbose) console.warn(`[backfill] token ${tokenId} Alchemy ipfsGateway HTTP ${r1.status} for ${cid} — trying public gateways`);
        }catch(e){
          if(verbose) console.warn(`[backfill] token ${tokenId} Alchemy ipfsGateway fetch failed for ${cid}: ${e.message} — trying public gateways`);
        }
      }
      ...
      for(const base of PUBLIC_GATEWAYS){
        try{
          const r = await fetch(`${base}${cid}`, { timeout: 20000 });
          if(r.ok) return await r.json();
          if(verbose) console.warn(`[backfill] token ${tokenId} gateway ${base} HTTP ${r.status} for ${cid}`);
        }catch(e){
          if(verbose) console.warn(`[backfill] token ${tokenId} gateway ${base} failed for ${cid}: ${e.message}`);
        }
      }
      if(verbose) console.warn(`[backfill] token ${tokenId} all IPFS gateways failed for ${cid}`);
      return null;
    }
...
async function writePage(pgPool, slug, nfts, contract, alchemyKey, alchemySubdomain){
  ...
  const enriched = await mapWithConcurrency(nfts, 8, async (nft) => {
    ...
    if(!attrs.length && contract){
      ...
      onChainMeta = await fetchTokenUriMetadata(contract, tokenId, alchemyKey, alchemySubdomain);
      if(onChainMeta){
        ...
        if(!imageUrl){
          imageUrl = onChainMeta.image || onChainMeta.image_url || onChainMeta.image_data || null;
    ...
    if(isNonEthereum && contract){
      if(!onChainMeta) onChainMeta = await fetchTokenUriMetadata(contract, tokenId, alchemyKey, alchemySubdomain);
      const verifiedImage = onChainMeta?.image || onChainMeta?.image_url || onChainMeta?.image_data || null;
      if(verifiedImage){
        imageUrl = verifiedImage.startsWith('ipfs://') ? `https://ipfs.io/ipfs/${verifiedImage.slice(7)}` : verifiedImage;
      }
    }
...
async function fixCollectionImages(pgPool, { slug, contract, chain }){
  ...
  for(const { id: tokenId } of rows){
    try{
      const onChainMeta = await fetchTokenUriMetadata(contract, tokenId, alchemyKey, alchemySubdomain);
      const verifiedImage = onChainMeta?.image || onChainMeta?.image_url || onChainMeta?.image_data || null;
      if(!verifiedImage){ unchanged++; continue; }
      const resolvedImage = verifiedImage.startsWith('ipfs://') ? `https://ipfs.io/ipfs/${verifiedImage.slice(7)}` : verifiedImage;
...
module.exports = { backfillCollectionTraits, SUPPORTED_CHAINS, fetchTokenUriMetadata, fixCollectionImages };
