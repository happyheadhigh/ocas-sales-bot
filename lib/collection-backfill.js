...
async function writePage(pgPool, slug, nfts){
  ...
  try{
    ...
    for(const nft of nfts){
      ...
      // Extract image URL — use cachedUrl (CDN) first, fall back to originalUrl
      const imageUrl = nft.image?.cachedUrl || nft.image?.originalUrl || nft.image?.thumbnailUrl || null;
