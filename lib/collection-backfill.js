  ...
  try{
    ...
    for(const nft of nfts){
        ...
        if(imageUrl){
          await client.query(
            `INSERT INTO tokens (id, collection_slug, trait_count, image_url)
             VALUES ($1,$2,0,$3)
             ON CONFLICT (id) DO UPDATE SET collection_slug=COALESCE($2, tokens.collection_slug), image_url=COALESCE($3, tokens.image_url)`,
      ...
      await client.query(
        `INSERT INTO tokens (id, collection_slug, trait_count, image_url)
         VALUES ($1,$2,$3,$4)
         ON CONFLICT (id) DO UPDATE SET collection_slug=COALESCE($2, tokens.collection_slug), trait_count=$3, image_url=COALESCE($4, tokens.image_url)`,
...
async function backfillCollectionTraits(pgPool, { contract, slug }){
  ...
  const stats = { written: 0, skipped: 0, pages: 0, failed: 0 };
  let pageKey = null;
