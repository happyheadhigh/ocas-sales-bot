...
async function resolveUri(uri, alchemyKey, alchemySubdomain, tokenId){
  ...
  try{
    ...
    // falling back to the public ipfs.io gateway. Previously this never fell
    // back at all, and any failure (bad HTTP status, network error, JSON
    // parse failure) was swallowed identically into a silent `null` with no
    // way to tell which of those actually happened — confirmed on
    // robinhood-chimps, where Alchemy's ipfsGateway proxy was returning
    // non-JSON/failing for CIDs with a folder-style subpath appended
    // (ipfs://<cid>/<tokenId>, as opposed to a bare per-token CID), silently
    // losing every token's traits+image even though the same content is
    ...
    if(uri.startsWith('ipfs://')){
      ...
      if(alchemyKey){
        ...
        try{
          ...
          if(verbose) console.warn(`[backfill] token ${tokenId} Alchemy ipfsGateway HTTP ${r1.status} for ${cid} — falling back to public ipfs.io gateway`);
        }catch(e){
          if(verbose) console.warn(`[backfill] token ${tokenId} Alchemy ipfsGateway fetch failed for ${cid}: ${e.message} — falling back to public ipfs.io gateway`);
        }
      }
      const publicGateway = `https://ipfs.io/ipfs/${cid}`;
      const r2 = await fetch(publicGateway, { timeout: 10000 });
      if(!r2.ok){
        if(verbose) console.warn(`[backfill] token ${tokenId} public ipfs.io gateway also failed: HTTP ${r2.status} for ${cid}`);
        return null;
      }
