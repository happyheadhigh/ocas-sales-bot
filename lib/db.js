...
async function runMigrations(){
  ...
  // tap per person if doable". No login system exists on the site at
  // all, so "per person" means a persistent anonymous client id the
  // frontend generates once and stores in localStorage (see
  ...
  // lightweight, no-account reaction feature. The UNIQUE constraint
  // below is what actually enforces "one reaction per person per
  // collection": a client_id can have at most one row per slug, upserted
  // (changing their pick) rather than accumulating extra rows.
