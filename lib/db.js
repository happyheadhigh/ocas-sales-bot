...
async function runMigrations(){
  ...
  // Ensure collection_slug exists on wallet_token_intervals (may be missing on existing DBs)
  ...
  await q(`ALTER TABLE wallet_token_intervals ADD COLUMN IF NOT EXISTS sale_eth NUMERIC`).catch(()=>{});
  await q(`ALTER TABLE wallet_token_intervals ADD COLUMN IF NOT EXISTS cost_eth NUMERIC DEFAULT 0`).catch(()=>{});
