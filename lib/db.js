'use strict';

const { Pool } = require('pg');
const fs       = require('fs');
const {
  SERVER_FILE, ALERTS_FILE,
  ERROR_WEBHOOK_URL, BOT_ENV,
} = require('./constants');

// ── PostgreSQL pool ───────────────────────────────────────────────────────────
const pgPool = new Pool({
  connectionString: process.env.DATABASE_URL,
  ssl: process.env.DATABASE_URL?.includes('railway.internal')
    ? false
    : { rejectUnauthorized: false },
  max: 8,                            // slightly more slots for wallet backfill
  idleTimeoutMillis: 60000,          // 60s idle timeout
  connectionTimeoutMillis: 10000,   // 10s — longer timeout for backfill operations
  // Per-context timeouts used instead (pgQueryCommand/pgQueryPoller)
  keepAlive: true,                  // TCP keepalive — prevents Railway proxy from silently dropping
  keepAliveInitialDelayMillis: 10000,
});

// Heartbeat query every 4 minutes to prevent Railway proxy from dropping idle connections
setInterval(async () => {
  try{ await pgPool.query('SELECT 1'); }
  catch(e){ console.warn('[DB Heartbeat] failed:', e.message); }
}, 4 * 60 * 1000);

pgPool.on('error', e => {
  console.error('[PG bot]', e.message);
  // sendErrorWebhook imported lazily to avoid circular dep
  try{ require('./error').sendErrorWebhook('DB Pool Error', e); }catch(_){}
});


// Discord command queries — must finish within 3s or interaction expires
async function pgQueryCommand(sql, params){
  return pgPool.query({ text: sql, values: params, query_timeout: 3000 });
}

// Burn poller queries — longer window, not Discord time-critical
async function pgQueryPoller(sql, params){
  return pgPool.query({ text: sql, values: params, query_timeout: 20000 });
}

async function runMigrations(){
  const q = sql => pgPool.query(sql).catch(() => {});
  await q(`CREATE TABLE IF NOT EXISTS user_registrations (
    discord_id   TEXT NOT NULL,
    guild_id     TEXT NOT NULL DEFAULT 'global',
    wallet       TEXT,
    verified     BOOLEAN DEFAULT false,
    verified_at  TIMESTAMPTZ,
    updated_at   TIMESTAMPTZ DEFAULT NOW(),
    PRIMARY KEY (discord_id, guild_id)
  )`);
  // Migrate old single-column PK to composite PK if needed
  await q(`ALTER TABLE user_registrations ADD COLUMN IF NOT EXISTS guild_id TEXT NOT NULL DEFAULT 'global'`).catch(()=>{});
  await q(`DO $$
    BEGIN
      IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints
        WHERE table_name='user_registrations' AND constraint_type='PRIMARY KEY'
          AND constraint_name='user_registrations_pkey'
      ) THEN
        ALTER TABLE user_registrations DROP CONSTRAINT IF EXISTS user_registrations_pkey;
        ALTER TABLE user_registrations ADD PRIMARY KEY (discord_id, guild_id);
      END IF;
    END $$`).catch(()=>{});
  // Only recreate PK if it doesn't already cover both columns
  await q(`DO $$
    BEGIN
      IF NOT EXISTS (
        SELECT 1 FROM pg_index i
        JOIN pg_attribute a ON a.attrelid = i.indrelid AND a.attnum = ANY(i.indkey)
        WHERE i.indrelid = 'user_registrations'::regclass AND i.indisprimary
          AND a.attname = 'guild_id'
      ) THEN
        ALTER TABLE user_registrations DROP CONSTRAINT IF EXISTS user_registrations_pkey;
        ALTER TABLE user_registrations ADD CONSTRAINT user_registrations_pkey PRIMARY KEY (discord_id, guild_id);
      END IF;
    END $$`).catch(()=>{});
  // Drop stale wallet unique constraint if it exists — wallet is not globally unique (same wallet can appear in multiple guild rows)
  await q(`ALTER TABLE user_registrations DROP CONSTRAINT IF EXISTS user_registrations_wallet_key`).catch(()=>{});

  // jv: "I have 2 separate wallets that I want connected. The bot should
  // support multiple wallets." user_registrations (above) is one-wallet-
  // per-(discord_id,guild_id) by its own primary key -- verifying a second
  // wallet there silently overwrites the first rather than adding to it.
  // linked_wallets is additive: wallet is part of the primary key instead
  // of being replaced by it, so one Discord user can have any number of
  // wallets linked per guild. user_registrations is NOT being dropped or
  // stopped -- kept updated in parallel as a fallback while this settles
  // in, exactly as planned with jv, precisely because this touches real
  // people's already-earned roles and a mistake here would be a real
  // incident, not a cosmetic bug.
  await q(`CREATE TABLE IF NOT EXISTS linked_wallets (
    discord_id   TEXT NOT NULL,
    guild_id     TEXT NOT NULL DEFAULT 'global',
    wallet       TEXT NOT NULL,
    verified     BOOLEAN DEFAULT false,
    verified_at  TIMESTAMPTZ,
    linked_at    TIMESTAMPTZ DEFAULT NOW(),
    PRIMARY KEY (discord_id, guild_id, wallet)
  )`);
  await q(`CREATE INDEX IF NOT EXISTS linked_wallets_wallet_idx ON linked_wallets(wallet)`);
  await q(`CREATE INDEX IF NOT EXISTS linked_wallets_discord_idx ON linked_wallets(discord_id, guild_id)`);
  // One-time (but safe to re-run every startup -- ON CONFLICT DO NOTHING
  // makes this a no-op once a row's already been copied over) backfill:
  // every existing verified wallet becomes that user's first linked
  // wallet, so no one loses their current verification when this ships.
  // Runs every startup rather than truly once because user_registrations
  // is still being written to in parallel during the transition -- this
  // keeps catching anyone who verifies through the old path too, not just
  // a single migration moment.
  await q(`INSERT INTO linked_wallets (discord_id, guild_id, wallet, verified, verified_at, linked_at)
    SELECT discord_id, guild_id, LOWER(wallet), verified, verified_at, COALESCE(verified_at, updated_at, NOW())
    FROM user_registrations
    WHERE wallet IS NOT NULL
    ON CONFLICT (discord_id, guild_id, wallet) DO NOTHING`).catch(()=>{});

  await q(`CREATE TABLE IF NOT EXISTS collection_traits (
    slug         TEXT NOT NULL,
    trait_name   TEXT NOT NULL,
    trait_value  TEXT NOT NULL,
    token_count  INTEGER DEFAULT 0,
    PRIMARY KEY (slug, trait_name, trait_value)
  )`);

  // Global registry of collections TraitView knows about, independent of any
  // Discord guild's server_configs. Previously collections only existed as
  // entries buried in per-guild JSON — this is the first standalone source
  // of truth, needed so TraitView can onboard a collection nobody's Discord
  // config references. status drives the on-demand onboarding flow:
  // pending -> backfilling_traits -> backfilling_market -> ready
  //                                                      -> failed (any stage)
  await q(`CREATE TABLE IF NOT EXISTS collections (
    slug             TEXT PRIMARY KEY,
    contract         TEXT NOT NULL,
    chain            TEXT NOT NULL DEFAULT 'ethereum',
    name             TEXT,
    status           TEXT NOT NULL DEFAULT 'pending',
    token_standard   TEXT,
    total_supply     INT,
    is_animated      BOOLEAN DEFAULT false,
    has_svg_images   BOOLEAN DEFAULT false,
    error_message    TEXT,
    requested_by     TEXT,
    traits_synced_at TIMESTAMPTZ,
    market_synced_at TIMESTAMPTZ,
    created_at       TIMESTAMPTZ DEFAULT NOW(),
    updated_at       TIMESTAMPTZ DEFAULT NOW()
  )`);
  // Confirmed live: argonauts genuinely declares EIP-4906 support via
  // supportsInterface(0x49064906) -- checked once during onboarding/backfill
  // and cached here so the metadata-update poller (lib/metadata-update-poller.js)
  // knows which collections to watch without re-checking on every cycle.
  // metadata_update_last_block is that poller's own per-collection cursor,
  // same pattern as burn_events' block-based cursor -- avoids rescanning
  // from genesis on every restart.
  await q(`ALTER TABLE collections ADD COLUMN IF NOT EXISTS metadata_updates_supported BOOLEAN NOT NULL DEFAULT false`).catch(()=>{});
  await q(`ALTER TABLE collections ADD COLUMN IF NOT EXISTS metadata_update_last_block BIGINT`).catch(()=>{});
  // jv: "there should be an automatic retry, server admins shouldn't have
  // to manually re run it if it fails." seedMarketHistory (sync-listings.js)
  // failing (e.g. an OpenSea 429 mid-pull) used to just set status='failed'
  // and stop there -- nothing ever looked at that status again except a
  // human manually re-running /config. market_retry_count and
  // market_next_retry_at back an actual scheduled retry job
  // (retryFailedMarketHistory, bot.js) with exponential backoff, so a
  // transient rate-limit clears on its own without anyone needing to
  // notice the failure webhook and act on it.
  await q(`ALTER TABLE collections ADD COLUMN IF NOT EXISTS market_retry_count INT NOT NULL DEFAULT 0`).catch(()=>{});
  await q(`ALTER TABLE collections ADD COLUMN IF NOT EXISTS market_next_retry_at TIMESTAMPTZ`).catch(()=>{});
  // jv shared two consecutive retry-failure webhooks an hour apart, both
  // failing at the exact same "page 180" -- confirming seedFullSalesHistory
  // (sync-listings.js) restarts its OpenSea event-history pagination from
  // page 1 on every single attempt, with no delay between page requests at
  // all. That combination is *why* it keeps tripping the same rate limit at
  // the same point: every retry re-fetches the same ~180 pages as fast as
  // possible, then dies at the same wall, instead of resuming from wherever
  // the last attempt actually got to. These two columns let it persist its
  // cursor and pick up where it left off instead of restarting from zero
  // every time.
  await q(`ALTER TABLE collections ADD COLUMN IF NOT EXISTS sales_seed_cursor TEXT`).catch(()=>{});
  await q(`ALTER TABLE collections ADD COLUMN IF NOT EXISTS sales_seed_written INT NOT NULL DEFAULT 0`).catch(()=>{});
  await q(`ALTER TABLE collections ADD COLUMN IF NOT EXISTS sales_seed_pages INT NOT NULL DEFAULT 0`).catch(()=>{});
  // jv: hamburger menu's social links (OpenSea/Website/Twitter) were
  // hardcoded to OCAS's own for every collection. opensea_url is always
  // derivable from the slug alone (https://opensea.io/collection/{slug}),
  // stored anyway for a single consistent shape the frontend can read the
  // same way regardless of source. website_url/twitter_url come from
  // OpenSea's own collection metadata (external_url/twitter_username) at
  // onboarding time -- see lib/collection-onboard.js. NULL for either just
  // means OpenSea's own collection page didn't have that filled in, not a
  // fetch failure.
  await q(`ALTER TABLE collections ADD COLUMN IF NOT EXISTS opensea_url TEXT`).catch(()=>{});
  await q(`ALTER TABLE collections ADD COLUMN IF NOT EXISTS website_url TEXT`).catch(()=>{});
  await q(`ALTER TABLE collections ADD COLUMN IF NOT EXISTS twitter_url TEXT`).catch(()=>{});
  // jv: OpenSea-style banner + avatar shown above the collection stats bar
  // on the site. Sourced from OpenSea's own collection object
  // (image_url/banner_image_url) -- see resolveCollectionForOnboarding()
  // and backfillCollectionLinks() in lib/collection-onboard.js.
  await q(`ALTER TABLE collections ADD COLUMN IF NOT EXISTS avatar_image_url TEXT`).catch(()=>{});
  await q(`ALTER TABLE collections ADD COLUMN IF NOT EXISTS banner_image_url TEXT`).catch(()=>{});
  // jv confirmed live on Argonauts: this contract has NO totalSupply() and
  // no conventional sequential mint at all -- ACK holders claim a specific,
  // pre-assigned token ID whenever they like, with no deadline, so most of
  // the 9,999-token collection can be genuinely unclaimed at any given time.
  // Its full source (0x387c41b0...) confirmed the trait table itself is
  // written ONCE before the sale opens and frozen forever after (setPool()
  // permanently locks setTraitChunk()) -- so every token's traits are
  // predetermined on-chain, independent of whether it's ever claimed.
  // trait_source_mode='predetermined_onchain' flags a collection as having
  // this architecture; trait_source_renderer_contract is the separate
  // contract (confirmed via the main contract's own `renderer` getter) whose
  // tokenURI(tokenId, traits, printed) needs no ownership at all -- see
  // lib/predetermined-traits.js. Default 'minted' preserves the existing
  // Alchemy/OpenSea-indexed-NFT behavior unchanged for every other
  // collection, which is the correct source whenever traits only exist
  // once a token is actually minted.
  await q(`ALTER TABLE collections ADD COLUMN IF NOT EXISTS trait_source_mode TEXT NOT NULL DEFAULT 'minted'`).catch(()=>{});
  await q(`ALTER TABLE collections ADD COLUMN IF NOT EXISTS trait_source_renderer_contract TEXT`).catch(()=>{});
  // jv: "is there anyway to set personal alerts for trait counts" ->
  // "just to make sure, the trait counts for each collection match
  // traitview trait counts correct?" -- they didn't, for argonauts
  // specifically: traitview's own displayed trait count excludes a
  // per-collection list of "non-worn"/cosmetic categories
  // (js/config.js's COLLECTIONS[slug].nonWornTraitCategories, argonauts:
  // Bones/Palette/Print/Fate) that only ever existed hardcoded in the
  // frontend -- the bot's trait-count alert (lib/poll.js) had no way to
  // know about it, so it counted every trait in OpenSea's raw payload
  // unfiltered. Moving this into the collections registry itself (with
  // argonauts' existing value backfilled below) makes it the one shared
  // source both the site and the bot read from, instead of two
  // separately hardcoded copies that can silently drift apart -- exactly
  // the kind of multi-collection consistency jv's asked to keep in mind
  // going forward. Empty for every other collection (nothing to
  // exclude), same as before.
  await q(`ALTER TABLE collections ADD COLUMN IF NOT EXISTS non_worn_trait_categories TEXT[] NOT NULL DEFAULT '{}'`).catch(()=>{});
  await q(`UPDATE collections SET non_worn_trait_categories = ARRAY['Bones','Palette','Print','Fate'] WHERE slug = 'argonauts' AND non_worn_trait_categories = '{}'`).catch(()=>{});
  // jv: emoji reactions on the landing page -- "open, but still only 1
  // tap per person if doable" -- then "yes i want multiple picks per
  // person just not the same one twice". No login system exists on the
  // site at all, so "per person" means a persistent anonymous client id
  // the frontend generates once and stores in localStorage (see
  // js/landing.js) -- not airtight against someone clearing storage or
  // switching browsers, but the standard approach for this kind of
  // lightweight, no-account reaction feature. UNIQUE is now
  // (slug, client_id, emoji) rather than just (slug, client_id) -- a
  // client can have one row per DISTINCT emoji for a collection (so
  // 🚀 and ❤️ can both be active at once), but never two rows for the
  // same emoji, which is what actually enforces "not the same one
  // twice". Each row is inserted/deleted individually (a toggle per
  // emoji) rather than one row's value being changed.
  await q(`CREATE TABLE IF NOT EXISTS collection_reactions (
    id SERIAL PRIMARY KEY,
    slug TEXT NOT NULL,
    client_id TEXT NOT NULL,
    emoji TEXT NOT NULL,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE(slug, client_id, emoji)
  )`).catch(()=>{});
  await q(`ALTER TABLE collection_reactions DROP CONSTRAINT IF EXISTS collection_reactions_slug_client_id_key`).catch(()=>{});
  // jv: "clicking on an emoji to select just highlights it, doesn't
  // actually select" -- this table already existed from the previous
  // deploy (one-per-person model), so the CREATE TABLE IF NOT EXISTS
  // above was a no-op there and its UNIQUE(slug, client_id, emoji)
  // never actually got applied -- only the DROP CONSTRAINT ran, leaving
  // an EXISTING table with no unique constraint on these three columns
  // at all. The POST route's ON CONFLICT (slug, client_id, emoji)
  // clause then throws a real Postgres error ("no unique or exclusion
  // constraint matching the ON CONFLICT specification") on every single
  // request, which is why every reaction attempt silently failed --
  // the button dims on click (the frontend's own pending state) and
  // just stays that way, since the request never actually succeeds to
  // refresh it back. Explicitly adding the constraint for any table
  // that's missing it; harmlessly ignored once it already exists.
  await q(`ALTER TABLE collection_reactions ADD CONSTRAINT collection_reactions_slug_client_id_emoji_key UNIQUE(slug, client_id, emoji)`).catch(()=>{});
  await q(`CREATE INDEX IF NOT EXISTS idx_collection_reactions_slug ON collection_reactions(slug)`).catch(()=>{});
  // jv: "There are actually trades happening on Gondi as well. Is there
  // anyway to catch those in traitview?" Gondi is a real, separate NFT
  // marketplace (with its own order book, distinct from OpenSea's) that
  // supports direct trading in addition to its lending features -- see
  // lib/gondi-sync.js for the full integration. This tracks active
  // Gondi listings specifically; price_wei/currency_address are
  // nullable because the SDK's documented listings() response type
  // doesn't show a price field at all -- left in place defensively in
  // case the real runtime response includes one anyway, filled in only
  // if so.
  await q(`CREATE TABLE IF NOT EXISTS gondi_listings (
    id SERIAL PRIMARY KEY,
    collection_slug TEXT NOT NULL,
    token_id INT NOT NULL,
    gondi_listing_id TEXT NOT NULL,
    marketplace_name TEXT,
    seller_wallet TEXT,
    price_wei NUMERIC,
    currency_address TEXT,
    gondi_created_at TIMESTAMPTZ,
    first_seen_at TIMESTAMPTZ DEFAULT NOW(),
    last_seen_at TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE(collection_slug, gondi_listing_id)
  )`).catch(()=>{});
  await q(`CREATE INDEX IF NOT EXISTS idx_gondi_listings_slug ON gondi_listings(collection_slug)`).catch(()=>{});
  // jv: "Is it better to store an .eth addresses in a db and then run
  // that db for the holdings of that address?" ENS names and linked X
  // accounts almost never change, unlike holdings (which can change
  // any time a token trades hands) -- a correct target for this kind
  // of caching in a way live holdings genuinely aren't. Checked before
  // ever calling ENS/OpenSea for a given address; refreshed only once
  // stale (see /tv/identity in api.js), rather than on every single
  // lookup.
  await q(`CREATE TABLE IF NOT EXISTS wallet_identity (
    address TEXT PRIMARY KEY,
    ens_name TEXT,
    twitter_username TEXT,
    checked_at TIMESTAMPTZ DEFAULT NOW()
  )`).catch(()=>{});
  // Confirmed live via a direct supportsInterface(0x49064906) eth_call
  // (not just an event with a matching name) that argonauts genuinely
  // supports EIP-4906. Idempotent — safe to re-run every startup, and
  // harmless for any guild/server that hasn't onboarded this slug yet
  // (UPDATE against a non-existent row is a no-op).
  await q(`UPDATE collections SET metadata_updates_supported = true WHERE slug = 'argonauts'`).catch(()=>{});
  // Seed OCAS itself as 'ready' so the registry is a complete picture from day one
  await q(`INSERT INTO collections (slug, contract, name, status, traits_synced_at, market_synced_at)
    VALUES ('on-chain-all-stars', '0x078be86f3104a32313a47815792230a3808642cc', 'On-Chain All Stars', 'ready', NOW(), NOW())
    ON CONFLICT (slug) DO NOTHING`).catch(()=>{});

  // jv: Stackers rugged and scammed its holders, and the feature was
  // already fully removed from the bot/API/frontend (code, commands,
  // pollers). This drops the data itself too, per jv's explicit
  // confirmation -- "remove it from everywhere" includes what's sitting
  // in the database, not just the code that used to read/write it.
  // DROP TABLE IF EXISTS is idempotent: harmless to leave running on every
  // future startup, since after the first successful drop this is just a
  // silent no-op forever after.
  await q(`DROP TABLE IF EXISTS stackers_snapshots`).catch(()=>{});
  await q(`DROP TABLE IF EXISTS stackers_image_cache`).catch(()=>{});
  await q(`DROP TABLE IF EXISTS stackers_vault_listings`).catch(()=>{});
  await q(`DROP TABLE IF EXISTS stackers_token_status`).catch(()=>{});
  await q(`DROP TABLE IF EXISTS stackers_vault_snapshots`).catch(()=>{});
  await q(`DROP TABLE IF EXISTS stackers_round_history`).catch(()=>{});
  // Also remove Stackers' own row from the generic collections registry --
  // otherwise it would still surface as a registered collection (e.g. via
  // /db/collections, and therefore on TraitView's own collection list)
  // even with every feature/command that used to serve it already gone.
  await q(`DELETE FROM collections WHERE slug = 'stackersxyz'`).catch(()=>{});

  await q(`CREATE TABLE IF NOT EXISTS verification_codes (
    discord_id  TEXT NOT NULL,
    guild_id    TEXT NOT NULL DEFAULT 'global',
    wallet      TEXT,
    code        TEXT,
    expires_at  TIMESTAMPTZ,
    PRIMARY KEY (discord_id, guild_id)
  )`);
  await q(`ALTER TABLE user_registrations ADD COLUMN IF NOT EXISTS guild_id TEXT NOT NULL DEFAULT 'global'`);

  await q(`CREATE TABLE IF NOT EXISTS bot_state (key TEXT PRIMARY KEY, value TEXT)`);

  // TraitView↔Discord verification
  await q(`CREATE TABLE IF NOT EXISTS tv_verify_codes (
    code        TEXT PRIMARY KEY,
    discord_id  TEXT NOT NULL,
    guild_id    TEXT NOT NULL,
    wallet      TEXT NOT NULL,
    direction   TEXT NOT NULL DEFAULT 'discord', -- 'discord' = generated by bot, 'traitview' = generated by TV
    expires_at  TIMESTAMPTZ NOT NULL,
    claimed_at  TIMESTAMPTZ,
    created_at  TIMESTAMPTZ DEFAULT NOW()
  )`);
  await q(`CREATE INDEX IF NOT EXISTS tv_verify_discord_idx ON tv_verify_codes(discord_id)`);

  await q(`CREATE TABLE IF NOT EXISTS traitview_links (
    discord_id  TEXT NOT NULL,
    guild_id    TEXT NOT NULL,
    wallet      TEXT NOT NULL,
    linked_at   TIMESTAMPTZ DEFAULT NOW(),
    PRIMARY KEY (discord_id, guild_id)
  )`);
  await q(`CREATE TABLE IF NOT EXISTS server_configs (guild_id TEXT PRIMARY KEY, config JSONB NOT NULL DEFAULT '{}')`);
  await q(`CREATE TABLE IF NOT EXISTS user_alert_configs (user_id TEXT NOT NULL, guild_id TEXT NOT NULL, config JSONB NOT NULL DEFAULT '{}', PRIMARY KEY(user_id,guild_id))`);
  await q(`CREATE TABLE IF NOT EXISTS verification_panels (
    guild_id     TEXT PRIMARY KEY,
    channel_id   TEXT,
    role_id      TEXT,
    holder_role_id TEXT,
    min_tokens   INTEGER DEFAULT 0,
    message_id   TEXT,
    welcome_text TEXT,
    created_at   TIMESTAMPTZ DEFAULT NOW()
  `);
  await q(`ALTER TABLE verification_panels ADD COLUMN IF NOT EXISTS holder_role_id TEXT`);
  await q(`CREATE TABLE IF NOT EXISTS trait_roles (
    id             SERIAL PRIMARY KEY,
    guild_id       TEXT NOT NULL,
    trait_type     TEXT NOT NULL,
    trait_value    TEXT,
    role_id        TEXT NOT NULL,
    minimum_count  INTEGER NOT NULL DEFAULT 1,
    created_at     TIMESTAMPTZ DEFAULT NOW()
  )`);
  // Make trait_value nullable if it was NOT NULL before
  await q(`ALTER TABLE trait_roles ALTER COLUMN trait_value DROP NOT NULL`).catch(()=>{});
  // Drop old unique constraint and recreate with COALESCE-friendly version
  await q(`ALTER TABLE trait_roles DROP CONSTRAINT IF EXISTS trait_roles_guild_id_trait_type_trait_value_role_id_minimu_key`).catch(()=>{});
  await q(`CREATE UNIQUE INDEX IF NOT EXISTS trait_roles_unique ON trait_roles (guild_id, trait_type, COALESCE(trait_value,''), role_id, minimum_count)`).catch(()=>{});
  await q(`ALTER TABLE trait_roles ADD COLUMN IF NOT EXISTS minimum_count INTEGER NOT NULL DEFAULT 1`);
  // collection_slug scopes a rule to a specific collection; NULL means it applies to the guild's primary collection
  await q(`ALTER TABLE trait_roles ADD COLUMN IF NOT EXISTS collection_slug TEXT`);
  // Migrate old verification_codes to guild_id format if needed
  await q(`ALTER TABLE verification_codes ADD COLUMN IF NOT EXISTS guild_id TEXT NOT NULL DEFAULT 'global'`);
  // Migrate primary key from (discord_id) to (discord_id, guild_id) if needed
  await q(`DO $$
    BEGIN
      IF EXISTS (
        SELECT 1 FROM information_schema.table_constraints
        WHERE table_name='verification_codes' AND constraint_type='PRIMARY KEY'
        AND constraint_name NOT LIKE '%discord_id%guild_id%'
      ) THEN
        ALTER TABLE verification_codes DROP CONSTRAINT IF EXISTS verification_codes_pkey;
        ALTER TABLE verification_codes ADD PRIMARY KEY (discord_id, guild_id);
      END IF;
    END $$`).catch(()=>{});
  // Ensure PK covers both columns regardless
  await q(`DO $$
    BEGIN
      IF NOT EXISTS (
        SELECT 1 FROM pg_index i
        JOIN pg_attribute a ON a.attrelid = i.indrelid AND a.attnum = ANY(i.indkey)
        WHERE i.indrelid = 'verification_codes'::regclass AND i.indisprimary
          AND a.attname = 'guild_id'
      ) THEN
        ALTER TABLE verification_codes DROP CONSTRAINT IF EXISTS verification_codes_pkey;
        ALTER TABLE verification_codes ADD CONSTRAINT verification_codes_pkey PRIMARY KEY (discord_id, guild_id);
      END IF;
    END $$`).catch(()=>{});
  await q(`CREATE TABLE IF NOT EXISTS sales (id SERIAL PRIMARY KEY, token_id INT, sale_price NUMERIC, sale_ts TIMESTAMPTZ, tx_hash TEXT, buyer TEXT, seller TEXT, collection_slug TEXT, UNIQUE(tx_hash, token_id))`);
  // price_eth/currency: confirmed real columns sync-listings.js actually
  // writes to (INSERT INTO sales (token_id, price_eth, currency, ...)) --
  // missing from the table above entirely, same pattern as the tokens
  // table's obs_rank/rarity_score gap found earlier tonight. sale_price
  // is a separate, legacy column left in place, unused by current code
  // but harmless to leave.
  await q(`ALTER TABLE sales ADD COLUMN IF NOT EXISTS price_eth NUMERIC`).catch(()=>{});
  await q(`ALTER TABLE sales ADD COLUMN IF NOT EXISTS currency TEXT`).catch(()=>{});
  // Where a sale happened when it wasn't OpenSea (NULL = OpenSea, the
  // default source). Set by lib/gondi-activity-sync.js: 'gondi' for sales
  // Gondi indexed on a non-OpenSea marketplace, 'gondi-trade' for a Gondi
  // P2P trade that was one NFT for ETH/WETH only (effectively a sale).
  await q(`ALTER TABLE sales ADD COLUMN IF NOT EXISTS marketplace TEXT`).catch(()=>{});
  // Every executed Gondi P2P trade ("deal"), both sides in full -- swaps
  // with NFTs on both sides have no single price, so they live here (token
  // history / wallet panel) rather than as sales.
  await q(`CREATE TABLE IF NOT EXISTS gondi_trades (
    deal_id TEXT NOT NULL, collection_slug TEXT NOT NULL,
    maker TEXT, taker TEXT, tx_hash TEXT, executed_at TIMESTAMPTZ,
    maker_nfts JSONB, taker_nfts JSONB,
    maker_erc20s JSONB, maker_erc20_amounts JSONB, taker_erc20s JSONB, taker_erc20_amounts JSONB,
    is_sale BOOLEAN DEFAULT FALSE,
    PRIMARY KEY (deal_id, collection_slug))`).catch(()=>{});
  // The real ON CONFLICT target sync-listings.js actually uses is
  // (token_id, sale_ts, collection_slug) -- confirmed via its own insert
  // statements -- not the (tx_hash, token_id) constraint created above.
  // Postgres requires an actual matching unique constraint for ON CONFLICT
  // to work at all; without this, every sales insert would fail with a
  // different error even after price_eth/currency exist.
  await q(`CREATE UNIQUE INDEX IF NOT EXISTS sales_token_ts_slug_unique ON sales (token_id, sale_ts, collection_slug)`).catch(()=>{});
  // CREATE TABLE IF NOT EXISTS above silently does nothing for the
  // collection_slug column on a table that already existed in an older
  // shape — that's exactly what happened here in production (discovered
  // via migrations/004). Explicit ALTER below ensures it actually gets
  // added on any deploy where the table predates this column.
  await q(`ALTER TABLE sales ADD COLUMN IF NOT EXISTS collection_slug TEXT DEFAULT 'on-chain-all-stars'`);
  await q(`UPDATE sales SET collection_slug = 'on-chain-all-stars' WHERE collection_slug IS NULL`).catch(()=>{});
  await q(`CREATE TABLE IF NOT EXISTS listings (token_id INT, price_eth NUMERIC, list_ts TIMESTAMPTZ, order_hash TEXT, collection_slug TEXT, url TEXT, updated_at TIMESTAMPTZ, UNIQUE(token_id, collection_slug))`);
  // jv: "make the weth and eth wording through the page green for eth and
  // red for weth" -- listings never tracked which currency they were
  // actually in, only a numeric price_eth value, unlike sales (which
  // already has its own currency column). Defaulting existing rows to
  // 'ETH' rather than leaving them NULL, since every listing synced
  // before this column existed was, in practice, priced in ETH.
  await q(`ALTER TABLE listings ADD COLUMN IF NOT EXISTS currency TEXT DEFAULT 'ETH'`);
  await q(`ALTER TABLE listings ADD COLUMN IF NOT EXISTS collection_slug TEXT DEFAULT 'on-chain-all-stars'`);
  await q(`ALTER TABLE listings ADD COLUMN IF NOT EXISTS price_eth NUMERIC`).catch(()=>{});
  await q(`ALTER TABLE listings ADD COLUMN IF NOT EXISTS url TEXT`).catch(()=>{});
  await q(`ALTER TABLE listings ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ`).catch(()=>{});
  await q(`UPDATE listings SET collection_slug = 'on-chain-all-stars' WHERE collection_slug IS NULL`).catch(()=>{});
  await q(`ALTER TABLE listings ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT NOW()`);
  await q(`CREATE TABLE IF NOT EXISTS burn_events (id SERIAL PRIMARY KEY, tx_hash TEXT UNIQUE, block_number BIGINT, burned_token_id INT, survivor_token_id INT, burn_type TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`);
  await q(`ALTER TABLE burn_events ADD COLUMN IF NOT EXISTS survivor_token_id INT`);
  await q(`ALTER TABLE burn_events ADD COLUMN IF NOT EXISTS burn_type TEXT`);
  await q(`ALTER TABLE burn_events DROP CONSTRAINT IF EXISTS burn_events_tx_hash_key`);
  await q(`CREATE UNIQUE INDEX IF NOT EXISTS burn_events_tx_hash_idx ON burn_events(tx_hash)`);
  await q(`CREATE INDEX IF NOT EXISTS burn_events_burner_wallet_idx ON burn_events(LOWER(burner_wallet))`).catch(()=>{});
  await q(`CREATE TABLE IF NOT EXISTS burn_event_inputs (id SERIAL PRIMARY KEY, burn_event_id INT REFERENCES burn_events(id), burned_token_id INT, UNIQUE(burn_event_id,burned_token_id))`);
  // burn_started_events/burn_started_inputs were previously only ever created
  // by the one-off burn-backfill-repair.js script, not by this regular
  // migration path -- meaning any fresh environment that never had that
  // script run against it (e.g. a new API/bot service) would be missing
  // these tables entirely. Adding them here so they self-heal like
  // everything else runMigrations() covers. Schema matches burn-backfill-
  // repair.js exactly.
  await q(`CREATE TABLE IF NOT EXISTS burn_started_events (
    id SERIAL PRIMARY KEY, tx_hash TEXT NOT NULL, block_number BIGINT NOT NULL,
    log_index INT NOT NULL, owner_wallet TEXT NOT NULL, survivor_token_id INT NOT NULL,
    points_used INT, result_body_type INT, result_is_angel BOOLEAN DEFAULT FALSE,
    boost_chance INT, reveal_block BIGINT, selection_hash TEXT, started_at TIMESTAMPTZ DEFAULT NOW()
  )`);
  await q(`CREATE UNIQUE INDEX IF NOT EXISTS burn_started_tx_log_idx ON burn_started_events(tx_hash, log_index)`);
  await q(`CREATE INDEX IF NOT EXISTS burn_started_survivor_idx ON burn_started_events(survivor_token_id)`);
  await q(`CREATE TABLE IF NOT EXISTS burn_started_inputs (
    id SERIAL PRIMARY KEY,
    burn_started_id INT NOT NULL REFERENCES burn_started_events(id) ON DELETE CASCADE,
    burned_token_id INT NOT NULL
  )`);
  await q(`CREATE UNIQUE INDEX IF NOT EXISTS burn_started_inputs_event_token_idx ON burn_started_inputs(burn_started_id, burned_token_id)`);
  // Fix for the pendingBurns race condition (2026-07-06): a survivor burned
  // more than once in quick succession could have its BurnFinalized events
  // matched to the WRONG BurnStarted record (whichever was most recently
  // cached/queried, not the one that actually paired with that finalize),
  // corrupting burn_event_inputs with duplicated fuel-token lists. This
  // column marks a start event as consumed once matched, so both the
  // in-memory FIFO queue and the DB fallback in loadBurnStartFromDB() can
  // never re-select an already-used start for a later finalize.
  await q(`ALTER TABLE burn_started_events ADD COLUMN IF NOT EXISTS matched_burn_event_id INT`).catch(()=>{});
  await q(`CREATE INDEX IF NOT EXISTS burn_started_events_unmatched_idx ON burn_started_events(survivor_token_id, block_number) WHERE matched_burn_event_id IS NULL`).catch(()=>{});
  await q(`CREATE TABLE IF NOT EXISTS burn_alert_posts (id SERIAL PRIMARY KEY, tx_hash TEXT, log_index INT, channel_id TEXT, posted_at TIMESTAMPTZ DEFAULT NOW(), UNIQUE(tx_hash,log_index,channel_id))`);
  await q(`CREATE TABLE IF NOT EXISTS token_traits (token_id INT NOT NULL, trait_name TEXT NOT NULL, trait_value TEXT, trait_index INT NOT NULL DEFAULT 0, source TEXT, updated_at TIMESTAMPTZ DEFAULT NOW(), PRIMARY KEY(token_id,trait_index), UNIQUE(token_id,trait_index))`);
  await q(`ALTER TABLE token_traits ADD COLUMN IF NOT EXISTS collection_slug TEXT`).catch(()=>{});
  await q(`CREATE INDEX IF NOT EXISTS token_traits_slug_idx ON token_traits(token_id, collection_slug)`).catch(()=>{});
  // token_traits.PRIMARY KEY(token_id, trait_index) predates multi-collection
  // support, same as tokens.id above -- collection_slug was bolted on as a
  // plain nullable column, but the actual enforced constraints were never
  // widened to match. Confirmed live: adding "argonauts" (a new collection on
  // Ethereum) aborted its backfill entirely with "duplicate key value
  // violates unique constraint token_traits_pkey" on every single page --
  // its token IDs collided with an EARLIER, unrelated collection's rows at
  // the same (token_id, trait_index) pairs, since nothing in the actual
  // schema distinguishes them by collection. Worse than the tokens table's
  // version of this same bug: this table was declared with BOTH a PRIMARY
  // KEY(token_id, trait_index) AND a separate, redundant
  // UNIQUE(token_id, trait_index) in the same CREATE TABLE statement --
  // Postgres treats these as two independent constraints, so widening only
  // the primary key would leave the second one silently enforcing the exact
  // same too-narrow rule. Both need dropping and replacing. Idempotent --
  // checks the actual constraint columns first so this is a no-op after the
  // first successful run.
  try{
    const constraintsRes = await pgPool.query(`
      SELECT con.conname, con.contype, string_agg(a.attname::text, ',' ORDER BY a.attnum) AS cols
      FROM pg_constraint con
      JOIN pg_attribute a ON a.attrelid = con.conrelid AND a.attnum = ANY(con.conkey)
      WHERE con.conrelid = 'token_traits'::regclass AND con.contype IN ('p', 'u')
      GROUP BY con.conname, con.contype
    `);
    // a.attname is Postgres's internal `name` type -- array_agg() over it
    // produces a name[] array, and node-postgres has no default parser
    // registered for that specific array OID, so it silently comes back as
    // a raw string instead of a parsed JS array. Confirmed live: this threw
    // "r.cols.sort is not a function" on every startup, meaning the
    // migration never actually ran at all -- the try/catch caught it and
    // logged a warning, but "argonauts" kept hitting the exact same
    // unfixed constraint on every retry. string_agg(...::text, ',') sidesteps
    // this entirely by returning a plain string, which is unambiguous
    // across any Postgres driver.
    const tooNarrow = constraintsRes.rows.filter(r => r.cols.split(',').sort().join(',') === 'token_id,trait_index');
    if(tooNarrow.length){
      // Explicit transaction, not sequential pgPool.query() calls — this
      // migration drops TWO constraints (unlike tokens' single-constraint
      // version) before adding the new one back, so a failure partway
      // through has a real, worse failure mode here: the table could be
      // left with no primary key at all until manually fixed. All-or-
      // nothing keeps it at its original (at least known-working) state on
      // any failure instead.
      const client = await pgPool.connect();
      try{
        await client.query('BEGIN');
        await client.query(`UPDATE token_traits SET collection_slug='on-chain-all-stars' WHERE collection_slug IS NULL`);
        await client.query(`ALTER TABLE token_traits ALTER COLUMN collection_slug SET NOT NULL`);
        for(const { conname, contype } of tooNarrow){
          await client.query(`ALTER TABLE token_traits DROP CONSTRAINT "${conname}"`);
          console.log(`[Migration] token_traits: dropped too-narrow ${contype === 'p' ? 'PRIMARY KEY' : 'UNIQUE'} constraint "${conname}" on (token_id, trait_index)`);
        }
        await client.query(`ALTER TABLE token_traits ADD PRIMARY KEY (token_id, trait_index, collection_slug)`);
        await client.query('COMMIT');
        console.log('[Migration] token_traits: widened PRIMARY KEY to (token_id, trait_index, collection_slug)');
      }catch(e){
        await client.query('ROLLBACK');
        throw e;
      }finally{
        client.release();
      }
    }
  }catch(e){
    console.warn('[Migration] token_traits PK widen failed (non-fatal, will retry next startup):', e.message);
  }
  await q(`CREATE TABLE IF NOT EXISTS token_image_snapshots (token_id INT PRIMARY KEY, image_url TEXT, image_source TEXT, updated_at TIMESTAMPTZ DEFAULT NOW())`);
  await q(`CREATE TABLE IF NOT EXISTS token_original_snapshots (token_id INT PRIMARY KEY, image_url TEXT, image_source TEXT, captured_at TIMESTAMPTZ DEFAULT NOW())`);
  await q(`CREATE TABLE IF NOT EXISTS burn_state_snapshots (id SERIAL PRIMARY KEY, burn_event_id INT REFERENCES burn_events(id), snapshot_type TEXT, token_id INT, image_url TEXT, image_source TEXT, metadata JSONB, captured_at TIMESTAMPTZ DEFAULT NOW())`);
  await q(`CREATE TABLE IF NOT EXISTS burn_lotteries (id SERIAL PRIMARY KEY, guild_id TEXT, channel_id TEXT, created_by TEXT, title TEXT, prize TEXT, mode TEXT DEFAULT 'wallet', start_time TIMESTAMPTZ, end_time TIMESTAMPTZ, seed TEXT, status TEXT DEFAULT 'active', winner_wallet TEXT, qualified_wallets INT, total_burns INT, result_json JSONB, timezone TEXT, completed_at TIMESTAMPTZ, message_id TEXT)`);
  await q(`CREATE TABLE IF NOT EXISTS generic_lotteries (id SERIAL PRIMARY KEY, guild_id TEXT, channel_id TEXT, created_by TEXT, title TEXT, type TEXT DEFAULT 'giveaway', start_time TIMESTAMPTZ, end_time TIMESTAMPTZ, seed TEXT, status TEXT DEFAULT 'active', winner_display TEXT, min_number INT, max_number INT, winning_number INT, result_json JSONB, timezone TEXT, completed_at TIMESTAMPTZ, message_id TEXT)`);
  await q(`CREATE TABLE IF NOT EXISTS generic_lottery_entries (id SERIAL PRIMARY KEY, lottery_id INT REFERENCES generic_lotteries(id), user_id TEXT, username TEXT, guess_number INT, entered_at TIMESTAMPTZ DEFAULT NOW(), UNIQUE(lottery_id,user_id))`);
  await q(`ALTER TABLE generic_lotteries ADD COLUMN IF NOT EXISTS message_id TEXT`);
  await q(`CREATE TABLE IF NOT EXISTS tokens (id INT PRIMARY KEY, os_rank INT, tv_rank INT, updated_at TIMESTAMPTZ DEFAULT NOW())`);
  await q(`ALTER TABLE tokens ADD COLUMN IF NOT EXISTS collection_slug TEXT`).catch(()=>{});
  await q(`ALTER TABLE tokens ADD COLUMN IF NOT EXISTS image_url TEXT`).catch(()=>{});
  // obs_rank/rarity_score/trait_count/os_score: legacy columns that existed
  // on the original database with no tracked migration file anywhere adding
  // them -- migrations 005/006 only ever adjust obs_rank/rarity_score's
  // nullability, never create them. Added here so any fresh database (this
  // bot's own new deployment included) gets the full schema automatically,
  // rather than silently missing columns that /sweep, /rankfind, and other
  // rank-aware commands all expect to exist.
  await q(`ALTER TABLE tokens ADD COLUMN IF NOT EXISTS obs_rank INTEGER`).catch(()=>{});
  await q(`ALTER TABLE tokens ADD COLUMN IF NOT EXISTS rarity_score NUMERIC(18,6)`).catch(()=>{});
  await q(`ALTER TABLE tokens ADD COLUMN IF NOT EXISTS trait_count INTEGER`).catch(()=>{});
  await q(`ALTER TABLE tokens ADD COLUMN IF NOT EXISTS os_score NUMERIC(18,6)`).catch(()=>{});
  // is_burned: generic, contract-agnostic burned-token flag (lib/burn-detect.js)
  // for collections with no protocol-level burn mechanic of their own (e.g.
  // Argonauts, where a third-party account independently sent tokens to a
  // dead address) -- separate from and additional to OCAS's own
  // burn_events-based tracking, which models a real, different mechanic
  // (multiple tokens burned to produce one survivor) this flag doesn't
  // capture at all.
  await q(`ALTER TABLE tokens ADD COLUMN IF NOT EXISTS is_burned BOOLEAN DEFAULT FALSE`).catch(()=>{});
  // jv: "the burns... are actually animated. Anyway to get that
  // animation to display on the site??" Argonauts' burned tokens have
  // their own animated art the collection's creator set, distinct from
  // image_url/token_svg_cache (both static) -- captured by
  // lib/metadata-update-poller.js's refreshSingleTokenMetadata()
  // alongside the static image, exposed via /db/all-traits and
  // /db/token/:id for the frontend to render.
  await q(`ALTER TABLE tokens ADD COLUMN IF NOT EXISTS animation_url TEXT`).catch(()=>{});
  // is_minted: TRUE for every row written by the normal Alchemy/OpenSea
  // backfill path (an actually-owned ERC-721), which is why this defaults
  // to true -- every row any collection has ever written before this column
  // existed genuinely represents a minted token. Only lib/predetermined-traits.js
  // ever writes FALSE, for a predetermined-onchain collection's unclaimed
  // token IDs: real trait/rarity data exists and belongs in rank-compute's
  // full-collection stats (matching what the official site already shows),
  // but nothing ownership-dependent -- who holds it, sales, listings, a
  // wallet view -- is real yet. Any endpoint that answers an ownership
  // question needs `AND is_minted = true`; anything answering a
  // trait/rarity question about the collection as a whole should NOT filter
  // on it, the same way the official site counts the full 9,999.
  await q(`ALTER TABLE tokens ADD COLUMN IF NOT EXISTS is_minted BOOLEAN NOT NULL DEFAULT true`).catch(()=>{});

  // jv: "Trait Pulse" -- ranks trait/value combinations by unusual recent
  // activity (sales velocity relative to that trait's OWN historical rate,
  // listing-supply depletion, trait-floor movement), not raw sale counts,
  // so a common trait's larger raw volume doesn't automatically win over a
  // rare trait's real, unusual burst. Two tables: a periodic snapshot
  // (listed count + trait floor per trait/value, taken regularly so
  // deltas over 1h/6h/24h/7d windows are computable) and the final,
  // per-window cached scores the API actually serves -- computing this
  // live on every page load would mean scanning every trait/value
  // combination across four windows on every request, exactly the kind of
  // work this codebase already established a pattern for doing as a
  // periodic background job instead (lib/trait-pulse.js).
  await q(`CREATE TABLE IF NOT EXISTS trait_pulse_snapshots (
    id SERIAL PRIMARY KEY,
    collection_slug TEXT NOT NULL,
    trait_name TEXT NOT NULL,
    trait_value TEXT NOT NULL,
    snapshot_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    listed_count INT NOT NULL,
    trait_floor_eth NUMERIC
  )`);
  await q(`CREATE INDEX IF NOT EXISTS trait_pulse_snapshots_lookup
    ON trait_pulse_snapshots (collection_slug, trait_name, trait_value, snapshot_at)`);

  await q(`CREATE TABLE IF NOT EXISTS trait_pulse_cache (
    collection_slug TEXT NOT NULL,
    trait_name TEXT NOT NULL,
    trait_value TEXT NOT NULL,
    window_key TEXT NOT NULL,
    supply INT NOT NULL,
    sales_count INT NOT NULL,
    baseline_expected NUMERIC,
    velocity_multiplier NUMERIC,
    listed_count INT,
    listed_count_prior INT,
    listing_pressure_pct NUMERIC,
    trait_floor_eth NUMERIC,
    trait_floor_eth_prior NUMERIC,
    floor_change_pct NUMERIC,
    heat_score NUMERIC NOT NULL,
    computed_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    PRIMARY KEY (collection_slug, trait_name, trait_value, window_key)
  )`);
  // tokens.id was the SOLE PRIMARY KEY from before multi-collection existed.
  // collection_slug was later bolted on as a plain nullable column (the
  // ALTER TABLE above), but the PK itself was never widened to match --
  // meaning `id` alone still had to be globally unique across every
  // collection in the table. Since token IDs restart from 1 in nearly every
  // collection, this silently blocked any second collection from ever
  // writing an overlapping ID: writePage()'s
  // `ON CONFLICT (id, collection_slug)` doesn't match a PK that's just
  // `(id)`, so Postgres throws "no unique or exclusion constraint matching
  // the ON CONFLICT specification" on every single write. This is the
  // actual root cause behind robinhood-chimps' backfill repeatedly writing
  // zero tokens. Confirmed no other table has a foreign key into
  // tokens(id), so widening the PK is safe. Idempotent — checks the actual
  // PK columns first so this is a no-op after the first successful run.
  try{
    const pkColsRes = await pgPool.query(`
      SELECT a.attname FROM pg_index i
      JOIN pg_attribute a ON a.attrelid = i.indrelid AND a.attnum = ANY(i.indkey)
      WHERE i.indrelid = 'tokens'::regclass AND i.indisprimary
    `);
    const pkCols = pkColsRes.rows.map(r => r.attname).sort().join(',');
    if(pkCols !== 'collection_slug,id'){
      // Legacy rows written before collection_slug existed are all OCAS --
      // it was the only collection ever tracked before this column existed.
      await pgPool.query(`UPDATE tokens SET collection_slug='on-chain-all-stars' WHERE collection_slug IS NULL`);
      await pgPool.query(`ALTER TABLE tokens ALTER COLUMN collection_slug SET NOT NULL`);
      const pkNameRes = await pgPool.query(`SELECT conname FROM pg_constraint WHERE conrelid = 'tokens'::regclass AND contype = 'p'`);
      const pkName = pkNameRes.rows[0]?.conname;
      if(pkName) await pgPool.query(`ALTER TABLE tokens DROP CONSTRAINT "${pkName}"`);
      await pgPool.query(`ALTER TABLE tokens ADD PRIMARY KEY (id, collection_slug)`);
      console.log('[Migration] tokens: widened PRIMARY KEY from (id) to (id, collection_slug)');
    }
  }catch(e){
    console.warn('[Migration] tokens PK widen failed (non-fatal, will retry next startup):', e.message);
  }
  await q(`CREATE TABLE IF NOT EXISTS token_svg_cache (
    token_id INT NOT NULL,
    collection_slug TEXT NOT NULL,
    image_data TEXT NOT NULL,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    PRIMARY KEY (token_id, collection_slug)
  )`).catch(()=>{});

  // ── Metadata repair queue ───────────────────────────────────────────────────
  // Part of the tiered backfill rewrite: the bulk-first onboarding pass writes
  // whatever data a collection's provider(s) already have cached immediately,
  // and queues anything genuinely missing/incomplete here instead of making
  // the ENTIRE collection wait on the slowest few percent of tokens. A
  // separate, non-blocking repair worker (runMetadataRepairQueue in
  // lib/collection-backfill.js) drains this table using the full
  // gateway-race/retry/cooldown machinery, after the collection is already
  // usable.
  await q(`CREATE TABLE IF NOT EXISTS metadata_repair_jobs (
    id SERIAL PRIMARY KEY,
    collection_slug TEXT NOT NULL,
    token_id INT NOT NULL,
    contract TEXT NOT NULL,
    chain TEXT NOT NULL,
    attempts INT NOT NULL DEFAULT 0,
    status TEXT NOT NULL DEFAULT 'pending',
    last_attempt_at TIMESTAMPTZ,
    last_error TEXT,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE (collection_slug, token_id)
  )`).catch(()=>{});
  await q(`CREATE INDEX IF NOT EXISTS metadata_repair_jobs_pending_idx ON metadata_repair_jobs(collection_slug, status) WHERE status = 'pending'`).catch(()=>{});

  // Price alerts — user watching specific tokens
  await q(`CREATE TABLE IF NOT EXISTS user_price_alerts (
    id          SERIAL PRIMARY KEY,
    discord_id  TEXT NOT NULL,
    slug        TEXT NOT NULL,
    token_id    INT NOT NULL,
    threshold_eth NUMERIC(18,8) NOT NULL,
    alert_once  BOOLEAN DEFAULT true,
    repeat_alert BOOLEAN DEFAULT false,
    triggered_at TIMESTAMPTZ,
    created_at  TIMESTAMPTZ DEFAULT NOW()
  )`);
  await q(`ALTER TABLE user_price_alerts ADD COLUMN IF NOT EXISTS is_active BOOLEAN DEFAULT true`);
  await q(`CREATE INDEX IF NOT EXISTS user_price_alerts_token_idx ON user_price_alerts(slug, token_id)`);
  await q(`CREATE INDEX IF NOT EXISTS user_price_alerts_user_idx ON user_price_alerts(discord_id)`);

  // Floor alerts — user watching collection floor
  await q(`CREATE TABLE IF NOT EXISTS user_floor_alerts (
    id              SERIAL PRIMARY KEY,
    discord_id      TEXT NOT NULL,
    slug            TEXT NOT NULL,
    threshold_eth   NUMERIC(18,8) NOT NULL,
    repeat_alert    BOOLEAN DEFAULT true,
    cooldown_minutes INT DEFAULT 60,
    direction       TEXT DEFAULT 'below',
    last_alerted_at TIMESTAMPTZ,
    created_at      TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE(discord_id, slug)
  )`);
  await q(`ALTER TABLE user_floor_alerts ADD COLUMN IF NOT EXISTS cooldown_minutes INT DEFAULT 60`);
  await q(`ALTER TABLE user_floor_alerts ADD COLUMN IF NOT EXISTS direction TEXT DEFAULT 'below'`);
  await q(`ALTER TABLE user_floor_alerts ADD COLUMN IF NOT EXISTS is_active BOOLEAN DEFAULT true`);
  await q(`CREATE INDEX IF NOT EXISTS user_floor_alerts_slug_idx ON user_floor_alerts(slug)`);

  // Wallet sync status — tracks per-user per-collection sync progress
  await q(`CREATE TABLE IF NOT EXISTS wallet_sync_status (
    id           SERIAL PRIMARY KEY,
    discord_id   TEXT NOT NULL,
    wallet       TEXT NOT NULL,
    slug         TEXT NOT NULL,
    status       TEXT DEFAULT 'pending',
    token_count  INT DEFAULT 0,
    started_at   TIMESTAMPTZ DEFAULT NOW(),
    completed_at TIMESTAMPTZ,
    UNIQUE(discord_id, wallet, slug)
  )`);
  await q(`CREATE INDEX IF NOT EXISTS wss_discord_idx ON wallet_sync_status(discord_id)`);
  // nft_transfers — explicit ALTERs required for existing DBs
  await q(`CREATE TABLE IF NOT EXISTS nft_transfers (
    id              SERIAL PRIMARY KEY,
    token_id        INT,
    from_address    TEXT,
    to_address      TEXT,
    value_eth       NUMERIC(18,8) DEFAULT 0,
    block_number    INT,
    transferred_at  TIMESTAMPTZ,
    tx_hash         TEXT,
    collection_slug TEXT DEFAULT 'on-chain-all-stars'
  )`).catch(()=>{});
  await q(`ALTER TABLE nft_transfers ADD COLUMN IF NOT EXISTS token_id INT`).catch(()=>{});
  await q(`ALTER TABLE nft_transfers ADD COLUMN IF NOT EXISTS from_address TEXT`).catch(()=>{});
  await q(`ALTER TABLE nft_transfers ADD COLUMN IF NOT EXISTS to_address TEXT`).catch(()=>{});
  await q(`ALTER TABLE nft_transfers ADD COLUMN IF NOT EXISTS value_eth NUMERIC(18,8) DEFAULT 0`).catch(()=>{});
  await q(`ALTER TABLE nft_transfers ADD COLUMN IF NOT EXISTS block_number INT`).catch(()=>{});
  await q(`ALTER TABLE nft_transfers ADD COLUMN IF NOT EXISTS transferred_at TIMESTAMPTZ`).catch(()=>{});
  await q(`ALTER TABLE nft_transfers ADD COLUMN IF NOT EXISTS tx_hash TEXT`).catch(()=>{});
  await q(`ALTER TABLE nft_transfers ADD COLUMN IF NOT EXISTS collection_slug TEXT DEFAULT 'on-chain-all-stars'`).catch(()=>{});
  await q(`UPDATE nft_transfers SET collection_slug='on-chain-all-stars' WHERE collection_slug IS NULL`).catch(()=>{});
  await q(`ALTER TABLE nft_transfers ADD COLUMN IF NOT EXISTS contract TEXT`).catch(()=>{});
  await q(`UPDATE nft_transfers SET contract='0x078be86f3104a32313a47815792230a3808642cc' WHERE contract IS NULL`).catch(()=>{});
  await q(`ALTER TABLE nft_transfers ADD COLUMN IF NOT EXISTS log_index INT DEFAULT 0`).catch(()=>{});
  await q(`UPDATE nft_transfers SET log_index=0 WHERE log_index IS NULL`).catch(()=>{});
  // block_ts/event_type: read by /db/token/:id/history and /db/wallet/:address/transfers
  // (both COALESCE block_ts with transferred_at, and pass event_type through for the
  // frontend's own from/to-zero-address mint/burn classifier) but never actually
  // defined here -- only ever added out-of-band on OCAS's own long-lived database.
  // Any collection provisioned from this stub alone (e.g. argonauts) was missing
  // both columns outright, which 500'd instead of just returning empty/NULL rows.
  await q(`ALTER TABLE nft_transfers ADD COLUMN IF NOT EXISTS block_ts TIMESTAMPTZ`).catch(()=>{});
  await q(`ALTER TABLE nft_transfers ADD COLUMN IF NOT EXISTS event_type TEXT`).catch(()=>{});
  await q(`CREATE UNIQUE INDEX IF NOT EXISTS nft_transfers_tx_token_slug_idx ON nft_transfers(tx_hash, token_id, collection_slug) WHERE tx_hash IS NOT NULL`).catch(()=>{});
  await q(`CREATE INDEX IF NOT EXISTS nft_transfers_to_idx ON nft_transfers(to_address, collection_slug)`).catch(()=>{});
  await q(`CREATE INDEX IF NOT EXISTS nft_transfers_from_idx ON nft_transfers(from_address, collection_slug)`).catch(()=>{});

  // wallet_token_intervals — explicit ALTERs for existing DBs
  await q(`CREATE TABLE IF NOT EXISTS wallet_token_intervals (
    id              SERIAL PRIMARY KEY,
    wallet_address  TEXT,
    token_id        INT,
    acquired_at     TIMESTAMPTZ,
    disposed_at     TIMESTAMPTZ,
    cost_eth        NUMERIC(18,8) DEFAULT 0,
    sale_eth        NUMERIC(18,8),
    collection_slug TEXT DEFAULT 'on-chain-all-stars'
  )`).catch(()=>{});
  await q(`ALTER TABLE wallet_token_intervals ADD COLUMN IF NOT EXISTS wallet_address TEXT`).catch(()=>{});
  await q(`ALTER TABLE wallet_token_intervals ADD COLUMN IF NOT EXISTS token_id INT`).catch(()=>{});
  await q(`ALTER TABLE wallet_token_intervals ADD COLUMN IF NOT EXISTS acquired_at TIMESTAMPTZ`).catch(()=>{});
  await q(`ALTER TABLE wallet_token_intervals ADD COLUMN IF NOT EXISTS disposed_at TIMESTAMPTZ`).catch(()=>{});
  await q(`ALTER TABLE wallet_token_intervals ADD COLUMN IF NOT EXISTS cost_eth NUMERIC(18,8) DEFAULT 0`).catch(()=>{});
  await q(`ALTER TABLE wallet_token_intervals ADD COLUMN IF NOT EXISTS sale_eth NUMERIC(18,8)`).catch(()=>{});
  await q(`ALTER TABLE wallet_token_intervals ADD COLUMN IF NOT EXISTS collection_slug TEXT DEFAULT 'on-chain-all-stars'`).catch(()=>{});
  await q(`UPDATE wallet_token_intervals SET collection_slug='on-chain-all-stars' WHERE collection_slug IS NULL`).catch(()=>{});
  await q(`CREATE UNIQUE INDEX IF NOT EXISTS wallet_token_intervals_unique ON wallet_token_intervals(wallet_address, token_id, acquired_at, collection_slug) WHERE acquired_at IS NOT NULL`).catch(()=>{});
  await q(`CREATE INDEX IF NOT EXISTS wti_wallet_idx ON wallet_token_intervals(wallet_address, collection_slug)`).catch(()=>{});

  await q(`CREATE TABLE IF NOT EXISTS skipped_listing_batches (
    id              SERIAL PRIMARY KEY,
    guild_id        TEXT,
    collection_slug TEXT,
    listing_count   INT,
    token_ids       JSONB,
    reset_cursor_to TEXT,
    created_at      TIMESTAMPTZ DEFAULT NOW()
  )`);

  await q(`CREATE TABLE IF NOT EXISTS wallet_favorites (
    wallet_address  TEXT NOT NULL,
    collection_slug TEXT NOT NULL,
    token_ids       JSONB NOT NULL DEFAULT '[]',
    updated_at      TIMESTAMPTZ DEFAULT NOW(),
    PRIMARY KEY (wallet_address, collection_slug)
  )`);

  console.log('[DB] Migrations complete');
}

// ── Persistence helpers (bot_state table) ────────────────────────────────────
// Only these two keys have a real on-disk JSON file equivalent to fall back to.
// Every other key (sale_cursors, listing_cursors, burn_config, etc.) has no file
// backing it — the fallback must NOT silently substitute unrelated data for them.
const FILE_FALLBACKS = {
  server_configs: () => SERVER_FILE,
  user_alerts:    () => ALERTS_FILE,
};

async function dbLoad(key){
  try{
    const r = await pgPool.query('SELECT value FROM bot_state WHERE key=$1', [key]);
    if(r.rows.length) return JSON.parse(r.rows[0].value);
  }catch(e){ console.error(`[dbLoad] ${key} failed:`, e.message); }
  // Fallback to JSON file — only for keys that actually have one.
  const fileFor = FILE_FALLBACKS[key];
  if(fileFor){
    try{
      const file = fileFor();
      if(fs.existsSync(file)) return JSON.parse(fs.readFileSync(file, 'utf8'));
    }catch(e){ console.error(`[dbLoad] ${key} file fallback failed:`, e.message); }
  }
  return null;
}

async function dbSave(key, value){
  try{
    await pgPool.query(
      `INSERT INTO bot_state(key,value) VALUES($1,$2)
       ON CONFLICT(key) DO UPDATE SET value=EXCLUDED.value`,
      [key, typeof value === 'string' ? value : JSON.stringify(value)]
    );
  }catch(e){
    console.warn('[dbSave]', e.message);
  }
}

// ── Skipped-listing tracking ──────────────────────────────────────────────────
// Records a batch of listings that pollListings() detected but did NOT post,
// because the backlog exceeded the 50-listing startup-catchup cap (see
// lib/poll.js). Non-blocking / best-effort — this is diagnostic data, it must
// never affect the poll loop itself.
async function recordSkippedListings({ guildId, slug, tokenIds, resetCursorTo }){
  try{
    await pgPool.query(
      `INSERT INTO skipped_listing_batches (guild_id, collection_slug, listing_count, token_ids, reset_cursor_to)
       VALUES ($1,$2,$3,$4,$5)`,
      [guildId, slug, tokenIds.length, JSON.stringify(tokenIds), resetCursorTo]
    );
  }catch(e){
    console.warn('[recordSkippedListings]', e.message);
  }
}

// ── Config helpers ────────────────────────────────────────────────────────────
let _serverConfigs = {};
let _userAlerts    = {};

async function loadAllConfigs(){
  // Load server configs from server_configs table (primary source)
  try{
    const rows = await pgPool.query('SELECT guild_id, config FROM server_configs');
    for(const row of rows.rows){
      _serverConfigs[row.guild_id] = typeof row.config === 'string' ? JSON.parse(row.config) : row.config;
    }
  }catch(e){
    console.warn('[loadAllConfigs] DB read failed, falling back to bot_state:', e.message);
    let db = await dbLoad('server_configs');
    if(db) _serverConfigs = (typeof db === 'string') ? JSON.parse(db) : db;
  }
  // Load user alerts from bot_state
  let ua = await dbLoad('user_alerts');
  if(ua){
    _userAlerts = (typeof ua === 'string') ? JSON.parse(ua) : ua;
  }
}

function getConfig(guildId){ return _serverConfigs[guildId] || {}; }

function getAllConfigs(){ return Object.entries(_serverConfigs); }

async function setConfig(guildId, cfg){
  // Always merge with existing DB config to prevent partial overwrites
  // This protects against memory loading incomplete config then saving it
  try{
    const existing = await pgPool.query({
      text: 'SELECT config FROM server_configs WHERE guild_id=$1',
      values: [guildId],
      query_timeout: 3000
    });
    if(existing.rows.length){
      const dbCfg = typeof existing.rows[0].config === 'string'
        ? JSON.parse(existing.rows[0].config)
        : (existing.rows[0].config || {});
      // Merge: incoming cfg takes priority but preserves DB keys not in cfg
      // Special case: collections array — if incoming cfg has it, use it exactly (no merge)
      // This prevents deleted collections from being restored from DB state
      const incomingCollections = cfg.collections; // save before spread
      cfg = { ...dbCfg, ...cfg };
      if(incomingCollections !== undefined) cfg.collections = incomingCollections;
    }
  }catch(e){ console.warn('[setConfig] merge fetch failed:', e.message); }

  _serverConfigs[guildId] = cfg;
  // Persist to DB
  try{
    await pgPool.query(
      `INSERT INTO server_configs(guild_id,config) VALUES($1,$2)
       ON CONFLICT(guild_id) DO UPDATE SET config=EXCLUDED.config`,
      [guildId, JSON.stringify(cfg)]
    );
  }catch(e){
    console.warn('[setConfig]', e.message);
    await dbSave('server_configs', _serverConfigs);
  }
}

// getConfig() reads only from the in-memory _serverConfigs cache, never the
// DB directly -- deleting the DB row alone would leave the bot still
// behaving as if the old config existed until a restart repopulated the
// cache. This clears both together.
async function deleteConfig(guildId){
  delete _serverConfigs[guildId];
  try{
    await pgPool.query(`DELETE FROM server_configs WHERE guild_id=$1`, [guildId]);
  }catch(e){
    console.warn('[deleteConfig]', e.message);
  }
}

function getUserAlerts(){ return _userAlerts; }

async function setUserAlerts(alerts){
  _userAlerts = alerts;
  await dbSave('user_alerts', alerts);
}

// ── Sale cursor ───────────────────────────────────────────────────────────────
let _saleCursor = null;

async function loadSaleCursor(){
  const v = await dbLoad('last_sale_ts');
  _saleCursor = v || null;
  return _saleCursor;
}

async function saveSaleCursor(ts){
  _saleCursor = ts;
  await dbSave('last_sale_ts', ts);
}

function getSaleCursor(){ return _saleCursor; }

// ── Collection trait cache (OpenSea → collection_traits table) ────────────────
// Single source of truth — used by config.js, setup.js, any future call site.
// Skips the OS API call entirely if collection_traits already has rows for
// this slug (dedup check). Force-refetch can be added later if needed.
//
// Returns a result object so callers that actually need to know what happened
// (e.g. the /db/collections/:slug/sync-trait-index diagnostic endpoint) can
// report it — fire-and-forget callers elsewhere (.catch(()=>{})) are
// unaffected since they already ignored the return value.
async function fetchAndStoreCollectionTraits(slug, pgPool){
  if(!slug) return { ok:false, reason:'no slug provided' };
  try{
    // Dedup: if we already have trait rows for this slug, skip the OS call.
    const existing = await pgPool.query(
      `SELECT 1 FROM collection_traits WHERE slug=$1 LIMIT 1`, [slug]
    ).catch(()=>null);
    if(existing?.rows?.length){
      console.log(`[TraitCache] ${slug} already cached — skipping OS fetch`);
      return { ok:true, skipped:true, reason:'already cached' };
    }
    const { osHeaders } = require('./constants');
    const fetch = require('node-fetch');
    const res = await fetch(
      `https://api.opensea.io/api/v2/traits/${slug}`,
      { headers: osHeaders() }
    );
    if(!res.ok){
      const bodyText = await res.text().catch(()=>'');
      console.warn('[TraitCache] OS traits fetch failed:', res.status, slug, bodyText.slice(0,200));
      return { ok:false, reason:`OpenSea returned HTTP ${res.status}`, status: res.status, body: bodyText.slice(0,200) };
    }
    const data = await res.json();
    const counts = data.counts || {};
    let count = 0;
    for(const [traitName, valueCounts] of Object.entries(counts)){
      if(typeof valueCounts !== 'object' || Array.isArray(valueCounts)) continue;
      for(const [val, cnt] of Object.entries(valueCounts)){
        await pgPool.query(
          `INSERT INTO collection_traits (slug, trait_name, trait_value, token_count)
           VALUES ($1,$2,$3,$4)
           ON CONFLICT (slug, trait_name, trait_value) DO UPDATE SET token_count=$4`,
          [slug, traitName, val, cnt]
        ).catch(()=>{});
        count++;
      }
    }
    console.log(`[TraitCache] Stored ${count} trait values for ${slug}`);
    if(!count) return { ok:false, reason:'OpenSea returned 0 traits for this slug — check the slug is exactly right', rawKeys: Object.keys(data||{}) };
    return { ok:true, count };
  }catch(e){
    console.warn('[TraitCache] Error fetching traits for', slug, ':', e.message);
    return { ok:false, reason: e.message };
  }
}

module.exports = {
  pgPool, pgQueryCommand, pgQueryPoller,
  runMigrations,
  dbLoad, dbSave,
  loadAllConfigs, getConfig, setConfig, deleteConfig, getAllConfigs,
  getUserAlerts, setUserAlerts,
  loadSaleCursor, saveSaleCursor, getSaleCursor,
  fetchAndStoreCollectionTraits,
  recordSkippedListings,
};








