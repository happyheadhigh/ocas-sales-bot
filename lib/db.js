...
async function runMigrations(){
  ...
  // Stackers per-token status cache — tier, active status, and split, kept
  // genuinely current via a poller watching dedicated on-chain events
  // (Activated, Deactivated, TierUpgraded, SplitSet) rather than needing a
  // slow periodic sweep. Deliberately excludes vault balance, which has no
  // confirmed event mapping yet and still relies on the full snapshot.
