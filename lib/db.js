...
async function runMigrations(){
  ...
  // Persistent dedupe for market events posted via the OpenSea Stream
  // connector (lib/opensea-stream.js). The existing in-memory dedupe in
  // lib/cache.js resets on every restart — fine for the REST poller (which
  // re-derives "new" from a saved cursor either way), but not safe for
  // Stream, where a restart mid-flight could otherwise cause a repeat post.
  await q(`CREATE TABLE IF NOT EXISTS posted_market_events (
    channel_id  TEXT NOT NULL,
    token_id    TEXT NOT NULL,
    event_type  TEXT NOT NULL,
    posted_at   TIMESTAMPTZ DEFAULT NOW(),
    PRIMARY KEY (channel_id, token_id, event_type)
  )`);
