...
async function runMigrations(){
  ...
  try{
    const constraintsRes = await pgPool.query(`
      SELECT con.conname, con.contype, array_agg(a.attname ORDER BY a.attnum) AS cols
      FROM pg_constraint con
      JOIN pg_attribute a ON a.attrelid = con.conrelid AND a.attnum = ANY(con.conkey)
      WHERE con.conrelid = 'token_traits'::regclass AND con.contype IN ('p', 'u')
      GROUP BY con.conname, con.contype
    `);
    const tooNarrow = constraintsRes.rows.filter(r => r.cols.sort().join(',') === 'token_id,trait_index');
    if(tooNarrow.length){
