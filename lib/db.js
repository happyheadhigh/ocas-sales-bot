...
async function runMigrations(){
  ...
  await q(`DO $$
    BEGIN
      ...
      END IF;
    END $$`).catch(()=>{});
  await q(`ALTER TABLE user_registrations DROP CONSTRAINT IF EXISTS user_registrations_pkey`).catch(()=>{});
  await q(`ALTER TABLE user_registrations ADD CONSTRAINT user_registrations_pkey PRIMARY KEY (discord_id, guild_id)`).catch(()=>{});
  ...
  await q(`ALTER TABLE verification_codes DROP CONSTRAINT IF EXISTS verification_codes_pkey`).catch(()=>{});
  await q(`ALTER TABLE verification_codes ADD CONSTRAINT verification_codes_pkey PRIMARY KEY (discord_id, guild_id)`).catch(()=>{});
...
async function setConfig(guildId, cfg){
  ...
  try{
    const existing = await pgPool.query(
      'SELECT config FROM server_configs WHERE guild_id=$1', [guildId]
    );
...
module.exports = {
  pgPool, pgQueryCommand, pgQueryPoller,pgPool,
  runMigrations,
  dbLoad, dbSave,
