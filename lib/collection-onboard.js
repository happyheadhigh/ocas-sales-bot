...
async function onboardCollection(pgPool, rawSlug){
  ...
  await pgPool.query(`
    INSERT INTO collections (slug, contract, chain, name, status, total_supply, token_standard, opensea_url, website_url, twitter_url)
...
// opensea_url/website_url/twitter_url fields for an already-onboarded
// collection, without touching its status or triggering any backfill work.
// Exists specifically for Argonauts (and any other collection onboarded
// before these three columns existed at all) -- reuses
...
async function backfillCollectionLinks(pgPool, slug){
  ...
  await pgPool.query(
    `UPDATE collections SET opensea_url=$2, website_url=$3, twitter_url=$4, updated_at=NOW() WHERE slug=$1`,
    [resolved.slug, `https://opensea.io/collection/${resolved.slug}`, resolved.websiteUrl, resolved.twitterUrl]
  );
  return { slug: resolved.slug, websiteUrl: resolved.websiteUrl, twitterUrl: resolved.twitterUrl };
}
