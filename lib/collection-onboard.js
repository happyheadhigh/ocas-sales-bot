...
// Lightweight companion to onboardCollection() -- updates the
// opensea_url/website_url/twitter_url/avatar_image_url/banner_image_url
// fields for an already-onboarded collection, without triggering any
// actual backfill work. Exists specifically for Argonauts (and any other
// collection onboarded before these columns existed at all) -- reuses
// resolveCollectionForOnboarding()'s own OpenSea fetch rather than
// duplicating that lookup logic.
//
// jv confirmed live: Argonauts' own registry row was stuck at
// status='pending' with traits_synced_at/market_synced_at both NULL --
// stale bookkeeping from however it was onboarded before this registry's
// status-tracking existed, despite Argonauts being fully operational in
// every other respect (real listings, sales, ranks -- all working the
// entire time this session). Silently gated three things that all check
// WHERE status='ready' and nothing else: lib/burn-detect.js's burn
// poller, lib/rank-sync.js's periodic re-rank, and lib/sale-stream.js's
// real-time sale stream -- none of which had ever actually run for
// Argonauts. Running this endpoint at all is itself the signal that the
// collection is meant to be treated as fully operational, so it now also
// corrects status (only forward, from a still-pending-looking state to
// ready -- never overwrites a genuinely different status like 'failed'
// or a live 'backfilling_*' run in progress) and the two sync timestamps.
async function backfillCollectionLinks(pgPool, slug){
  const resolved = await resolveCollectionForOnboarding(slug);
  await pgPool.query(
    `UPDATE collections SET opensea_url=$2, website_url=$3, twitter_url=$4, avatar_image_url=$5, banner_image_url=$6,
       status = CASE WHEN status IN ('pending', 'ready') THEN 'ready' ELSE status END,
       traits_synced_at = COALESCE(traits_synced_at, NOW()),
       market_synced_at = COALESCE(market_synced_at, NOW()),
       updated_at=NOW()
     WHERE slug=$1`,
