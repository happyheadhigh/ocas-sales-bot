...
function buildArbitrageEmbed({ slug, tokenId, listingPriceEth, offerPriceEth, spreadEth, contract = null, chain = 'ethereum', name }){
  // Confirmed live: this previously used slug in the URL path where
  // OpenSea's actual asset URL format expects a contract address
  // (https://opensea.io/assets/{chain}/{CONTRACT}/{tokenId}) -- every
  // "View on OpenSea" link this embed ever produced was broken since this
  // feature was first built. Falls back to a collection-level link (still
  // useful, just not token-specific) if contract is ever unavailable,
  // rather than building another broken link with an empty segment.
  const osUrl = contract
