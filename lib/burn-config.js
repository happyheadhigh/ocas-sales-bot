...
const { checkCommandCooldown, fetchBotApiJson, slideshowSessions } = require('./cache');
...
async function loadBurnConfig(){
  // Retry up to 3 times with 5s delay — DB may not be ready at startup
  for(let attempt = 1; attempt <= 3; attempt++){
    ...
    if(attempt < 3){
      console.warn(`[Burn] Config load attempt ${attempt} returned null — retrying in 5s...`);
      await new Promise(r => setTimeout(r, 5000));
    }
  }
  // All retries failed — start with empty config, poller will still run
  burnConfig = {};
  console.error('[Burn] Config failed to load after 3 attempts — no alert channels configured');
}
...
async function getTraitIndex(RAILWAY_URL, API_SECRET, slug=null){
  const cacheKey = slug || '__ocas_default__';
  global.__traitIndexCache = global.__traitIndexCache || {};
  const now = Date.now();
  const cached = global.__traitIndexCache[cacheKey];
  if(cached && now - cached.ts < 60*60*1000)
    return cached.items;
  if(!RAILWAY_URL) return [];
  const qs = new URLSearchParams({ key: API_SECRET||"" });
  if(slug) qs.set('slug', slug);
  const r = await fetch(`${RAILWAY_URL}/db/trait-index?${qs}`, { timeout: 15000 });
  ...
  global.__traitIndexCache[cacheKey] = { ts:now, items };
  return items;
}
...
function sweepTokenUrl(item, collCfg){
  const tokenId = getSweepTokenId(item);
  // jv: "make sure ... all links lead to correct links via it be OpenSea
  // or traitview." This used to hardcode OCAS's own contract address
  // directly -- harmless for OCAS itself, but /sweep is explicitly
  // available for non-OCAS collections on a paid tier, so any other
  // collection's sweep silently built an OpenSea link pointing at the
  // wrong contract entirely. collCfg (threaded from the sweep session,
  // set at /sweep time) is the real, per-collection contract/chain --
  // falls back to OCAS's own values only when collCfg isn't provided at
  // all, so any caller that genuinely is OCAS-only keeps working
  // unchanged.
  const contract = collCfg?.contract || '0x078be86f3104a32313a47815792230a3808642cc';
  const chain = collCfg?.chain || 'ethereum';
  const slug = collCfg?.slug || 'on-chain-all-stars';
  return tokenId ? ('https://opensea.io/assets/' + chain + '/' + contract + '/' + tokenId) : ('https://opensea.io/collection/' + slug);
}

function formatSweepTokenLine(item, collCfg){
  const tokenId = getSweepTokenId(item);
  const rank = item?.os_rank ? ('⬥' + Number(item.os_rank).toLocaleString()) : (item?.obs_rank ? ('⬥' + Number(item.obs_rank).toLocaleString()) : null);
  const tokenLink = '[#' + tokenId + '](' + sweepTokenUrl(item, collCfg) + ')';
...
module.exports = {
  ...
  cachedFloors,
  // Found while fixing sweep's hardcoded-OCAS-contract link bug: neither
  // of these was ever actually exported, and bot.js's sweep-pagination
  // handler (the sweep:next/prev/showall button interactions) called
  // formatSweepTokenLine without importing it from anywhere at all --
  // meaning every click on those buttons has been throwing a plain
  // ReferenceError this whole time, unrelated to the link bug itself.
  formatSweepTokenLine, sweepTokenUrl,
};
