...
// Starting chunk size is now adaptive — see lib/adaptive-block-chunk.js for
// why. Starts optimistic (2000) and shrinks automatically toward the
// known-safe floor (10, the confirmed Ethereum-account limit this was
// originally copied from) only if Robinhood Chain actually rejects a range
// that large. Module-level so it persists across calls within this
// process's lifetime, not just within one poll cycle.
const chunkSizer = new AdaptiveChunkSizer({ initial: 2000, floor: 10, label: 'StackersFusion' });
...
async function pollFusionEvents(maxChunksOverride){
  try{
    ...
    // to true real-time parity given fusion is worth catching promptly.
    // Chunk size itself is adaptive now — see chunkSizer above.
    ...
    while(cursor <= safeLatest && chunksProcessed < MAX_CHUNKS_PER_CYCLE){
      ...
      let events;
      try{
        events = await vault.queryFilter(filter, cursor, chunkTo);
      }catch(e){
