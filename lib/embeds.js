...
async function buildSaleEmbed(sale, config){
  ...
  // Fall back to dbMeta's image if the sale event's own nft data doesn't have
  // one — needed for collections where OpenSea's own event data is thin
  // (seen on a brand-new chain/collection), same reasoning as the trait
  // fallback just below.
