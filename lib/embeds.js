...
async function buildSaleEmbed(sale, config){
  ...
  // fallback just below.
  const saleNftForImage = (sale.nft?.image_url || sale.nft?.display_image_url || sale.nft?.image_preview_url)
    ? sale.nft
    : { ...sale.nft, image_url: dbMeta?.image_url || null };
...
async function buildListingEmbed(listing, config){
  ...
  const nftLike = {
    identifier:        id,
    image_url:         asset.image_url || dbMeta?.image_url || null,
