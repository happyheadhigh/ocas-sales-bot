...
async function resolveOnChainImage(contract, tokenId, chain){
  ...
  try{
    ...
    if(isSvg(image)){
      ...
      qs.set(isDataUri ? 'svgData' : 'svgUrl', image);
      return `${RAILWAY_URL}/render/svg-token?${qs}`;
    }
    return isDiscordOk(image) ? image : null;
  }catch(e){
...
async function buildSaleEmbed(sale, config){
  ...
  const saleNftForImage = { ...sale.nft, image_url: onChainImage || dbMeta?.image_url || sale.nft?.image_url || null };
...
async function buildListingEmbed(listing, config){
  ...
  const nftLike = {
    identifier:        id,
    image_url:         onChainImage || dbMeta?.image_url || asset.image_url || null,
