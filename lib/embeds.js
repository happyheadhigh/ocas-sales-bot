...
const { isDiscordOk, isSvg } = require('../utils/format');
...
// Confirmed live: Alchemy's CDN can serve raw SVG content through a URL with
// zero textual indication of that — no .svg extension, no "image/svg"
// substring anywhere in the URL string, just a hash-like path
// (nft2-cdn.alchemy.com/eth-mainnet/<hash>). isDiscordOk/needsSvgRender's
// pattern-matching approach can never catch this, since there is nothing in
// the URL itself to match against — the only reliable way to know is to
// actually check what the server says it's serving. This is why every sale
// alert for an affected token resolved a "valid" URL (isDiscordOk passed,
// no error anywhere) and still never rendered: Discord's embed
// thumbnail/image fields don't support SVG at all, only raster formats.
// A HEAD request (no body download) is enough — checks the real
// content-type, not the URL shape. If the check itself fails for any
// reason (network hiccup, server doesn't support HEAD), falls back to
// treating it as fine rather than losing a good image over an unrelated
...
async function fetchTokenMetaFromDb(tokenId, slug=OCAS_SLUG){
  ...
  try{
    ...
    cacheTokenMeta(cacheKey, { meta, expires: Date.now() + 5 * 60 * 1000 });
    return meta;
  }catch(e){
...
async function resolveOnChainImage(contract, tokenId, chain){
  ...
  try{
    ...
    return isDiscordOk(image) ? { type: 'url', url: image } : null;
  }catch(e){
